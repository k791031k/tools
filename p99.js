javascript:(function() {
    ...
    /**
     * =================================================================================
     * KGI Plan Code Query Tool v1.0.0
     *
     * @version 1.0.0
     * @author Optimized Version
     *
     * v1.0.0 Release Notes:
     * - [主要功能] 新增「全表預覽」功能，可在新分頁中以超寬表格形式檢視所有原始 API 欄位。
     * - [主要功能] 在「全表預覽」頁面新增「欄位置換」功能，可將表格行列對調。
     * - [主要功能] 重製「銷售通路」欄位的顯示與提示方式，使其更加清晰、資訊完整。
     * - [功能增強] 在「主約銷售時間」查詢條件中，增加「全部」選項。
     * - [欄位修正] 修正「商品名稱」全名切換功能，使其正確對應 `careCodeName` 欄位。
     * - [樣式修正] 整合並修正所有 CSS，確保顏色、寬度、欄位對齊、標題列固定等功能完全正常。
     * - [穩定性修正] 修正 Token 輸入框不會再被錯誤地轉為大寫。
     * =================================================================================
     */
    ...
    const UIModule = (() => {
        const injectStyle = () => {
            ...
            style.textContent = `
              ...
              .pct-modal-close-btn { position: absolute; top: 10px; right: 10px; background: transparent; border: none; font-size: 28px; font-weight: bold; color: var(--secondary-color); cursor: pointer; width: 36px; height: 36px; border-radius: 50%; transition: background-color .2s, color .2s, transform .2s; display: flex; align-items: center; justify-content: center; line-height: 1; }
              ...
              .pct-modal[data-size="results"] .pct-modal-body { overflow-y: hidden; }
              ...
              .pct-btn.pct-btn-outline:hover { background-color: var(--background-light); transform: translateY(-2px); }
              ...
              .pct-mode-card::before { content: ''; position: absolute; top: -50%; right: -50%; width: 100%; height: 100%; background: radial-gradient(circle, rgba(74, 144, 226, 0.1) 0%, transparent 70%); opacity: 0; transition: opacity 0.3s ease; pointer-events: none; }
              ...
              .pct-mode-card.selected:hover { transform: translateY(-4px) scale(1.01); box-shadow: 0 12px 32px rgba(74, 144, 226, 0.4); }
              .pct-mode-card:active { transform: scale(0.95); transition: all 0.1s ease; }
              @media (max-width: 900px) { .pct-modal[data-size="query"] { width: 90vw; margin: 20px; } .pct-mode-card-grid { grid-template-columns: repeat(2, 1fr); gap: 12px; } }
              @media (max-width: 600px) { .pct-mode-card-grid { grid-template-columns: 1fr; } .pct-modal { width: 95vw; margin: 10px; } }
              ...
              .pct-table th:nth-child(3) { width: 25%; }
              ...
              .pct-table th:nth-child(9) { width: 9%; }
              .pct-table th:nth-child(10) { width: 7%; }
    ...
    const DataModule = (() => {
        ...
        const mergeData = () => {
            ...
            const mergedData = masterDataCache.map((rawMasterItem) => {
                ...
                const processedItem = {
                    planCode,
                    fullName: rawMasterItem.careCodeName || rawMasterItem.planName || rawMasterItem.shortName || '-',
                    displayName: rawMasterItem.shortName || rawMasterItem.careCodeName || rawMasterItem.planName || '-',
    ...
    const ControllerModule = (() => {
        ...
        const showTokenDialog = (showRetryButton = false) => {
            ...
            <div class="pct-retry-section" style="margin-bottom: 15px; text-align: center;">
                <button id="pct-retry-auto-check" class="pct-btn pct-btn-outline">♻️ 重新檢測 TOKEN</button>
                <small style="display: block; margin-top: 5px; color: #666;">如果您剛完成登入，可點擊此按鈕重新檢測</small>
            </div>` : '';
            const html = `
              <div class="pct-modal-header">商品查詢 (${env})<button class="pct-modal-close-btn">&times;</button></div>
        ...
        const showQueryDialog = (preserveState = false) => {
            ...
            UIModule.Modal.show(html, (modal) => {
                ...
                const updateDynamicOptions = (mode) => {
                    ...
                    switch (mode) {
                        ...
                        case ConfigModule.QUERY_MODES.PLAN_NAME:
                            ...
                            break;
                        case ConfigModule.QUERY_MODES.MASTER_CLASSIFIED:
                            const allOption = `<div class="pct-sub-option" data-status="all"><strong>全部</strong></div>`;
                            const statusOptions = Object.values(ConfigModule.MASTER_STATUS_TYPES).map(s => `<div class="pct-sub-option" data-status="${s}">${ConfigModule.MASTER_STATUS_TEXT[s] || s}</div>`).join('');
                            content = `<label>主約銷售狀態：</label><div class="pct-sub-option-grid master-status">${allOption}${statusOptions}</div>`;
                            break;
                        case ConfigModule.QUERY_MODES.CHANNEL_CLASSIFIED:
                            content = `
                                <label>選擇通路：</label>
                ...
                const bindDynamicEvents = () => {
                    const masterStatusOptions = document.querySelectorAll('.pct-sub-option[data-status]');
                    masterStatusOptions.forEach(o => o.addEventListener('click', () => {
                        const status = o.dataset.status;
                        const allBtn = document.querySelector('.pct-sub-option[data-status="all"]');
                        const individualOptions = Array.from(masterStatusOptions).filter(opt => opt.dataset.status !== 'all');
                        if (status === 'all') {
                        ...
                        const selected = new Set(Array.from(document.querySelectorAll('.pct-sub-option[data-status].selected')).map(el => el.dataset.status).filter(s => s !== 'all'));
                    ...
                    document.querySelectorAll('#pct-dynamic-options .pct-input').forEach(input => input.addEventListener('input', EventModule.autoFormatInput));
                    document.querySelectorAll('#pct-dynamic-options .pct-input').forEach(input => input.addEventListener('input', checkCanStartQuery));
                };
                    ...
                    if (queryMode) {
                        ...
                        if (modeCard) {
                            ...
                            if (queryMode === 'planCode' || queryMode === 'planCodeName') {
                                ...
                                const allBtn = document.querySelector('.pct-sub-option[data-status="all"]');
                                const individualOptions = Array.from(document.querySelectorAll('.pct-sub-option[data-status]')).filter(opt => opt.dataset.status !== 'all');
            ...
            const html = `
                      ...
                      <table class="pct-table">
                          <thead>
                              <tr>
                                  ...
                                  <th data-key="saleStartDate">主檔銷售日</th>
                                  <th data-key="saleEndDate">主檔停賣日</th>
                                  <th data-key="mainStatus">主檔狀態</th>
              ...
              <div class="pct-modal-footer">
                  <div class="pct-modal-footer-left" style="display:flex; align-items:center; gap:15px;">
                      ...
                      <button id="pct-load-all-polpln" class="pct-btn pct-btn-outline" disabled>[取得Name]</button>
                  ...
                  <div class="pct-modal-footer-right" style="display:flex; gap: 10px;">
                      <button id="pct-preview-all" class="pct-btn pct-btn-outline">全表預覽</button>
        ...
        const setupResultsDialog = (modal) => {
            ...
            document.getElementById('pct-preview-all').addEventListener('click', () => {
                const dataToPreview = DataModule.getFilteredData();
                if (dataToPreview.length === 0) {
                    UIModule.Toast.show('無資料可預覽', 'warning');
                    return;
                }
                
                const standardHeaders = Array.from(modal.querySelectorAll('.pct-table th')).map(th => th.textContent);
                const masterKeys = [...new Set(dataToPreview.flatMap(item => Object.keys(item._raw.master)))];
                const channelKeys = [...new Set(dataToPreview.flatMap(item => item._raw.channels.flatMap(c => Object.keys(c))))];
                
                let tableHtml = '<thead><tr>';
                tableHtml += standardHeaders.map(h => `<th>${UtilsModule.escapeHtml(h)}</th>`).join('');
                tableHtml += masterKeys.map(k => `<th>Master: ${UtilsModule.escapeHtml(k)}</th>`).join('');
                ConfigModule.FIELD_MAPS.CHANNELS.forEach(channel => {
                    channelKeys.forEach(key => {
                        tableHtml += `<th>${UtilsModule.escapeHtml(channel)}: ${UtilsModule.escapeHtml(key)}</th>`;
                    });
                });
                tableHtml += '</tr></thead>';

                tableHtml += '<tbody>';
                dataToPreview.forEach(item => {
                    ...
                    let channelsCellHtml;
                    if (item.channels.length === 0) {
                        channelsCellHtml = '【無資料】';
                    } else {
                        const channelDisplayData = ConfigModule.FIELD_MAPS.CHANNELS.map(channelCode => {
                            const data = item.channels.find(c => c.channel === channelCode);
                            return { channel: channelCode, status: data ? data.status : 'not sold' };
                        });
                        const statusOrder = { 'currently sold': 1, 'coming soon': 2, 'discontinued': 3, 'abnormal date': 4, 'not sold': 5 };
                        channelDisplayData.sort((a, b) => (statusOrder[a.status] || 99) - (statusOrder[b.status] || 99) || a.channel.localeCompare(b.channel));
                        channelsCellContent = channelDisplayData.map(ch => `<span class="${ch.status === 'currently sold' ? 'pct-channel-insale' : ch.status === 'not sold' ? 'pct-channel-notsold' : 'pct-channel-offsale'}">${ch.channel}</span>`).join(' ');
                    }

                    tableHtml += `<tr>`;
                    // Standard Cells
                    tableHtml += `<td>${item.no}</td>`;
                    tableHtml += `<td>${UtilsModule.escapeHtml(item.planCode)}</td>`;
                    tableHtml += `<td style="text-align: left;">${UtilsModule.escapeHtml(nameToShow)}</td>`;
                    tableHtml += `<td>${UtilsModule.escapeHtml(item.currency)}</td>`;
                    tableHtml += `<td>${UtilsModule.escapeHtml(item.unit)}</td>`;
                    tableHtml += `<td>${UtilsModule.escapeHtml(item.coverageType)}</td>`;
                    tableHtml += `<td>${UtilsModule.escapeHtml(item.saleStartDate)}</td>`;
                    tableHtml += `<td>${UtilsModule.escapeHtml(item.saleEndDate)}</td>`;
                    tableHtml += `<td>${UtilsModule.escapeHtml(ConfigModule.MASTER_STATUS_TEXT[item.mainStatus] || item.mainStatus)}</td>`;
                    tableHtml += `<td>${UtilsModule.escapeHtml(polplnValue)}</td>`;
                    tableHtml += `<td style="text-align: left;">${channelsCellContent}</td>`;

                    // Raw Master Cells
                    masterKeys.forEach(key => {
                        tableHtml += `<td>${UtilsModule.escapeHtml(item._raw.master[key])}</td>`;
                    });

                    // Raw Channel Cells
                    const productChannelsMap = new Map(item._raw.channels.map(c => [c.channel, c]));
                    ConfigModule.FIELD_MAPS.CHANNELS.forEach(channel => {
                        const channelData = productChannelsMap.get(channel);
                        channelKeys.forEach(key => {
                            tableHtml += `<td>${channelData ? UtilsModule.escapeHtml(channelData[key]) : ''}</td>`;
                        });
                    });
                    tableHtml += `</tr>`;
                });
                ...
                const transposeScript = `
                    <script>
                        const originalTableHTML = document.getElementById('preview-table').innerHTML;
                        ...
                        document.getElementById('transpose-btn').addEventListener('click', () => {
                            ...
                            for (let j = 0; j < numCols; j++) {
                                ...
                                for (let i = 0; i < numRows; i++) {
                                    newTableHTML += (i === 0 ? '<th>' : '<td>') + (rows[i].children[j] ? rows[i].children[j].innerHTML : '') + (i === 0 ? '</th>' : '</td>');
                                }
                                newTableHTML += '</tr>';
                            }
                            newTableHTML += '</tbody>';
                ...
                const previewHtml = `
                    ...
                    <head>
                        ...
                        <title>全表預覽</title>
                        <style>
                            body { font-family: 'Microsoft JhengHei', sans-serif; margin: 15px; }
                            table { border-collapse: collapse; width: 100%; font-size: 12px; }
                            th, td { border: 1px solid #999; padding: 6px; text-align: left; vertical-align: top; white-space: nowrap; }
                            thead th { background-color: #f0f0f0; text-align: center; position: sticky; top: 0; }
                    ...
                    <body>
                        <div class="preview-header">
                            <h2>查詢結果預覽 (共 ${dataToPreview.length} 筆)</h2>
                            <button id="transpose-btn" style="padding: 5px 15px; font-size: 14px;">欄位置換</button>
                        </div>
                        <div style="width: 100%; overflow-x: auto;">
                            <table id="preview-table">${tableHtml}</table>
                        </div>
                        ${transposeScript}
                    </body>
                    </html>`;
            ...
            document.getElementById('pct-copy-all').addEventListener('click', () => {
                ...
                const headers = ['No', '代號', '商品名稱', '幣別', '單位', '型態', '主檔銷售日', '主檔停賣日', '主檔狀態', 'POLPLN', '銷售通路'];
                const rows = dataToCopy.map(item => {
                    ...
                    return [
                        ...
                        item.saleStartDate, item.saleEndDate, item.mainStatus, StateModule.get().polplnDataCache.get(item.planCode) || '',
                        channelText
                    ];
                });
        ...
        const renderTableRow = (item, polplnDataCache) => {
            ...
            // --- 銷售通路欄位渲染邏輯 ---
            let channelsCellContent, tooltipText;
            if (item.channels.length === 0) {
                channelsCellContent = '【無資料】';
                tooltipText = '此商品無通路銷售資料';
            } else {
                const fixedChannels = ConfigModule.FIELD_MAPS.CHANNELS;
                const productChannelsMap = new Map(item.channels.map(c => [c.channel, c]));
                
                const tooltipLines = fixedChannels.map(channelCode => {
                    if (productChannelsMap.has(channelCode)) {
                        const ch = productChannelsMap.get(channelCode);
                        const statusText = ConfigModule.MASTER_STATUS_TEXT[ch.status] || ch.status;
                        return `${channelCode} 【${statusText}】 • 起日: ${ch.saleStartDate || '無'} | 迄日: ${ch.saleEndDate || '無'}`;
                    } else {
                        return `${channelCode} 【未銷售】`;
                    }
                });
                tooltipText = tooltipLines.join('&#10;');

                const channelDisplayData = fixedChannels.map(channelCode => {
                ...
                channelsCellContent = channelDisplayData.map(ch => {
                    let className = '';
                    if (ch.status === 'currently sold') className = 'pct-channel-insale';
                    else if (ch.status === 'not sold') className = 'pct-channel-notsold';
                    else className = 'pct-channel-offsale';
                    return `<span class="${className}">${ch.channel}</span>`;
                }).join(' ');
            }
        ...
        const renderStatusPill = (status) => {
            const config = {
                ...
                [ConfigModule.MASTER_STATUS_TYPES.DISCONTINUED]: { e: '⛔️' },
