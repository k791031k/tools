javascript:(function() {
    ...
    /**
     * =================================================================================
     * KGI Plan Code Query Tool v29.8.0 - Final Build
     *
     * @version 2025.07.26 - Final Build
     * @author Optimized Version
     *
     * 更新內容 (v29.8.0):
     * - 修正：徹底解決結果表格標題列(Header)無法固定的問題，將修正與用戶最新CSS結合。
     * - 重製：「全表預覽」功能，現在會在新分頁附加一欄，顯示最原始的API回傳資料(Master/Channel)。
     * - 調整：修改資料合併邏輯，以保留原始API資料供預覽使用。
     * - (及先前版本所有修改)
     * =================================================================================
     */
    ...
    const UIModule = (() => {
        const injectStyle = () => {
            ...
            // [v29.8.0] 整合使用者提供的最新CSS與Sticky Header修正
            style.textContent = `
              /* CSS 變數定義 */
              :root { 
              ...
              .pct-modal[data-size="results"] .pct-modal-body { overflow-y: hidden; } /* 標題列固定關鍵修正 */
    ...
    const ControllerModule = (() => {
        ...
        const setupResultsDialog = (modal) => {
            ...
            document.getElementById('pct-preview-all').addEventListener('click', () => {
                ...
                const headers = [...Array.from(modal.querySelectorAll('.pct-table th')).map(th => th.textContent), '原始API資料'];

                const tableRows = dataToPreview.map(item => {
                    ...
                    if (item.channels.length === 0) {
                        ...
                        channelsCellHtml = fixedChannels.map(channelCode => {
                    ...
                    const rawDataCell = `
                        <div style="text-align: left;">
                            <b>主約資料:</b>
                            <pre style="white-space: pre-wrap; word-break: break-all; font-size: 10px; margin-top: 0;">${UtilsModule.escapeHtml(JSON.stringify(item._raw.master, null, 2))}</pre>
                            <b>通路資料:</b>
                            <pre style="white-space: pre-wrap; word-break: break-all; font-size: 10px; margin-top: 0;">${UtilsModule.escapeHtml(JSON.stringify(item._raw.channels, null, 2))}</pre>
                        </div>`;
                    ...
                    return `<tr>${[
                        ...
                        `<td>${polplnValue}</td>`,
                        `<td style="text-align: left; white-space: nowrap;">${channelsCellHtml}</td>`,
                        `<td>${rawDataCell}</td>`
                    ].join('')}</tr>`;
                }).join('');
            ...
            document.getElementById('pct-copy-all').addEventListener('click', () => {
                ...
                const rows = dataToCopy.map(item => {
                    const channelText = item.channels.length > 0 
                        ? item.channels.map(ch => ch.channel).join(' ')
                        : '無資料';
                    return [
