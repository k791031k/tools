javascript:(function() {
    ...
    // 模組一：應用程式狀態與設定
    // ===================================================================================
    const config = { TOOL_ID: 'full-api-collector', DEBOUNCE_DELAY: 300, ITEMS_PER_PAGE: 10 };
    const state = { isRunning: false, logs: [], ui: {}, originals: { XMLHttpRequest: window.XMLHttpRequest, fetch: window.fetch.bind(window) }, debounceTimer: null, currentPage: 1, isCollapsed: false, isMinimized: false };
    ...
    const UIManager = {
        ...
        renderPagination: function(totalPages) {
            ...
            let startPage = Math.max(1, state.currentPage - 2), endPage = Math.min(totalPages, state.currentPage + 2);
    ...
    const EventHandlers = {
        ...
        toggleCollecting: function() {
            state.isRunning ? EventHandlers.onStop() : EventHandlers.onStart();
        },
        ...
        onResend: function() {
            ...
            if (method === 'GET') {
                ...
                formEditor.querySelectorAll('.fac-kv-row').forEach(r => {
                    ...
                    const v = r.querySelector('.fac-kv-value').value;
                    if (k) p.append(k, v);
                });
                const q = p.toString();
                url = q ? `${b}?${q}` : b;
            ...
            .then(res => {
                ...
                return res.text().then(b => ({res: res, body: b, rawBody: b}));
            })
            .then(({res, body, rawBody}) => {
        ...
        onParseUrl: function() {
            ...
            const a = f.querySelector('.fac-kv-add');
            f.innerHTML = '';
            kv.forEach(p => {
                const n = document.createElement('div');
                n.className = 'fac-kv-row';
        ...
        onCopyResponse: function(data) {
            ...
            navigator.clipboard.writeText(data).then(
                () => UIManager.updateEditorStatus('回應內容已複製', false),
                () => UIManager.updateEditorStatus('複製失敗', 'error')
            );
        },
        onExport: function(data) {
            ...
            if (f.toLowerCase() === 'txt') {
                ...
                try {
                    ...
                    if (j.length === 0) {
                        EventHandlers.triggerDownload(new Blob([''], {type: 'text/csv;charset=utf-8'}), `${n}.csv`);
                        return;
                    }
