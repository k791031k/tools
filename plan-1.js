javascript:(function() {
    ...
    /**
     * =================================================================================
     * KGI Plan Code Query Tool v30.0.0 - Final Build
     *
     * @version 2025.07.26 - Final Build
     * @author Optimized Version
     *
     * 更新內容 (v30.0.0):
     * - [主要更新] 重製「全表預覽」功能：在新分頁中，以「主次詳細資料列」方式呈現，包含所有原始 API 回傳欄位。
     * - [主要更新] 重製「銷售通路」欄位：儲存格內固定顯示五通路並依狀態排序上色；滑鼠提示顯示完整詳細資訊；無通路資料時顯示【無資料】。
     * - [主要更新] 修正標題列固定問題：整合用戶最新 CSS 並確保 Sticky Header 正常運作。
     * - 更新：全面替換為使用者提供的最新 CSS 樣式表，調整欄位寬度與排版。
     * - 新增：在「主約銷售時間」查詢條件中，增加「全部」選項並實現其功能。
     * - 修正：確保「簡稱/全名」(shortName/planCodeName)切換功能與提示正常。
     * - (及先前版本所有修改)
     * =================================================================================
     */
    ...
    const DataModule = (() => {
        ...
        const mergeData = () => {
            ...
            const mergedData = masterDataCache.map((rawMasterItem) => {
                ...
                const processedItem = {
                    planCode,
                    fullName: rawMasterItem.planName || rawMasterItem.shortName || '-',
                    displayName: rawMasterItem.shortName || rawMasterItem.planName || '-',
        ...
        const getFilteredData = () => {
            ...
            if (searchKeyword) {
                ...
                data = data.filter(item => {
                    ...
                    delete searchableItem._raw; // Don't search in raw data
        ...
        const setupResultsDialog = (modal) => {
            ...
            document.getElementById('pct-preview-all').addEventListener('click', () => {
                ...
                dataToPreview.forEach(item => {
                    ...
                    const mainStatusText = ConfigModule.MASTER_STATUS_TEXT[item.mainStatus] || item.mainStatus;
                    const channelText = item.channels.length > 0 ? item.channels.map(ch => ch.channel).join(' ') : '[無]';

                    // Main Row
                    tableRows += `<tr>${[
                        `<td>${item.no}</td>`,
                        `<td>${item.planCode}</td>`,
                        `<td style="text-align: left;">${nameToShow}</td>`,
                        `<td>${item.currency}</td>`,
                        `<td>${item.unit}</td>`,
                        `<td>${item.coverageType}</td>`,
                        `<td>${item.saleStartDate}</td>`,
                        `<td>${item.saleEndDate}</td>`,
                        `<td>${mainStatusText}</td>`,
                        `<td>${polplnValue}</td>`,
                        `<td style="text-align: left;">${channelText}</td>`
                    ].join('')}</tr>`;

                    // Detail Rows
                    const rawMasterHtml = `<pre style="white-space: pre-wrap; word-break: break-all; font-size: 11px; margin: 0; text-align: left;">${UtilsModule.escapeHtml(JSON.stringify(item._raw.master, null, 2))}</pre>`;
                    const rawChannelsHtml = `<pre style="white-space: pre-wrap; word-break: break-all; font-size: 11px; margin: 0; text-align: left;">${UtilsModule.escapeHtml(JSON.stringify(item._raw.channels, null, 2))}</pre>`;
                    
                    tableRows += `
                        <tr class="detail-row">
                            <td colspan="11" style="padding: 10px 15px; background: #fafafa; text-align: left;">
                ...
                const previewHtml = `
                    ...
                    <head>
                        ...
                        <style>
                            ...
                            table { border-collapse: collapse; width: 100%; font-size: 13px; border: 1px solid #ccc; }
                            th, td { border: 1px solid #ccc; padding: 8px; text-align: center; vertical-align: top; }
                            th { background-color: #f2f2f2; }
                            tr:not(.detail-row):nth-of-type(odd) { background-color: #f9f9f9; }
                            .detail-row td { border-top: 2px solid #aaa; }
                        </style>
                    </head>
                    <body>
