javascript:(function() {
    ...
    /**
     * =================================================================================
     * KGI Plan Code Query Tool v29.6.0 - Final Build
     *
     * @version 2025.07.26 - Final Build
     * @author Optimized Version
     *
     * 更新內容 (v29.6.0):
     * - 新增：「全表預覽」功能，可在新分頁開啟完整結果表格。
     * - 新增：在「主約銷售時間」查詢條件中，增加「全部」選項並實現其功能。
     * - 調整：「銷售通路」欄位內容改為永遠顯示五通路，並依狀態排序上色；滑鼠提示則顯示完整詳細資訊。
     * - 調整：修改「銷售狀態」與「POLPLN」欄位寬度。
     * - 調整：將「商品名稱」與「銷售通路」的資料內容改回置左對齊。
     * - 修正：確保結果表格的標題列絕對固定。
     * - (及先前版本所有修改)
     * =================================================================================
     */
    ...
    const UIModule = (() => {
        const injectStyle = () => {
            ...
            style.textContent = `
              ...
              .pct-table th:nth-child(9) { width: 100px; }
              .pct-table th:nth-child(10) { width: 80px; }
              .pct-table th:nth-child(11) { width: 180px; }
    ...
    const ControllerModule = (() => {
        ...
        const setupResultsDialog = (modal) => {
            ...
            document.getElementById('pct-preview-all').addEventListener('click', () => {
                ...
                const tableRows = dataToPreview.map(item => {
                    ...
                    let channelsCellHtml = fixedChannels.map(channelCode => {
        ...
        const renderTableRow = (item, polplnDataCache) => {
            ...
            const channelDisplayData = fixedChannels.map(channelCode => {
                ...
                return {
                    channel: channelCode,
                    status: data ? data.status : 'not sold',
                };
            });
            ...
            const channelsCellContent = channelDisplayData.map(ch => {
                let className = '';
