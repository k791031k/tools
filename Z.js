javascript:(function() {
    ...
    /**
     * =================================================================================
     * KGI Plan Code Query Tool v28.1.0 - Personalized UI/UX Edition
     *
     * @version 2025.07.23 - Personalized Naming, Sorting & Interaction Update
     * @author Optimized Version
     *
     * 更新內容 (基於使用者儲存的個人化設定):
     * - 狀態名稱更新: '現售' -> '銷售中', '尚未開賣' -> '即將上市', '停售' -> '已停售'。
     * - 按鈕文字更新: '載入' 險種英文按鈕 -> '更多'。
     * - 通路排序邏輯: 依 '銷售中' > '已停售' > '日期異常' > '即將上市' 順序排列通路。
     * - 懸停提示優化: 商品名稱懸停顯示全名；通路懸停顯示其銷售狀態。
     * - 分隔符號: 維持使用 '|' 區隔不同狀態的通路群組。
     * =================================================================================
     */
    ...
    const ConfigModule = Object.freeze({
        ...
        VERSION: '28.1.0-Personalized-UI-UX',
        QUERY_MODES: {
            PLAN_CODE: 'planCode',
    ...
    const ControllerModule = (() => {
        ...
        const handleStartQuery = async () => {
            ...
            if (!StateModule.get().isTokenVerified) {
                document.getElementById('pct-table-body').innerHTML = `<tr><td colspan="11" style="text-align:center; padding: 20px; color: var(--error-color);">無有效權限，請設定正確的 Token。</td></tr>`;
            ...
            const html = `
              ...
              <div class="pct-modal-footer">
                  ...
                  <div class="pct-modal-footer-right" style="gap: 15px;">
                      <button id="pct-copy-all" class="pct-btn">全表複製</button>
                      <button id="pct-back-to-query" class="pct-btn">返回查詢</button>
                  </div>
              </div>
            `;
