javascript:(function() {
    /**
     * HAR API 工具 v5.1 - 書籤啟動器
     * 說明：
     * 點擊此書籤，即可在當前頁面動態載入並顯示功能完整的 API 工具。
     * v5.1 版修正了重大的版面破圖與 fetch 指令解析錯誤。
     */
    ...
    const Tool = {
        ...
        getStyles: function() {
            return `
                #har-api-tool-container-v5, #har-api-tool-container-v5 * {
                    margin: 0; padding: 0; border: 0; font-size: 100%; font: inherit; vertical-align: baseline; box-sizing: border-box; text-align: left; color: #1f2937;
                }
                #har-api-tool-container-v5 {
                    position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.5); z-index: 2147483647; display: flex; align-items: center; justify-content: center; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;
                }
                #har-api-tool-container-v5 button, #har-api-tool-container-v5 input, #har-api-tool-container-v5 select, #har-api-tool-container-v5 textarea { font-family: inherit; font-size: inherit; }
                #har-api-tool-container-v5 button { background-color: transparent; }
        ...
        run: function(container) {
            const App = {
                ...
                bindEvents: function() {
                    ...
                    this.elements.step1.fileInput.addEventListener('change', (e) => { const file = e.target.files[0]; if (file) { const reader = new FileReader(); reader.onload = (event) => { this.elements.step1.harInput.value = event.target.result; }; reader.readAsText(file); } });
                    ...
                    if (rawContent.startsWith('fetch(')) {
                        ...
                        if (optionsMatch) {
                            ...
                            try {
                                ...
                                body = options.body || '';
                            } catch (e) { throw new Error("無法解析 fetch options 物件。請確保其為有效的 JavaScript 物件格式。"); }
                        }
