javascript:(function() {
    ...
    /**
     * =================================================================================
     * KGI Plan Code Query Tool v29.5.5 - Final Build
     *
     * @version 2025.07.26 - Final Build
     * @author Optimized Version
     *
     * 更新內容 (v29.5.5):
     * - 調整：將 POLPLN 按鈕文字恢復為 'more'。
     * - 調整：統一所有介面中的銷售狀態名詞 (例如，統一使用「銷售中」、「已停售」)。
     * - 中文化：將所有介面中的英文靜態文字調整為繁體中文。
     * - 修正 (Bug Fix): Token 輸入框的內容不會再被自動轉換為大寫。
     * - 調整：將所有視窗標題統一為「商品查詢 (環境)」。
     * - 調整：移除部分欄位的向左對齊，使表格所有內容皆置中顯示。
     * - 修正：查詢結果表格的標題列在捲動時無法固定的問題。
     * - 調整：結果頁中的 [取得Name] 按鈕預設為禁用狀態。
     * =================================================================================
     */
    ...
    const ControllerModule = (() => {
                ...
                const updateDynamicOptions = (mode) => {
                    ...
                    switch (mode) {
                        ...
                        case ConfigModule.QUERY_MODES.PLAN_NAME:
                            ...
                            break;
                        case ConfigModule.QUERY_MODES.MASTER_CLASSIFIED:
                            content = `<label>主約銷售狀態：</label><div class="pct-sub-option-grid master-status">${Object.values(ConfigModule.MASTER_STATUS_TYPES).map(s => `<div class="pct-sub-option" data-status="${s}">${ConfigModule.MASTER_STATUS_TEXT[s] || s}</div>`).join('')}</div>`;
                            break;
                        case ConfigModule.QUERY_MODES.CHANNEL_CLASSIFIED:
                            content = `
                                <label>選擇通路：</label>
        ...
        const renderTableRow = (item, polplnDataCache) => {
            ...
            sortedChannels.forEach((ch, index) => {
                ...
                const title = `狀態: ${statusText} | 起日: ${ch.saleStartDate || '無'} | 迄日: ${ch.saleEndDate || '無'}`;
            ...
            return `
              <tr>
                  ...
                  <td class="clickable-cell" title="${UtilsModule.escapeHtml(nameInTitle)}">${UtilsModule.escapeHtml(nameToShow)}</td>
