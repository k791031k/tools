javascript: void((() => {
     ...
     * @description
     * 整合所有使用者回饋的最終版本，包含：
     * 1. [核心] 新增「獨佔模式」，啟動時自動清理舊版工具。
     * 2. [核心] 優化啟動流程，無TOKEN也能開啟介面，需要時才提示。
     * 3. [UI/UX] 重構「手動派件」頁籤，改為三按鈕動態切換內容，流程更清晰。
     * 4. [UI/UX] 派件結果改為獨立的彈出小視窗通知，不干擾主介面操作。
     * 5. [UI/UX] 重構查詢視窗，合併所有欄位至單一介面，採用分區網格佈局，並預設日期。
     * 6. [UI/UX] 齒輪選單簡化，只保留一個核心的「開啟新查詢」功能。
     * 7. [視覺] 強化整體色彩對比度，表格加入斑馬紋與懸停高亮，Toast通知更醒目。
     * 8. [修正] 解決頂部頁籤被遮擋、按鈕失靈、視窗位置/大小不一致等問題。
     * 9. [修正] 為篩選器加入`(空白)`值選項，並移除重複的篩選欄位。
     * =================================================================================
     */
    'use strict';
    ...
    const AppConfig = (() => {
        const staticConfig = {
            VERSION: '0811',
            TOOL_CONTAINER_ID: 'dispatch-tool-container-v19',
            STYLE_ELEMENT_ID: 'dispatch-tool-style-v19',
            TOKEN_KEY: 'euisToken',
            BATCH_PAGE_SIZE: 50,
    ...
    const createUIManager = (appConfig, appState, utils, domHelper) => {
        function injectStyle() {
            ...
            const style = domHelper.create('style', {
                ...
                textContent: `
                    :root {
                        --primary-50: #eef2ff; --primary-100: #e0e7ff; --primary-500: #4f46e5; --primary-600: #4338ca; --primary-700: #3730a3;
                    ...
                    .tab-content-wrapper { flex-grow: 1; overflow-y: auto; padding: 16px; }
                    ...
                    .case-table tbody tr:nth-child(even) { background-color: var(--gray-50); }
    ...
    const createUIComponents = (appConfig, uiManager, utils, domHelper) => {
        ...
        const QueryBuilderDialog = {
            show: (opts = {}) => {
                ...
                return uiManager.Modal.show({
                    header: '案件進階查詢', width: '900px', body,
                    footer: domHelper.create('div', {
        ...
        const AssignmentResultDialog = {
            show: ({ successful, failed, assignee }) => {
                ...
                // Create a new, separate modal for the result
    ...
    const createAppRunner = (appConfig, appState, apiService, uiComponents, utils, uiManager, domHelper) => {
        ...
        const _generateFilterOptions = (caseList, mode) => {
            ...
            // Remove duplicates
            const uniqueKeys = [...keys].filter(key => key !== 'subStatus' || !config.batch_common.includes('subStatus'));
                ...
                function _createBody() {
                    ...
                    if (activeTabId === 'manual') {
                        ...
                        if (queryInfo) {
                            ...
                            queryDisplay = domHelper.create('div', { className: 'query-criteria-display', style: { backgroundColor: '#f8f9fa', border: '1px solid #dee2e6', borderRadius: '6px', padding: '10px', marginBottom: '15px', fontSize: '13px', lineHeight: '1.6', wordBreak: 'break-all' }, innerHTML: `查詢條件： ${criteriaHtml}` });
                        }
                        const createFilterControls = () => {
                            const config = appConfig.FILTER_CONFIG;
                            const type = viewConfig.type;
                            const commonKeys = (type === 'personal') ? config.personal_common : config.personal_advanced;
                            const advancedKeys = (type === 'personal') ? config.personal_advanced : config.batch_advanced;
                            const allKeys = [...new Set([...commonKeys, ...advancedKeys])];
                            ...
                            return [
                                domHelper.create('div', { className: 'filter-controls', children: allKeys.map(renderSelect).filter(Boolean) }),
                            ];
                        };
                        const listViewElements = [
                            ...(queryDisplay ? [queryDisplay] : []), ...createFilterControls(),
                            domHelper.create('div', {
                                className: 'controls-row', style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px', padding: '0 4px' },
                                children: [
        ...
        async function handlePersonnelSelection() {
            // This dialog is now part of the manual tab, so this flow is simplified
            // We can repurpose this if a similar dialog is needed elsewhere, or remove it.
            // For now, let's assume the action comes directly from the manual tab.
            await handleMainView();
        }
