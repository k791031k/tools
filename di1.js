javascript: (() => {
    /**
     * =================================================================================
     * 多功能派件整合 - v15.24 (Stable & Refactored)
     * =================================================================================
     * @version     15.24.0
     * @description
     * [最終修正] 恢復了 UIComponents 模組的完整原始碼，並對所有模組的依賴注入變數
     * 進行了全局命名統一（全部採用小寫駝峰式），徹底解決因命名不一致及程式碼
     * 損壞導致的互動功能完全失效問題。此版本為穩定可執行版。
     * =================================================================================
     */
    'use strict';
    ...
    const AppConfig = (() => {
        const staticConfig = {
            VERSION: '15.24 (Stable & Refactored)',
        ...
        const CaseListView = {
            show: async (opts) => {
                ...
                function _createHeader() {
                    return `多功能派件整合 v${appConfig.VERSION} (${appConfig.ENV})<button class="header-config-btn" id="config-btn" title="設定">⚙️</button><div class="config-menu" id="config-menu"><button id="change-token-btn">重新驗核token</button><button id="open-new-query-btn">開啟新查詢</button><button id="clear-cache-btn">清除暫存檔</button></div>`;
                }
                ...
                function _createTabbedPersonnelSelector(defaultUsers) {
                    ...
                    const createRadioList = (users) => {
                        ...
                        users.forEach(user => {
                            const li = domHelper.create('li', {
                                className: appConfig.SPECIAL_ASSIGNEES.includes(user) ? 'special-assignee' : '',
                                children: [
                                    domHelper.create('label', {
                                        children: [
                                            domHelper.create('input', {
                    ...
                    const importTabContent = domHelper.create('div', {
                        children: [domHelper.create('button', {
                            id: `import-btn-${selectorId}`,
                            className: 'dispatch-btn dispatch-outline small',
                            textContent: '從 .txt 匯入'
                        }), domHelper.create('div', {
                            id: `import-list-${selectorId}`,
                            style: {
                                marginTop: '10px'
                            }
                        })]
                    });
                    const manualTabContent = domHelper.create('input', {
                        id: `manual-input-${selectorId}`,
                        type: 'text',
                        className: 'dispatch-input',
                        attributes: {
                            placeholder: '輸入完整 AD 帳號'
                        }
                    });
                    const tabs = [{
                        id: 'default',
                    ...
                    importTabContent.querySelector(`#import-btn-${selectorId}`).onclick = async () => {
                        try {
                            ...
                            if (names.length > 0) {
                                const importedUsers = [...new Set(names)].sort();
                    ...
                    return {
                        ...
                        getSelectedValue: () => {
                            ...
                            if (activeTabId === 'manual') return manualTabContent.value.trim();
                ...
                function _bindHeaderEvents() {
                    ...
                    elements.modal.querySelector('#change-token-btn').onclick = () => elements.resolve({
                        action: appConfig.MODAL_ACTIONS.CHANGE_TOKEN
                    });
                    ...
                    elements.modal.querySelector('#clear-cache-btn').onclick = () => elements.resolve({
                ...
                function _bindManualOpEvents() {
                    ...
                    const personnelSelector = container.querySelector('.personnel-selector')._logic; // Assume logic is attached
                    dispatchBtn.onclick = () => {
                        const cases = utils.splitTextInput(casesInput.value);
                        if (cases.length === 0) return uiManager.Toast.show('請輸入至少一個受理號碼', 'error');
                        const assignee = personnelSelector.getSelectedValue();
                        if (!assignee) return uiManager.Toast.show('請選擇或輸入指派人員', 'error');
                        elements.resolve({
                ...
                return uiManager.Modal.show({
                    ...
                    onOpen: (modal, resolve) => {
                        ...
                        const personnelSelectorElement = modal.querySelector('.personnel-selector');
                        if (personnelSelectorElement) {
                            personnelSelectorElement._logic = {
                                getSelectedValue: () => {
                                    const activeTabId = personnelSelectorElement.querySelector('.personnel-tabs .active')?.dataset.tab;
                                    if (activeTabId === 'manual') return personnelSelectorElement.querySelector('input[type="text"]').value.trim();
                                    const selectorId = personnelSelectorElement.querySelector('input[type="radio"]')?.name;
                                    const selectedRadio = selectorId ? personnelSelectorElement.querySelector(`input[name="${selectorId}"]:checked`) : null;
                                    return selectedRadio ? selectedRadio.value : null;
                                }
                            };
                        }
    ...
    const createAppRunner = (appConfig, appState, apiService, uiComponents, utils, uiManager) => {
        ...
        async function handleMainView(opts = {}) {
            ...
            switch (res?.action) {
                ...
                case appConfig.MODAL_ACTIONS.OPEN_NEW_QUERY:
                    await handleNewQuery(res.currentFilters);
                    break;
        ...
        async function handleNewQuery(currentFilters) {
            const res = await uiComponents.QueryBuilderDialog.show({
                dynamicBatchDefaults: _getBatchDefaultFilters(),
                currentFilters
            });
            if (res?.action === appConfig.MODAL_ACTIONS.APPLY_SESSION_FILTERS) {
                ...
                if (result.data) {
                    ...
                    await uiComponents.ErrorDialog.show({
                        error: result.error
                    });
                }
