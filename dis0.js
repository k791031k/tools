javascript: (() => {
  /**
   * =================================================================================
   * 多功能派件整合 - v15.15 (錯誤修復版)
   * =================================================================================
   * @version   15.15.0 (Refactored)
   * @description
   * 本版本主要修復使用者回報的問題。
   *
   * - [修復] 解決了點擊「調整當次載入清單」按鈕後沒有反應的 Bug。
   * =================================================================================
   */
  'use strict';
  ...
  try {
            ...
            show: () => {
                ...
                const createFormColumn = (mode, data) => {
                    ...
                    const keys = mode === 'personal' 
                        ? [...config.common, ...config.personal_advanced]
                        : [...config.common, ...config.batch_advanced];
            ...
            const _applyFiltersAndSort = Utils.debounce(() => {
                ...
                let filteredData = caseList.filter(item => 
                    Object.entries(filterValues).every(([key, value]) => {
                        ...
                        if (key === 'applyDateStart') {
                            return new Date(item.applyDate) >= new Date(value);
                        }
                        if (key === 'applyDateEnd') {
                            return new Date(item.applyDate) <= new Date(value);
                        }
                ...
                const createFilterControls = () => {
                    ...
                    const renderSelect = (key) => {
                        ...
                        if (initialFilters && initialFilters[key]) {
                            if (!options.includes(initialFilters[key])) {
                                select.appendChild(DOMHelper.create('option', { textContent: initialFilters[key], attributes: { value: initialFilters[key] } }));
                            }
    ...
    const AppRunner = (() => {
        ...
        const _generateFilterOptions = (caseList, mode) => {
            const options = {};
            const config = AppConfig.FILTER_CONFIG;
            const filterKeys = new Set(mode === 'personal' 
              ? [...config.common, ...config.personal_advanced]
              : [...config.common, ...config.batch_advanced]
            );
        ...
        async function handleMainView() {
            ...
            const filterOptions = result.data ? _generateFilterOptions(result.data, mode) : {};
