javascript: (function() {
    ...
    const DataHandlers = {
        ...
        parseQueryString: function(str) {
            ...
            try {
                ...
                const a = [];
                p.forEach((v, k) => a.push({ key: k, value: v }));
    ...
    const UIManager = {
        getTemplate: function() {
            return `
                ...
                <style>
                    .fac-wrap { position: fixed; top: 10px; right: 10px; width: 520px; background: #fdfdfe; color: #333; border-radius: 12px; box-shadow: 0 5px 20px rgba(0,0,0,0.25); font-family: sans-serif; z-index: 999999; display: flex; flex-direction: column; max-height: calc(100vh - 20px); overflow: hidden; border: 1px solid #c0c0c0; resize: both; }
                    ...
                    .fac-download-controls { display: flex; gap: 6px; flex-wrap: wrap; margin-top: 8px; width: 100%; }
                    ...
                    .fac-list-item-content { flex-grow: 1; }
                    ...
                    .fac-summary { font-size: 12px; color: #666; background: #f8f9fa; padding: 2px 6px; border-radius: 3px; margin: 2px 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
                    ...
                    /* 分頁樣式 */
                    #fac-tabs-header { display: flex; background: #e9ecef; border-bottom: 1px solid #c0c0c0; padding: 0 5px; flex-wrap: wrap; max-height: 50px; overflow-y: auto; }
                    ...
                    #fac-tabs-content { flex: 1; overflow-y: auto; padding: 12px; }
                    ...
                    #fac-response-content { flex: 1; display: flex; flex-direction: column; overflow: hidden; }
                    ...
                    #fac-response-filter { margin-top: 10px; display: flex; gap: 4px; align-items: center; }
        ...
        renderList: function() {
            ...
            filteredLogs = DataHandlers.sortLogs(filteredLogs, state.sortKey.split('_')[0], state.sortKey.split('_')[1]);
        ...
        renderPagination: function(totalPages) {
            const state = AppConfig.state;
            const { pagination } = state.uiElements;
        ...
        showHttpEditor: function(item, container) {
            ...
            const contentType = Object.entries(item.requestHeaders).find(([key, value]) => key.toLowerCase() === 'content-type')?.[1] || '';
                ...
                const addEvent = (type, data = null, reason = null) => {
                    ...
                    if (AppConfig.state.isRunning) {
                        ...
                        if (existingLogIndex >= 0) {
                            AppConfig.state.logs[existingLogIndex] = { ...wsLog
                            };
                        } else {
                            AppConfig.state.logs.unshift({ ...wsLog
                            });
                        }
                        UIManager.renderList();
                    }
    ...
    const EventHandlers = {
        ...
        onClose: function() {
            ApiHooking.unhook();
            const { wrap, minimizedIcon } = AppConfig.state.uiElements;
            if (wrap) wrap.remove();
        ...
        onResend: function(tabId) {
            ...
            const responsePre = container.querySelector('#fac-response-pre');
            const templateName = API_TEMPLATES[selectedTemplateKey]?.name || '未知格式';
