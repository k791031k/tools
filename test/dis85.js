javascript: (function() {
    ...
    /**
     * =================================================================================
     * 多功能派件整合 - v13.0 (Architectural Refactor)
     * =================================================================================
     * @version 13.0.0
     * @description
     * 最終架構極致化版本，根據專業建議進行了深度重構：
     * 1. [架構]: UIModules 模組被深層分割為多個獨立子物件，徹底實現單一職責原則。
     * 2. [健壯性]: 加入了嚴格的參數型別防禦性檢查，提升程式穩定性。
     * 3. [健壯性]: 新增了全域 isLoading 狀態旗標，防止非同步流程的重複觸發。
     * 4. [功能確認]: 再次驗證並註解了視窗拖移功能。
     * 5. [功能確認]: 再次驗證並註解了中止查詢功能。
     * =================================================================================
     */
    ...
    try {
        ...
        const AppConfig = Object.freeze({
            VERSION: '13.0.0 (Architectural Refactor)',
            TOOL_CONTAINER_ID: 'dispatch-tool-container-v13',
            STYLE_ELEMENT_ID: 'dispatch-tool-style-v13',
            TOKEN_KEY: 'euisToken',
        ...
        /**
         * @module UIModules
         * @description 包含所有 UI 元件的集合。
         * @architectural_note 根據 v13.0 架構重構，此模組內部被分割為多個獨立的子物件，
         * 每個子物件負責一個 UI 元件。外部呼叫方式為 UIModules.ComponentName.show()。
         */
        const UIModules = (() => {
                    ...
                    return UIManager.Modal.show({
                        header, width: '90vw', body: modalBody, footer: modalFooter, onOpen: (modal, resolve) => {
                            ...
                            const renderTable = (data) => {
                                const headerHtml = `<thead><tr><th style="width:50px;"><input type="checkbox" id="select-all-header"></th>${Object.entries(TABLE_HEADERS).map(([key, label]) => `<th data-key="${key}" title="點擊排序">${label} <span class="sort-indicator">${sortState.key === key ? (sortState.order === 'asc' ? '▲' : '▼') : ''}</span></th>`).join('')}</tr></thead>`;
                                tableElem.innerHTML = headerHtml + '<tbody></tbody>'; const tbody = tableElem.querySelector('tbody'), rowHeight = 35; virtualContent.style.height = `${data.length * rowHeight}px`;
                                let lastRenderedStart = -1; const renderViewport = (force = false) => { const scrollTop = tableContainer.scrollTop, startIndex = Math.max(0, Math.floor(scrollTop / rowHeight) - 5); if (startIndex === lastRenderedStart && !force) return; const visibleRows = Math.ceil(tableContainer.clientHeight / rowHeight) + 10, endIndex = Math.min(data.length, startIndex + visibleRows); let rowsHtml = ''; for (let i = startIndex; i < endIndex; i++) { const item = data[i]; rowsHtml += `<tr style="position:absolute; top:${i * rowHeight}px; left:0; right:0; height:${rowHeight}px;"><td><input type="checkbox" class="case-checkbox" value="${item.applyNumber}"></td>${Object.keys(TABLE_HEADERS).map(key => `<td title="${Utils.escapeHtml(item[key])}">${Utils.escapeHtml(key === 'seq' ? i + 1 : (key.includes('Date') ? Utils.formatDisplayDate(item[key]) : item[key] ?? ''))}</td>`).join('')}</tr>`; } tbody.innerHTML = rowsHtml; lastRenderedStart = startIndex; };
                                renderViewport(true); if (viewState.scrollTop) tableContainer.scrollTop = viewState.scrollTop; tableContainer.onscroll = () => renderViewport();
                            };
            ...
            const PersonnelSelectDialog = { show: (opts) => { const { selectedCount, mode, onBack } = opts; const users = mode === 'personal' ? AppConfig.DEFAULT_PERSONAL_USERS : AppConfig.DEFAULT_BATCH_USERS; const userOptions = users.map(u => `<option value="${u}" ${AppConfig.SPECIAL_USERS.includes(u) ? 'style="font-weight:bold;color:var(--primary-dark);"' : ''}>${u}</option>`).join(''); return UIManager.Modal.show({ header: '選擇指派人員', width: '400px', body: `<p style="margin-top:0;">已選取 <strong>${selectedCount}</strong> 筆案件，請選擇要指派的人員：</p><select id="assignee-select" class="dispatch-input">${userOptions}</select>`, footer: `<div style="width:100%; display:flex; justify-content:space-between; align-items:center;"><div><button id="back-btn" class="dispatch-btn dispatch-outline">返回上一步</button></div><div><button id="confirm-assignment-btn" class="dispatch-btn">確認指派</button></div></div>`, onOpen: (modal, resolve) => { modal.querySelector('#back-btn').onclick = onBack; modal.querySelector('#confirm-assignment-btn').onclick = () => resolve({ action: '_confirm_assignment_', assignee: modal.querySelector('#assignee-select').value }); } }); } };
        ...
        const AppMain = (() => {
            ...
            async function autoToken() {
                ...
                try {
                    ...
                    if (token) { AppState.set({ userToken: token }); UIManager.Toast.show('已自動載入 Token', 'success'); await Utils.sleep(400); modeSelector(); } else { await UIModules.TokenDialog.show(true); }
        ...
        // Utils might not be defined if error is very early, so create a fallback escapeHtml
