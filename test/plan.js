javascript: (function() {
    ...
    /**
     * =================================================================================
     * KGI Plan Code Query Tool v1.9.0 (Refactored & Modified for Direct Query)
     * =================================================================================
     * 程式碼已重構為多個模組，以提高可讀性與可維護性，並保持原有功能不變。
     */
...
`;
    ...
    const ControllerModule = (() => {
        ...
        const initialize = () => {
            ...
            autoCheckTokenAndLoadData();
        };

        /**
         * 自動檢查並載入 Token，然後開始載入資料。
         */
        const autoCheckTokenAndLoadData = async () => {
        ...
        const loadDataAndShowMainUI = async () => {
            try {
                ...
                if (err.name !== 'AbortError') {
                    UIModule.Toast.show('資料載入失敗: ' + err.message, 'error');
                }
            }
        };
        ...
        const setupTokenDialogListeners = () => {
            ...
            const handleConfirm = () => {
                ...
                setTimeout(loadDataAndShowMainUI, 1000);
            };
            ...
            const queryControlsHTML = `
                <div class="pct-query-controls">
                    ...
                    <div class="pct-query-row">
                        ...
                        <div class="pct-query-buttons">
                            <button id="pct-search-button" class="pct-btn">查詢</button>
                            <button id="pct-new-query-button" class="pct-btn" style="background-color: #FFA500;">新查詢</button>
                        </div>
                    </div>
                </div>
            `;
        ...
        const setupResultsDialogListeners = (modal) => {
            ...
            const statusCheckboxes = modal.querySelectorAll('.pct-status-checkbox');
            const channelCheckboxes = modal.querySelectorAll('.pct-channel-checkbox');
            const viewModeRadios = modal.querySelectorAll('input[name="pct-view-mode"]');
            const searchBtn = document.getElementById('pct-search-button');
            const newQueryBtn = document.getElementById('pct-new-query-button');
            ...
            searchBtn?.addEventListener('click', () => {
                ...
                channelCheckboxes.forEach(cb => {
                    if (cb.checked) selectedChannels.add(cb.value);
                });
                const viewMode = [...viewModeRadios].find(r => r.checked)?.value || ConfigModule.QUERY_MODES.MASTER_CLASSIFIED;
            ...
            document.getElementById('pct-back-to-query')?.addEventListener('click', () => showTokenDialog());
        };
        ...
        const renderTableRow = (item) => {
            ...
            if (queryMode === ConfigModule.QUERY_MODES.CHANNEL_CLASSIFIED) {
                const channelCells = ConfigModule.FIELD_MAPS.CHANNELS.map(channelName => {
                    ...
                    return `<td ${bgColor} title="${title}">${renderStatusPill(statusInfo.status)}</td>`;
                }).join('');
        ...
        const handleForceUpdate = async () => {
            ...
            if (document.getElementById('pct-table-view-wrapper')) {
                ...
                UIModule.Toast.show('資料已清除，請重新查詢', 'info', 4000);
            } else {
                const controller = new AbortController();
                StateModule.set({
                    currentQueryController: controller
        ...
        const handleCopyAll = () => {
            ...
            const rows = dataToCopy.map(item => [
                ...
                item.channels.length > 0 ? item.channels.map(ch => ch.channel).join(' | ') : '無資料'
            ]);
        ...
        const generatePreviewPageHTML = (data) => {
            ...
            const processedData = data.map(item => {
                const channelStr = item.channels.length > 0 ?
                    item.channels.map(c => `${c.channel}(${ConfigModule.MASTER_STATUS_TEXT[c.status]||c.status})`).join(', ') :
                    '無通路';
