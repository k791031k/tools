javascript:(function() {
    ...
    /**
     * =================================================================================
     * KGI Plan Code Query Tool v29.7.0 - Final Build
     *
     ...
     * - 增強：「全表預覽」功能現在會額外附加一欄，顯示每筆資料的完整原始對應欄位 (JSON)。
     * - 調整：「銷售通路」欄位邏輯更新，無任何通路資料時明確顯示【無資料】。
     * - (及先前版本所有修改)
     * =================================================================================
     */
    ...
    const UIModule = (() => {
        const injectStyle = () => {
            ...
            // [v29.7.0] 更新：全面替換為使用者提供的最新 CSS 樣式
            style.textContent = `
              :root { 
              ...
              .pct-modal[data-size="results"] .pct-modal-body {
                  overflow-y: hidden;
              }
              .pct-modal-footer { 
    ...
    const ControllerModule = (() => {
        ...
        const setupResultsDialog = (modal) => {
            ...
            document.getElementById('pct-preview-all').addEventListener('click', () => {
                ...
                const headers = [...Array.from(modal.querySelectorAll('.pct-table th')).map(th => th.textContent), '原始對應資料'];
                ...
                const tableRows = dataToPreview.map(item => {
                    ...
                    delete itemForPreview.no; // 移除內部使用的流水號

                    const rawDataCell = `<pre style="white-space: pre-wrap; word-break: break-all; font-size: 10px; text-align: left;">${UtilsModule.escapeHtml(JSON.stringify(itemForPreview, null, 2))}</pre>`;
