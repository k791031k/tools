javascript:(function() {
    ...
    /**
     * =================================================================================
     * KGI Plan Code Query Tool v25.0.0
     *
     * @version 2025.07.22 - Ultimate Pro Edition
     * @author Gemini
     *
     * 更新說明 (v25.0.0):
     * - **【重大修復】排序與釘選**:
     ...
     * - 確保表格標題在滾動時能正確地釘選在頂部。
     * - **【重大優化】查詢邏輯**:
     * - 「商品代號」查詢模式從精確比對改為模糊查詢 (包含)。
     * - 結果頁的搜尋框確認支援模糊查詢。
     * - **【介面與體驗優化】**:
     * - 查詢結果筆數移至搜尋框右側，並加大字體。
     * - 表格標題字體加大並確保置中。
     * - 銷售通路顯示邏輯優化：按「現售 > 停售」排序，並加入分隔號。
     * - 確保「No」欄位在任何排序下都從 1 開始重新編號。
     * - **保留所有優點**: 沿用 v24.1.0 的所有優化，包括 Token 驗證、手動載入 POLPLN 等。
     * =================================================================================
     */
    ...
    const ControllerModule = (() => {
        ...
        const renderChannelsCell = (channels) => {
            ...
            const { IN_SALE, STOPPED } = ConfigModule.MASTER_STATUS_TYPES;
            const statusOrder = { [IN_SALE]: 1, [STOPPED]: 2 };

            const grouped = channels.reduce((acc, channel) => {
                const status = channel.status;
                if (!acc[status]) acc[status] = [];
                acc[status].push(channel.channel);
                return acc;
            }, {});
            ...
            return sortedGroups.map(([status, channels]) => {
