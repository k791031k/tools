javascript: void((() => {
    /**
     * =================================================================================
     * 案件清單作業 - v0825.4.interactive (互動優化版)
     * =================================================================================
     * @version      0825.4.interactive
     * @description
     * 1. [體驗] 允許在未設定 Token 的情況下開啟和操作介面，提升流暢度。
     * 2. [體驗] 對所有需要 Token 的操作增加前置守衛，若未設定 Token 則彈出提示，引導使用者操作。
     * 3. [結構] 繼承 v0825.3.refactored 的所有結構性優化。
     * =================================================================================
     */
    'use strict';
    ...
    const createAppRunner = (appConfig, appState, apiService, uiComponents, utils, uiManager, domHelper) => {
        ...
        const TableRenderer = {
            ...
            createDataRow: (item, viewConfig, index) => {
                ...
                viewConfig.columns.forEach(key => {
                    ...
                    if (key === 'select') {
                        // Prevent copy action on checkbox cell
                        td.dataset.action = '';
        ...
        async function handleMainView(opts = {}) {
            ...
            switch (res?.action) {
                ...
                case appConfig.MODAL_ACTIONS.RESET_WINDOW: uiManager.Modal.resetPosition(); await handleMainView(); break;
                default: break;
            }
        }
        async function handleTokenChange() {
            ...
            if (res?.action === appConfig.MODAL_ACTIONS.CONFIRM) {
                ...
                await handleMainView({ forceFetch: true });
                 ...
                 await handleMainView(); // Re-render without changing token
            }
        }
