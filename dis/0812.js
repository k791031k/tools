javascript: void((() => {
    /**
     * =================================================================================
     * 案件清單作業 - v0825.3.refactored (重構優化版)
     * =================================================================================
     * @version      0825.3.refactored
     * @description
     * 1. [結構] 表格渲染 (TableRenderer) 改為使用 DOM Helper 建立元素，提高可維護性。
     * 2. [結構] 主要事件處理改為事件委派模式，簡化事件綁定邏輯，提升效能。
     * 3. [結構] 新增 API 請求輔助函式 (executeApiCall)，統一處理載入、錯誤及重試流程。
     * 4. [結構] 統一 App 狀態管理，將所有頁籤資料整合至單一 state.tabs 結構中，簡化渲染邏輯。
     * 5. [穩定性] 繼承 v0825.2.final 的所有穩定性修復。
     * =================================================================================
     */
    'use strict';
    ...
    const AppConfig = (() => {
        const staticConfig = {
            VERSION: '0825.3.refactored',
            TOOL_CONTAINER_ID: 'dispatch-tool-container-v26',
            STYLE_ELEMENT_ID: 'dispatch-tool-style-v26',
            TOKEN_KEY: 'euisToken',
            BATCH_PAGE_SIZE: 50,
        ...
        const Progress = {
            show(text) {
                ...
                const stopButton = domHelper.create('button', {
                    className: 'dispatch-btn dispatch-outline', textContent: '停止查詢',
    ...
    const createAppRunner = (appConfig, appState, apiService, uiComponents, utils, uiManager, domHelper) => {
            ...
            createHeaderRow: (viewConfig) => {
                ...
                viewConfig.columns.forEach(key => {
                    ...
                    const th = domHelper.create('th', {
                        ...
                        attributes: { 'data-key': def.key, 'data-type': def.type || '', title: '點擊排序' }
                    });
        ...
        async function executeApiCall(apiFunction, ...args) {
            ...
            return null; // Return null if failed and user didn't retry
        }
        ...
        async function handleMainView(opts = {}) {
            ...
            if (!appState.get('userToken') && activeTabId !== 'manual') {
                 ...
                 activeTab.error = new Error('SSO-TOKEN 未設定。請點擊上方 🔑 圖示設定。');
                 activeTab.data = [];
            ...
            const caseListViewShow = (tab) => {
                ...
                function _renderTable(data) {
                    ...
                    elements.tbody.innerHTML = ''; // Clear existing rows
                    if (isErrorState) {
                ...
                function _createBody() {
                    ...
                    const tabButtons = allTabs.map(t => domHelper.create('button', {
                        className: t.id === activeTabId ? 'active' : '',
                        attributes: { 'data-tab-id': t.id, 'data-action': 'switch_tab' },
                        textContent: t.name,
                            ...
                            domHelper.create('div', {
                                ...
                                children: [
                                    ...
                                    domHelper.create('div', { className: 'right-controls', style: { display: 'flex', gap: '8px', alignItems: 'center' }, children: [
                                        ...
                                        domHelper.create('button', { 'data-action': 'reload_view', className: 'dispatch-btn small', textContent: '重新載入', style: { display: isErrorState ? 'inline-flex' : 'none' } })
                                    ]})
                                ]
                            }),
                ...
                async function handleAction(e, resolve) {
                    ...
                    // Stop propagation for actions that shouldn't bubble, e.g., closing a tab
                    if (action === 'close_query_tab') e.stopPropagation();

                    // --- TABLE ACTIONS ---
                    if (actionTarget.matches('th[data-key]')) {
                    ...
                    // --- ACTIONS THAT RESOLVE/TRIGGER STATE CHANGE ---
                    const resolveActions = [
                        'switch_tab', 'close_query_tab', 'change_token', 'open_new_query', 
                        'clear_cache', 'reset_window', 'reload_view', 'next_step', 'manual_dispatch'
                    ];
                    
                    if (resolveActions.includes(action)) {
                         let payload = {};
                         if (action === 'switch_tab' || action === 'close_query_tab') {
                            payload.tabId = actionTarget.dataset.tabId;
                         } else if (action === 'next_step') {
                            payload.selectedCases = Array.from(elements.modal.querySelectorAll('.case-checkbox:checked')).map(cb => cb.value);
                            if (payload.selectedCases.length === 0) return;
                         } else if (action === 'manual_dispatch') {
                             ...
                             payload = { cases, assignee };
                         }
                         resolve({ action, ...payload });
                    }
                     
                    // --- ACTIONS REQUIRING SPECIAL HANDLING ---
                     ...
                     if(action === 'switch_personnel_pane') {
                        ...
                        if (paneId === 'import') {
                            try {
                                ...
                                if (names.length > 0) {
                                    ...
                                    importPane.appendChild(createPersonnelList(names));
                                    
                                    const defaultPane = container.querySelector('[data-pane="default"]');
                                    defaultPane.innerHTML = '';
                                    defaultPane.appendChild(createPersonnelList(state.assigneeList));
                ...
                return uiManager.Modal.show({
                    ...
                    onOpen: (modal, resolve) => {
                        ...
                        modal.addEventListener('click', (e) => handleAction(e, resolve));
            ...
            switch (res?.action) {
                ...
                case appConfig.MODAL_ACTIONS.CLOSE_QUERY_TAB: delete state.tabs[res.tabId]; await handleMainView({ targetTabId: 'batch' }); break;
        ...
        async function handleNewQuery() {
            ...
            if (res?.action === appConfig.MODAL_ACTIONS.APPLY_SESSION_FILTERS) {
                ...
                state.tabs[newTabId] = {
                    ...
                    data: null,
                    status: 'idle',
                    viewConfig: {...appConfig.SHARED_VIEW_CONFIG, type: 'query'},
                    error: null,
        ...
        async function handleAssignment(assignee, cases) {
            ...
            if (casesToDispatch.length === 0) {
                uiManager.Toast.show('沒有選擇任何案件', 'warning'); return;
            }
            const result = await executeApiCall(() => apiService.manualAssign(casesToDispatch, assignee));
            
            if(result) {
                await uiComponents.AssignmentResultDialog.show({ ...result, assignee });
