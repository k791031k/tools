javascript: void((() => {
    /**
     * =================================================================================
     * 案件清單作業 - v1.0.1-hotfix (緊急修正版)
     * =================================================================================
     * @version      1.0.1-hotfix
     * @description
     * 1. [修正] 修復 v1.0.0 版本中 createAppRunner 模組遺失 run 函式定義導致的致命錯誤。
     * 2. [繼承] 包含先前所有 P0, P1, P2 階段的優化與重構。
     * =================================================================================
     */
    'use strict';
    ...
    const AppConfig = (() => {
        const staticConfig = {
            VERSION: '1.0.1-hotfix',
            TOOL_CONTAINER_ID: 'dispatch-tool-container-v30',
            STYLE_ELEMENT_ID: 'dispatch-tool-style-v30',
            TOKEN_KEY: 'euisToken',
            BATCH_PAGE_SIZE: 50,
            ...
            SHARED_VIEW_CONFIG: {
                columns: [
                    ...
                    'mainStatus', 'subStatus', 'currency', 'overpay', 'currentOwner', 'channel',
    ...
    const createOptimizedState = () => {
        ...
        const notify = (changedKeys) => {
            changedKeys.forEach(key => subscribers.get(key)?.forEach(cb => cb(state)));
    ...
    const createUtils = (appConfig) => {
        ...
        return {
            ...
            jsonToCsv: (items, {
                ...
                const headers = headersOrder && headersOrder.length > 0 ?
                    headersOrder :
                    Object.keys(items[0] || {});
                const headerRow = headers.map(h => JSON.stringify(appConfig.COLUMN_DEFINITIONS[h]?.label || h)).join(',');
            ...
            parseDate: (input) => {
                ...
                const [_, year, month, day] = m.map(s => parseInt(s, 10));
    ...
    const DOMHelper = (() => ({
        create: (tag, options = {}) => {
            ...
            if (options.events) {
                for (const [event, handler] of Object.entries(options.events)) el.addEventListener(event, handler);
            }
            return el;
        }
    }))();
            ...
            const style = domHelper.create('style', {
                ...
                textContent: `
                    :root {
                        ...
                        --shadow-xs:0 1px 2px 0 rgb(0 0 0/.05); --shadow-sm:0 1px 3px 0 rgb(0 0 0/.1),0 1px 2px -1px rgb(0 0 0/.1); --shadow-md:0 4px 6px -1px rgb(0 0 0/.1),0 2px 4px -2px rgb(0 0 0/.1); --shadow-lg:0 10px 15px -3px rgb(0 0 0/.1),0 4px 6px -4px rgb(0 0 0/.1); --shadow-xl:0 20px 25px -5px rgb(0 0 0/.1),0 8px 10px -6px rgb(0 0 0/.1);
                        --border-radius-sm:6px; --border-radius-md:8px; --border-radius-lg:12px; --border-radius-xl:16px;
                    }
                    .dispatch-mask{position:fixed;inset:0;background:rgba(0,0,0,0.6);backdrop-filter:blur(4px);opacity:0;transition:opacity .3s ease;pointer-events:none; z-index:${appConfig.ZINDEX.MASK};}
                    .dispatch-mask.show{opacity:1;pointer-events:auto}
                    .dispatch-modal{font-family:'Microsoft JhengHei','Segoe UI',sans-serif;position:fixed;top:50%;left:50%;z-index:${appConfig.ZINDEX.MODAL};display:flex;flex-direction:column;opacity:0;transition:opacity .2s,transform .2s,width .2s,height .2s;pointer-events:none;transform:translate(-50%,-50%) scale(.95);background:var(--gray-100);border-radius:var(--border-radius-xl);box-shadow:var(--shadow-xl);border:1px solid rgba(255,255,255,0.2);width:1300px;height:80vh;max-width:95vw;max-height:90vh}
                    ...
                    .dispatch-header{background:#fff;border-bottom:1px solid var(--gray-200);border-radius:var(--border-radius-xl) var(--border-radius-xl) 0 0;padding:12px 20px;position:relative;cursor:grab;text-align:center;font-size:1.1rem;font-weight:600;color:var(--gray-900)}
                    ...
                    .dispatch-progress .progress-text{margin-bottom:10px}
                    ...
                    .query-criteria-display{flex-shrink:0;background-color:var(--primary-50);border:1px solid var(--primary-100);border-radius:6px;padding:8px 12px;margin-bottom:12px;font-size:13px;line-height:1.6;word-break:break-all; display: flex; flex-wrap: wrap; align-items: center;}
        ...
        const Progress = (() => {
            ...
            return {
                show(text, onStop) {
                    ...
                    textEl = domHelper.create('div', {
                        className: 'progress-text',
                        textContent: text
                    });
                    progressEl = domHelper.create('div', {
                        id: 'dispatch-progress',
                        className: 'dispatch-progress',
                        children: [textEl, stopButton]
                    });
                    document.body.appendChild(progressEl);
                },
                update(percent, text) {
                    if (textEl) textEl.textContent = `${text}（進度: ${percent}%）`;
                },
                hide() {
        ...
        const Modal = (() => {
            ...
            return {
                ...
                resetPosition() {
                    ...
                    const modal = document.getElementById(appConfig.TOOL_CONTAINER_ID);
                    if (modal) Object.assign(modal.style, lastPosition);
                }
            };
        })();
    ...
    const createApiService = (appConfig, appState, uiManager, utils) => {
        const cache = new Map();
        const requestQueue = new Map();

        const getCacheKey = (endpoint, payload) => {
            const sortedPayload = Object.keys(payload).sort().reduce((acc, key) => {
                acc[key] = payload[key];
                return acc;
            }, {});
            return `${endpoint}_${JSON.stringify(sortedPayload)}`;
        };

        async function baseFetch(url, options) {
            const response = await fetch(url, options);
            if (response.status === 401 || response.status === 403) {
                const err = new Error('TOKEN無效或過期');
        ...
        /**
         * 批次分頁獲取所有資料。
         * 此函式會為整批次的請求創建一個獨立的 AbortController，
         * 確保 "停止查詢" 只會中斷當前的批次，而不會影響其他無關的 API 請求。
         */
        async function fetchAllPagesOptimized(endpoint, payload, listName) {
            ...
            let allRecords = [];
            let totalRecords = 0;
            uiManager.Progress.show(`載入${listName}中...`, onStop);
            ...
            try {
                ...
                allRecords = firstPageData?.records || [];
                totalRecords = Number(firstPageData?.total || allRecords.length);

                if (totalRecords > allRecords.length) {
                    const totalPages = Math.ceil(totalRecords / appConfig.BATCH_PAGE_SIZE);
                    ...
                    for (let i = 0; i < pagesToFetch.length; i += appConfig.CONCURRENT_API_LIMIT) {
                        ...
                        const promises = chunk.map(p => fetchPage(p));
                        const results = await Promise.all(promises);
                        results.forEach(res => {
                            if (res?.records?.length) allRecords.push(...res.records);
                        });

                        completedPages += chunk.length;
                        const percent = Math.round(100 * completedPages / totalPages);
                        uiManager.Progress.update(percent, `載入${listName}中... ${Math.min(allRecords.length, totalRecords)}/${totalRecords} 筆`);
                    }
                } else {
                    uiManager.Progress.update(100, `載入${listName}完成 ${allRecords.length}/${totalRecords} 筆`);
                }
    ...
    const createUIComponents = (appConfig, uiManager, utils, domHelper) => {
        ...
        const TokenDialog = {
            show: (opts = {}) => {
                ...
                return createSubWindow({
                    headerContent: domHelper.create('span', {
                        textContent: `重新驗證TOKEN (${appConfig.ENV})`
                    }),
                    body: domHelper.create('div', {
                        ...
                        children: [
                            domHelper.create('p', {
                                textContent: '請從 EUIS 系統的 local/session Storage 取得您的 SSO-TOKEN 並貼在下方。',
                                style: {
                                    marginTop: '0'
                                }
                            }),
                    ...
                    footer: domHelper.create('div', {
                        ...
                        style: {
                            justifyContent: isRevalidateMode ? 'space-between' : 'flex-end'
                        },
                        children: [
                            ...(isRevalidateMode ? [domHelper.create('button', {
                                id: 'auto-check-btn',
                                className: 'dispatch-btn dispatch-outline',
                                textContent: '自動檢核'
                            })] : []),
                            domHelper.create('button', {
                    ...
                    onOpen: (modal, resolve, close) => {
                        ...
                        const autoCheckBtn = modal.querySelector('#auto-check-btn');
                        const confirmBtn = modal.querySelector('#confirm-token-btn');

                        if (isRevalidateMode && autoCheckBtn) {
                            autoCheckBtn.addEventListener('click', () => {
                                const storedToken = utils.getStoredToken();
                                if (storedToken) {
                                    sessionStorage.setItem(appConfig.TOKEN_KEY, storedToken);
                                    resolve({
                                        action: appConfig.MODAL_ACTIONS.CONFIRM,
                                        value: storedToken
                                    });
                                    close();
                                } else {
                        ...
                        confirmBtn.addEventListener('click', () => {
                            ...
                            if (value) {
                                sessionStorage.setItem(appConfig.TOKEN_KEY, value);
                                resolve({
                                    action: appConfig.MODAL_ACTIONS.CONFIRM,
                                    value
                                });
        ...
        const ErrorDialog = {
            show: ({
                ...
                const details = () => {
                    const parts = [];
                    if (error.status) parts.push(`HTTP 狀態: ${error.status}`);
                    if (error.message) parts.push(`訊息: ${error.message}`);
                    if (error.data) {
                        let snippet = typeof error.data === 'string' ? error.data : JSON.stringify(error.data, null, 2);
                ...
                return createSubWindow({
                    ...
                    onOpen: (modal, resolve, close) => {
                        ...
                        modal.querySelector('#copy-btn').addEventListener('click', async () => {
                            try {
                                await navigator.clipboard.writeText(modal.querySelector('pre').textContent || '');
                                uiManager.Toast.show('已複製', 'success', 1200);
                            } catch (err) {
                                uiManager.Toast.show('複製失敗', 'error');
                            }
                        });
                        modal.querySelector('#back-btn').addEventListener('click', () => {
                            resolve({
                                action: appConfig.MODAL_ACTIONS.BACK
                            });
                            close();
                        });
                        modal.querySelector('#retry-btn').addEventListener('click', () => {
                            resolve({
                                action: appConfig.MODAL_ACTIONS.RETRY
                            });
                            close();
                        });
                    }
        ...
        const QueryBuilderDialog = {
            show: (opts = {}) => {
                ...
                const createField = (key, value) => {
                    ...
                    if (Object.keys(appConfig.SELECT_OPTIONS).includes(key)) {
                        control = domHelper.create('select', {
                            id: `query-${key}`,
                            className: 'dispatch-input'
                        });
                        const options = appConfig.SELECT_OPTIONS[key] || {};
                        for (const [val, text] of Object.entries(options)) {
                            control.add(new Option(text, val));
                        }
                    } else {
                        const parsedDate = def.type === 'date' ? utils.parseDate(value) : {
                            display: value || '',
                            full: value || ''
                        };
                        control = domHelper.create('input', {
                            ...
                            className: 'dispatch-input',
                            attributes: {
                                value: parsedDate.display,
                                'data-full-value': parsedDate.full,
                        ...
                        if (def.type === 'date') {
                            control.addEventListener('blur', (e) => {
                                ...
                                if (e.target.value && !formatted.display) {
                                    uiManager.Toast.show('日期格式不正確，請輸入 yyyy-MM-dd / yyyy/M/d / yyyymmdd', 'warning', 2500);
                                }
                            });
                        }
                    }
                    if (value) control.value = String(value).split(' ')[0];
                ...
                const createForm = (presets) => {
                    ...
                    for (const [groupTitle, keys] of Object.entries(fieldGroups)) {
                        const groupDiv = domHelper.create('div', {
                            className: 'query-form-group'
                        });
                        const groupHeader = domHelper.create('h3', {
                            textContent: groupTitle
                        });
                        const gridDiv = domHelper.create('div', {
                    ...
                    onOpen: (modal, resolve, close) => {
                        ...
                        modal.querySelector('#apply-filters-btn').addEventListener('click', () => {
                            ...
                            modal.querySelectorAll('.dispatch-input, select.dispatch-input').forEach(input => {
                                ...
                                const channelMap = appConfig.MAPPINGS.channel;
                                if (key === 'channel' && channelMap && channelMap[value]) {
                                    value = channelMap[value];
                                }
                                if (value) payload.batch[key] = value;
                            });
                            resolve({
                                action: appConfig.MODAL_ACTIONS.APPLY_SESSION_FILTERS,
                                payload
                            });
                            close();
                        });
                        modal.querySelector('#reset-defaults-btn').addEventListener('click', () => {
                            resolve({
                                action: appConfig.MODAL_ACTIONS.RESET_AND_RELOAD
                            });
                            close();
                        });
                        modal.querySelector('#back-btn').addEventListener('click', () => {
                ...
                return createSubWindow({
                    ...
                    body: domHelper.create('div', {
                        ...
                        children: [
                            domHelper.create('p', {
                                ...
                                children: [
                                    document.createTextNode(`已選取 `),
                                    domHelper.create('strong', {
                                        textContent: String(selectedCount)
                                    }),
                                    document.createTextNode(` 筆案件，請選擇或輸入指派人員：`)
                                ]
                            }),
                            domHelper.create('div', {
                                style: {
                                ...
                                children: [
                                    ...
                                    domHelper.create('button', {
                                        id: 'import-personnel-btn',
                                        className: 'dispatch-btn dispatch-outline small',
                                        textContent: '匯入人員'
                                    })
                                ]
                            }),
                            domHelper.create('div', {
                                ...
                                children: [
                                    ...
                                    domHelper.create('input', {
                                        type: 'text',
                                        id: 'manual-assignee-input',
                                        className: 'dispatch-input',
                                        attributes: {
                                            placeholder: '請輸入完整 AD 帳號'
                                        },
                                        style: {
                                            display: 'none'
                                        }
                    ...
                    onOpen: (modal, resolve, close) => {
                        ...
                        const populateSelect = (list) => {
                            ...
                            const special = [],
                                regular = [];
                            uniqueList.forEach(u => (appConfig.SPECIAL_ASSIGNEES.includes(u) ? special : regular).push(u));
                            special.sort();
                            regular.sort();
                            selectEl.innerHTML = '';
                            [...special, ...regular].forEach(u => {
                                const option = new Option(u, u);
                                if (appConfig.SPECIAL_ASSIGNEES.includes(u)) {
                                    option.classList.add('special-assignee');
                                }
                                selectEl.add(option);
                            });
                        };
                        const updateConfirmBtnState = () => {
                            confirmBtn.disabled = !(manualCheckbox.checked ? manualInput.value.trim() !== '' : !!selectEl.value);
                        };
                        ...
                        selectEl.addEventListener('change', updateConfirmBtnState);
                        manualInput.addEventListener('input', (e) => {
                            e.target.value = e.target.value.toLowerCase();
                            updateConfirmBtnState();
                        });
                        modal.querySelector('#import-personnel-btn').addEventListener('click', handleImport);
                        modal.querySelector('#back-btn').addEventListener('click', () => {
                            resolve({
                                action: appConfig.MODAL_ACTIONS.BACK
                            });
                        ...
                        confirmBtn.addEventListener('click', () => {
                            ...
                            resolve({
                                action: appConfig.MODAL_ACTIONS.CONFIRM_ASSIGNMENT,
                                assignee
                            });
                            close();
                        });
        ...
        const AssignmentResultDialog = {
            show: ({
                ...
                const bodyChildren = [
                    domHelper.create('p', {
                        children: [
                            document.createTextNode(`已完成對 `),
                            domHelper.create('strong', {
                                textContent: assignee
                            }),
                            document.createTextNode(` 的派件操作。`)
                        ]
                    }),
                    domHelper.create('p', {
                        textContent: `派發總案件數: ${totalCount} 筆`,
                        style: {
                            fontWeight: 'bold'
                        }
                    }),
                    domHelper.create('p', {
                        textContent: `成功 ${successful.length} 筆，失敗 ${failed.length} 筆。`,
                        style: {
                            color: hasFailures ? 'var(--warning-color)' : 'var(--success-color)'
                        }
                    })
                ];
                if (failed.length > 0) {
                    bodyChildren.push(domHelper.create('div', {
                        ...
                        children: [
                            ...
                            domHelper.create('textarea', {
                                ...
                                textContent: failed.map(f => `受理號碼: ${f.caseId}\n原因: ${f.reason}`).join('\n\n')
                            })
                        ]
                    }));
                }
                ...
                return createSubWindow({
                    headerContent: domHelper.create('span', {
                        textContent: hasFailures ? '派件部分成功' : '派件成功'
                    }),
                    body: domHelper.create('div', {
                        className: 'dispatch-body',
                        style: {
                    ...
                    footer: domHelper.create('div', {
                        ...
                        children: [domHelper.create('button', {
                            id: 'close-result-btn',
                            className: 'dispatch-btn',
                            textContent: '關閉'
                        })]
    ...
    const createAppRunner = (appConfig, appState, apiService, uiComponents, utils, uiManager, domHelper) => {
        ...
        const TableRenderer = {
            createHeaderRow: (viewConfig) => viewConfig.columns.map(key => {
                const def = appConfig.COLUMN_DEFINITIONS[key];
                if (!def) return '';
                ...
                return `<th ${className} data-key="${def.key}" data-type="${def.type || ''}" title="點擊排序">${utils.escapeHtml(def.label)} <span class="sort-indicator"></span></th>`;
            }).join(''),
            createDataRow: (item, viewConfig, index) => viewConfig.columns.map(key => {
                const def = appConfig.COLUMN_DEFINITIONS[key];
                if (!def) return '';
                const className = (viewConfig.foldedColumns || []).includes(key) ? 'class="folded-column"' : '';
                ...
                return `<td ${className} title="${utils.escapeHtml(String(item[key] ?? ''))}">${utils.escapeHtml(String(displayValue))}</td>`;
            }).join('')
        };

        const run = async () => {
            document.querySelectorAll(`[id^="${appConfig.TOOL_CONTAINER_ID.slice(0, -4)}"]`).forEach(el => el.remove());
            ...
            appState.set('userToken', utils.getStoredToken());
            await handleMainView();
        };

        /**
         * 快照當前UI狀態，用於UI刷新後恢復，避免使用者操作狀態丟失。
         * @param {HTMLElement} modal - 主視窗的DOM元素。
         */
        function snapshotView(modal) {
            const filters = {};
            modal.querySelectorAll('.filter-select, .filter-input').forEach(el => {
                filters[el.dataset.key || el.id.replace('filter-', '')] = el.value;
            });
            const sortTh = modal.querySelector('th[data-key] .sort-indicator:not(:empty)');
            let sortState = {
                key: null,
                order: 'asc'
            };
            if (sortTh) {
                const th = sortTh.closest('th[data-key]');
                if (th) {
                    sortState = {
                        key: th.dataset.key,
                        order: sortTh.textContent === '▼' ? 'desc' : 'asc'
                    };
                }
            }
            const checked = new Set(Array.from(modal.querySelectorAll('.case-checkbox:checked')).map(cb => cb.value));
            const scrollTop = modal.querySelector('.case-table-container')?.scrollTop || 0;
            const isFiltersCollapsed = modal.querySelector('.filter-controls')?.classList.contains('collapsed') || false;
            const showAllColumns = modal.querySelector('.case-table-container')?.classList.contains('show-all-columns') || false;
            state.lastSnapshot = {
                filters,
                sortState,
                checked,
        ...
        /**
         * 根據上一次的快照，還原UI狀態。
         * @param {HTMLElement} modal - 新渲染出的主視窗DOM元素。
         */
        function restoreView(modal) {
            const snap = state.lastSnapshot;
            if (!snap) return;
            Object.entries(snap.filters).forEach(([key, val]) => {
                const el = modal.querySelector(`[data-key="${key}"], #filter-${key}`);
                if (el) el.value = val;
            });
            if (snap.sortState?.key) {
                const th = modal.querySelector(`th[data-key="${snap.sortState.key}"]`);
                if (th) th.querySelector('.sort-indicator').textContent = snap.sortState.order === 'asc' ? '▲' : '▼';
            }
            modal.querySelectorAll('.case-checkbox').forEach(cb => {
                cb.checked = snap.checked.has(cb.value);
            });
            const container = modal.querySelector('.case-table-container');
            if (container) {
                container.scrollTop = snap.scrollTop;
                container.classList.toggle('show-all-columns', snap.showAllColumns);
                const btn = modal.querySelector('#toggle-columns-btn');
                if (btn) btn.textContent = snap.showAllColumns ? '隱藏部分欄位' : '顯示更多欄位';
            }
            const fc = modal.querySelector('.filter-controls');
            if (fc) {
                fc.classList.toggle('collapsed', snap.isFiltersCollapsed);
                const btn = modal.querySelector('#toggle-filters-btn');
                if (btn) btn.textContent = snap.isFiltersCollapsed ? '展開篩選條件' : '收折篩選條件';
            }
        }
        ...
        const _generateFilterOptions = (caseList) => {
            ...
            appConfig.SHARED_FILTER_CONFIG.forEach(key => {
                ...
                const sortedValues = [...values].sort((a, b) => a.localeCompare(b, undefined, {
                    numeric: true
                }));
        ...
        async function fetchCases(mode, filters) {
            ...
            try {
                ...
                cases.sort((a, b) => new Date(b.applyDate).getTime() - new Date(a.applyDate).getTime());
                return {
                    data: cases
                };
            } catch (error) {
        ...
        async function handleMainView(opts = {}) {
            const {
                forceFetch = false, targetTabId
            } = opts;
            const activeTabId = targetTabId || appState.get('activeTabId');
            appState.set('activeTabId', activeTabId);

            let tabData, currentViewConfig, filterOptions = {},
                initialFilters = null,
                error = null,
                queryInfo = null;
            const noTokenError = new Error('SSO-TOKEN 未設定。請點擊上方 🔑 圖示設定。');
            const sharedViewConfig = {
                ...appConfig.SHARED_VIEW_CONFIG
            };

            if (activeTabId === 'personal') {
                if (!appState.get('userToken')) {
                    error = noTokenError;
                    tabData = [];
                    ...
                    if (result.error) {
                        ...
                        state.personalCases = result.data;
                    }
                }
            ...
            const caseListViewShow = (viewOpts) => {
                ...
                let sortState = state.lastSnapshot?.sortState || {
                    key: null,
                    order: 'asc'
                };
                ...
                const _createPersonnelList = (assignees) => {
                    ...
                    const listContainer = domHelper.create('ul', {
                    ...
                    sortedList.forEach(assignee => {
                        ...
                        const li = domHelper.create('li', {
                            className: `selectable-item ${isSpecial ? 'special-assignee' : ''}`,
                            textContent: assignee,
                            attributes: {
                                'data-value': assignee
                            }
                ...
                const _applyFiltersAndSort = utils.debounce(() => {
                    ...
                    elements.modal.querySelectorAll('.filter-select, .filter-input').forEach(input => {
                        if (input.value) filterValues[input.dataset.key || input.id.replace('filter-', '')] = input.value;
                    });
                    currentData = (caseList || []).filter(item =>
                        Object.entries(filterValues).every(([key, value]) => {
                            const itemValue = item[key] ?? '';
                            if (value === '(空白)') return itemValue === '';
                            if (appConfig.COLUMN_DEFINITIONS[key]?.type === 'date') {
                                return String(itemValue).split(' ')[0].startsWith(String(value));
                            }
                    ...
                    if (sortState.key) {
                        const colDef = appConfig.COLUMN_DEFINITIONS[sortState.key] || {};
                        currentData.sort((a, b) => {
                            ...
                            if (colDef.type === 'date') {
                                ...
                                valA = String(valA ?? '');
                                valB = String(valB ?? '');
                                return sortState.order === 'asc' ? valA.localeCompare(valB, undefined, {
                                    numeric: true
                ...
                function _createHeader() {
                    const headerText = `案件清單作業 v${appConfig.VERSION} (${appConfig.ENV})`;
                    const toolbar = domHelper.create('div', {
                        className: 'header-toolbar'
                    });
                    const buttons = [{
                            action: appConfig.MODAL_ACTIONS.CHANGE_TOKEN,
                            icon: '🔑',
                            title: '重新驗證Token'
                        },
                        {
                            action: appConfig.MODAL_ACTIONS.OPEN_NEW_QUERY,
                            icon: '✨',
                            title: '開啟新查詢'
                        },
                        {
                            action: appConfig.MODAL_ACTIONS.CLEAR_CACHE,
                            icon: '🧹',
                            title: '清除查詢頁籤與快取'
                        },
                        {
                            action: appConfig.MODAL_ACTIONS.RESET_WINDOW,
                            icon: '🖼️',
                            title: '還原視窗預設值'
                        },
                    ];
                    buttons.forEach(btnInfo => {
                        const tooltip = domHelper.create('span', {
                            className: 'tooltip-text',
                            textContent: btnInfo.title
                        });
                        const btn = domHelper.create('button', {
                            textContent: btnInfo.icon,
                            children: [tooltip],
                            events: {
                                click: () => elements.resolve({
                    ...
                    const headerContent = domHelper.create('span', {
                ...
                function _createManualOpView() {
                    const defaultPane = domHelper.create('div', {
                        className: 'personnel-selector-pane active',
                        attributes: {
                            'data-pane': 'default'
                        }
                    });
                    defaultPane.appendChild(_createPersonnelList(assigneeList));
                    const importPane = domHelper.create('div', {
                        className: 'personnel-selector-pane',
                        attributes: {
                            'data-pane': 'import'
                        }
                    });
                    const manualPane = domHelper.create('div', {
                        className: 'personnel-selector-pane',
                        attributes: {
                            'data-pane': 'manual'
                        },
                        children: [domHelper.create('input', {
                            id: 'manual-assignee-input',
                            type: 'text',
                            className: 'dispatch-input',
                            attributes: {
                    ...
                    const personnelSection = domHelper.create('div', {
                        ...
                        children: [
                            ...
                            domHelper.create('div', {
                                className: 'personnel-selector-content',
                                children: [defaultPane, importPane, manualPane]
                            })
                        ]
                    });
                            ...
                            domHelper.create('div', {
                                ...
                                children: [
                                    ...
                                    domHelper.create('textarea', {
                                        ...
                                        className: 'dispatch-input',
                                        attributes: {
                                            placeholder: '請輸入受理號碼，以逗號、分號、空格或換行分隔',
                                            style: 'margin-top: 0; flex-grow: 1; resize: vertical;'
                                        }
                                    })
                                ]
                            }),
                ...
                function _createBody() {
                    const tabButtons = tabs.map(tab => domHelper.create('button', {
                        ...
                        children: tab.canClose ? [domHelper.create('button', {
                    ...
                    const contentWrapper = domHelper.create('div', {
                        className: 'tab-content-wrapper'
                    });
                    ...
                    if (activeTabId === 'manual') {
                            ...
                            const renderControl = (key) => {
                                ...
                                if (appConfig.TEXT_INPUT_FILTERS.includes(key)) {
                                    control = domHelper.create('input', {
                                        type: 'text',
                                        id: `filter-${key}`,
                                        className: 'dispatch-input filter-input',
                                        attributes: {
                                            placeholder: `搜尋 ${labelText}...`,
                                            ...(isErrorState && {
                                                disabled: true
                                            }),
                                            'data-key': key
                                    ...
                                    control = domHelper.create('select', {
                                        ...
                                        children: [
                                            ...
                                            ...options.map(opt => domHelper.create('option', {
                                                textContent: opt,
                                                attributes: {
                                                    value: opt
                                                }
                                    ...
                                    const countSpan = domHelper.create('span', {
                                        className: 'filter-label-count',
                                        textContent: `(${options.length})`
                                    });
                                    const label = domHelper.create('label', {
                                        style: {
                                            fontSize: '14px',
                                        ...
                                        children: [document.createTextNode(labelText), countSpan]
                                    });
                                    controlWrapper.append(label, control);
                                }
                                return controlWrapper;
                        ...
                        if (currentFilters) {
                            const criteriaItems = Object.entries(currentFilters).map(([key, value]) => {
                                if (!value) return null;
                                const label = appConfig.COLUMN_DEFINITIONS[key]?.label ?? key;
                                const displayValue = String(value).split(' ')[0];
                                return domHelper.create('span', {
                                    children: [
                                        domHelper.create('strong', {
                                            textContent: `${label}:`
                                        }),
                            ...
                            if (criteriaItems.length > 0) {
                                queryDisplay = domHelper.create('div', {
                                    className: 'query-criteria-display',
                                    children: [
                                        domHelper.create('strong', {
                                            textContent: '查詢條件: ',
                                            style: {
                                        ...
                                        ...criteriaItems.reduce((acc, item, index) => {
                                            acc.push(item);
                                            if (index < criteriaItems.length - 1) acc.push(domHelper.create('span', {
                                                textContent: '; ',
                                                style: {
                                                    margin: '0 4px'
                                                }
                        ...
                        const listViewElements = [
                                    ...
                                    domHelper.create('div', {
                                        ...
                                        children: [
                                            ...
                                            domHelper.create('button', {
                                                ...
                                                textContent: '重新載入',
                                                style: {
                                                    display: isErrorState ? 'inline-flex' : 'none'
                                                }
                                            })
                                        ]
                                    })
                            ...
                            domHelper.create('div', {
                                id: 'case-table-container',
                                className: 'case-table-container',
                                children: [
                                    domHelper.create('table', {
                                        id: 'case-table',
                                        className: 'case-table',
                                        children: [
                                            domHelper.create('thead', {
                                                children: [domHelper.create('tr', {
                                                    innerHTML: TableRenderer.createHeaderRow(viewConfig)
                                                })]
                                            }),
                        ...
                        contentWrapper.appendChild(domHelper.create('p', {
                            textContent: '請選擇一個頁籤。'
                        }));
                    }
                    return domHelper.create('div', {
                ...
                function _createFooter() {
                    ...
                    const rightSide = domHelper.create('div', {
                        style: {
                            display: 'flex',
                            gap: '8px',
                            marginLeft: 'auto'
                    ...
                    if (activeTabId === 'manual') {
                        rightSide.appendChild(domHelper.create('button', {
                            id: 'manual-dispatch-btn',
                            className: 'dispatch-btn',
                            textContent: '執行派件'
                        }));
                ...
                function _bindEvents(resolve) {
                    ...
                    elements.modal.querySelector('.dispatch-tabs')?.addEventListener('click', (e) => {
                        const target = e.target;
                        const button = target.closest('button[data-tab-id]');
                        if (target.matches('.close-tab-btn')) {
                            e.stopPropagation();
                            snapshotView(elements.modal);
                            elements.resolve({
                                action: appConfig.MODAL_ACTIONS.CLOSE_QUERY_TAB,
                                tabId: target.dataset.tabId
                            });
                        } else if (button && !button.classList.contains('active')) {
                            snapshotView(elements.modal);
                            elements.resolve({
                                action: appConfig.MODAL_ACTIONS.SWITCH_TAB,
                    ...
                    if (activeTabId === 'manual') {
                        ...
                        cardBtns.forEach(btn => {
                            btn.addEventListener('click', async () => {
                                ...
                                cardBtns.forEach(b => b.classList.toggle('active', b.getAttribute('data-pane') === paneId));
                                ...
                                if (paneId === 'import') {
                                    try {
                                        ...
                                        cardBtns.forEach(b => b.classList.toggle('active', b.getAttribute('data-pane') === 'default'));
                                        panes.forEach(p => p.classList.toggle('active', p.getAttribute('data-pane') === 'default'));
                                    }
                                }
                            });
                        });
                        ...
                        container.querySelector('#import-cases-btn').addEventListener('click', async () => {
                            try {
                        ...
                        container.querySelector('#clear-cases-btn').addEventListener('click', () => {
                            casesInput.value = '';
                        });
                        ...
                        if (toggleFiltersBtn) {
                            toggleFiltersBtn.addEventListener('click', () => {
                                snapshotView(elements.modal);
                                elements.resolve({
                                    action: appConfig.MODAL_ACTIONS.UI_REFRESH
                                });
                            });
                        }
                        ...
                        elements.modal.querySelector('#clear-filters-btn').addEventListener('click', () => {
                            snapshotView(elements.modal);
                        ...
                        elements.modal.querySelector('#export-csv-btn').addEventListener('click', () => {
                            ...
                            const headers = appConfig.SHARED_VIEW_CONFIG.columns.filter(c => c !== 'select');
                            const csvData = utils.jsonToCsv(currentData, {
                                headersOrder: headers
                            });
                            utils.downloadCsv(csvData, filename);
                        });
                        elements.modal.querySelector('#reload-view-btn')?.addEventListener('click', () => {
                            state.lastSnapshot = null;
                            elements.resolve({
                                action: appConfig.MODAL_ACTIONS.RELOAD_VIEW
                            });
                        });
                        if (isErrorState) {
                            elements.modal.querySelector('#retry-fetch-btn')?.addEventListener('click', () => {
                                state.lastSnapshot = null;
                                elements.resolve({
                                    action: appConfig.MODAL_ACTIONS.RELOAD_VIEW
                                });
                            ...
                            elements.modal.querySelectorAll('.filter-select, .filter-input').forEach(el => {
                                el.addEventListener('change', _applyFiltersAndSort);
                                if (el.tagName === 'INPUT') el.addEventListener('keyup', _applyFiltersAndSort);
                            });
                        }
                        ...
                        if (!isErrorState && elements.table) {
                            ...
                            elements.table.addEventListener('dblclick', e => {
                                const target = e.target;
                                if (target.tagName === 'TD' && !target.querySelector('input')) {
                                    navigator.clipboard.writeText(target.textContent || '').then(() => {
                                        uiManager.Toast.show('已複製', 'success', 1000);
                                    }).catch(err => {
                                        uiManager.Toast.show('複製失敗', 'error');
                    ...
                    if (dispatchBtn) {
                        dispatchBtn.addEventListener('click', () => {
                            ...
                            if (!assignee) return uiManager.Toast.show('請選擇或輸入指派人員', 'error');
                            elements.resolve({
                                action: appConfig.MODAL_ACTIONS.MANUAL_DISPATCH,
                                cases,
                                assignee
                            });
                        });
                    ...
                    if (elements.nextBtn) {
                        elements.nextBtn.addEventListener('click', () => {
                            ...
                            if (selectedCases.length > 0) {
                                snapshotView(elements.modal);
                                elements.resolve({
                                    action: appConfig.MODAL_ACTIONS.NEXT_STEP,
                                    selectedCases
                                });
                            }
                        });
            ...
            switch (res?.action) {
                ...
                case appConfig.MODAL_ACTIONS.RELOAD_VIEW:
                    ...
                    break;
                case appConfig.MODAL_ACTIONS.UI_REFRESH:
                    await handleMainView({
                        targetTabId: appState.get('activeTabId')
                    });
                    break;
        ...
        async function handleTokenChange() {
            ...
            if (res?.action === appConfig.MODAL_ACTIONS.CONFIRM && res.value) {
                appState.set('userToken', res.value);
                state.personalCases = null;
                state.batchCases = null;
                state.queryTabs = [];
                apiService.clearCache();
                uiManager.Toast.show('Token 已更新並儲存至當前會話', 'success');
                await handleMainView({
                    forceFetch: true
                });
            } else {
        ...
        async function handleNewQuery() {
            ...
            if (res?.action === appConfig.MODAL_ACTIONS.APPLY_SESSION_FILTERS) {
                ...
                if (result.data) {
                    ...
                    const retryRes = await uiComponents.ErrorDialog.show({
                        error: result.error
                    });
        ...
        async function handleClearCache() {
            if (window.confirm('您確定要清除所有「查詢結果」頁籤、快取與已儲存的 Token 嗎？')) {
                apiService.clearCache();
                ...
                uiManager.Toast.show('已清除所有查詢頁籤、快取與 Token', 'success');
        ...
        async function handleAssignment(assignee, cases) {
            ...
            try {
                ...
                if (error.name !== 'AbortError') {
                    uiManager.Progress.hide();
                    const res = await uiComponents.ErrorDialog.show({
                        error
                    });
                    if (res?.action === appConfig.MODAL_ACTIONS.RETRY) {
                        await handleAssignment(assignee, cases);
                    } else {
                        await handleMainView();
                    }
                }
    ...
    // === 8. 應用程式啟動區 (Application Bootstrap) ===
    try {
        ...
        const apiService = createApiService(AppConfig, appState, uiManager, utils);
        const uiComponents = createUIComponents(AppConfig, appState, uiManager, utils, domHelper);
        ...
        try {
            const uiManager = createUIManager({}, {}, createUtils({}), DOMHelper);
            uiManager.injectStyle();
