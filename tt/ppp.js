javascript:(function() {
    ...
    /**
     * =================================================================================
     * KGI Plan Code Query Tool v2.2.1 (Final Build)
     *
     * @version 2.2.1
     * @author Gemini AI
     * @description
     ...
     *
     * --- v2.2.1 更新日誌 ---
     * 1. [新增] 視窗管理：新增「最小化」功能與可拖曳的小部件，方便在查詢時進行多工操作 (移除收折功能)。
     * 2. [新增] 篩選器擴充：結果頁篩選器新增「通路」與「通路銷售狀態」選項，並為篩選器區塊加上外框。
     * 3. [新增] 資料時間戳：右上角標題列新增「資料更新時間」，並在更新資料後自動刷新。
     * 4. [新增] 「顯示未銷售通路」開關：在通路詳情視圖下，可動態決定是否顯示「未銷售」的通路。
     * 5. [優化] 「更新」邏輯：點擊「更新」將在背景重新擷取 API 資料，並自動刷新所有頁籤的現有查詢結果。
     * 6. [優化] 介面調整：簡化篩選器標題佈局、恢復「一頁顯示」按鈕、恢復狀態 Emoji 與通路顏色分隔符、
     * 簡化商品名稱 Tooltip、隱藏「修改 Token」按鈕並調整部分按鈕文字。
     * 7. [優化] 程式碼架構：採納了消除魔法字串、DRY原則、擴大事件委派範圍等建議，提升程式碼品質。
     * 8. [修正] 修正了排序與「more」按鈕在表格刷新後失效的 Bug。
     * 9. [修正] 恢復「預覽」功能的完整樣式，包含頁籤與「欄/列置換」功能。
     * =================================================================================
     */
    ...
    const ConfigModule = Object.freeze({
        ...
        UI_STATES: {
            ...
            WINDOW_COLLAPSED: 'collapsed', // Kept for potential future use, but UI removed
...
`;
            ...
            render: () => {
                ...
                if (windowState === ConfigModule.UI_STATES.WINDOW_MINIMIZED) {
                    ...
                    setTimeout(() => {
                        ...
                        if (!document.getElementById(ConfigModule.WIDGET_ID)) {
             ...
             return `<div class="pct-results-container">
                <div class="pct-result-top-controls">
                    <div class="pct-controls-left">
                        ...
                        <div style="display:flex; align-items:center; gap: 8px; ${viewMode === ConfigModule.UI_STATES.VIEW_STANDARD ? 'display:none;' : ''}">
    ...
    const EventModule = (() => {
        ...
        const bindModalEvents = () => {
            const modalHeader = document.querySelector('.pct-modal-header');
            if (modalHeader) makeDraggable(modalHeader.parentElement);
        ...
        const bindContentEvents = () => {
            ...
            if (activeTab.uiState === ConfigModule.UI_STATES.QUERY) {
                ...
                document.getElementById('pct-show-not-sold-toggle')?.addEventListener('change', ControllerModule.handleShowNotSoldToggle);
    ...
    const ControllerModule = (() => {
        ...
        const _getChannelsText = (item) => {
            const channels = Object.entries(item.fixedChannelStatuses)
                .map(([channel, data]) => ({ channel, ...data }))
                .filter(ch => ch.status !== ConfigModule.MASTER_STATUS_TYPES.NOT_SOLD)
                .sort((a, b) => (ConfigModule.STATUS_ORDER[a.status] || 99) - (ConfigModule.STATUS_ORDER[b.status] || 99));

            if (channels.length === 0) return "查無銷售通路";
            
            return channels.reduce((acc, ch, index) => {
                const span = ch.channel;
                if (index > 0 && ch.status !== channels[index - 1].status) acc.push(' | ');
                    ...
                    if (filter.type === 'standard') {
                        if (filter.key === 'channels') {
                            ...
                            Object.values(item.fixedChannelStatuses).forEach(ch => {
                                if(ch.status) values.add(ConfigModule.MASTER_STATUS_TEXT[ch.status]);
                            });
                        }
                        else {
