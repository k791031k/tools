javascript:(function() {
    ...
    /**
     * =================================================================================
     * KGI Plan Code Query Tool v2.1.0 (Feature Complete)
     *
     * @version 2.1.0
     * @author Gemini AI
     * @description
     * 功能完整版本。整合了所有使用者提出的需求，包含核心架構重構與多項體驗優化。
     *
     * --- v2.1.0 更新日誌 ---
     * 1. [新增] 結果頁視圖切換：新增「切換視圖」按鈕，允許在「標準視圖」
     * （含POLPLN）與「通路詳情視圖」（各通路獨立欄位）之間自由切換。
     * 2. [新增] 查詢條件摘要：結果頁頂部會顯示生成該結果的查詢條件，
     * 方便使用者了解當前資料的上下文。
     * 3. [新增] 頁籤懸停提示：滑鼠停留在頁籤上時，會以提示框形式顯示
     * 該頁籤的完整查詢條件。
     * 4. [優化] 開放無條件查詢：在查詢頁面不設定任何條件直接查詢，
     * 將會載入並顯示所有商品資料。
     * 5. [修正] 修正了點擊「more」按鈕後，在表格刷新後可能失效的 Bug。
     *
     * --- 核心架構 (v2.0) ---
     * - [統一查詢介面] 整合為單一智慧查詢介面。
     * - [頁籤系統] 支援多工查詢與結果保存。
    ...
    const StateModule = (() => {
        ...
        const createNewTabState = (id, name) => ({
            ...
            results: {
                initialData: [], filteredData: [], viewMode: 'standard', // 'standard' or 'channelDetail'
...
`;
    ...
    const EventModule = (() => {
        ...
        const bindContentEvents = () => {
            ...
            if (activeTab.uiState === 'query') {
                ...
                // Use robust event delegation on a persistent parent (#pct-table-wrap)
    ...
    const ControllerModule = (() => {
        ...
        const rerenderTable = () => {
            ...
            const colspan = activeTab.results.viewMode === 'standard' ? 11 : 9 + ConfigModule.FIELD_MAPS.CHANNELS.length;
        ...
        const handleViewToggle = () => {
            ...
            const button = document.getElementById('pct-view-toggle');
            if (button) button.textContent = results.viewMode === 'standard' ? '檢視通路詳情' : '檢視標準模式';
            rerenderTable();
        };
        ...
        const renderStatusPill = (status) => {
            ...
            return `<span class="pct-status-pill" title="${statusText}">${statusText}</span>`;
        };
        ...
        return { 
            ...
            handleResultSearch, handleClearResultSearch, handleNameToggle, handleViewToggle, changePage, handleTableClick,
            handleCopyAll, handlePreviewAll, handleForceUpdate, generateTabTooltipText
        };
    })();
