javascript: (function() {
    ...
    /**
     * =================================================================================
     * KGI Plan Code Query Tool v3.0.0 (Final Optimized & Refactored)
     * =================================================================================
     * 此版本根據使用者回饋，進行了最終優化：
     * - 介面：將查詢條件與篩選器整合為單一初始畫面。
     * - 流程：啟動時自動檢核權限，載入資料後，畫面只顯示查詢介面。
     * - 查詢：只有在使用者點擊「查詢」後，才動態顯示結果表格。
     * - 錯誤處理：權限失敗時提供「重新檢核」按鈕，徹底重試。
     */
...
`;
    ...
    const ApiModule = (() => {
        ...
        const callApi = async (endpoint, params, signal) => {
            ...
            const token = UtilsModule.findStoredToken();
            if (!token) throw new Error('API Token 不存在，請確認是否已登入。');
    ...
    const ControllerModule = (() => {
        ...
        const initialize = async () => {
            ...
            if (storedToken) {
                ...
                await loadDataAndShowMainUI();
            } else {
                showErrorDialog('API Token 不存在。請確認您是否已登入。', true);
            }
        };
         ...
         * 載入資料並顯示主要 UI。
         */
        const loadDataAndShowMainUI = async () => {
            try {
                ...
                showResultsDialog();
                updateFilterOptions(StateModule.get().mergedDataCache);
                rerenderTable(); // 初始畫面上沒有表格，但這一步會生成並顯示
                UIModule.Progress.hide();

            } catch (err) {
                UIModule.Progress.hide();
                if (err.name !== 'AbortError') {
                    showErrorDialog('資料載入失敗，可能原因：無權限或 API 服務異常。');
                }
            }
        };
        ...
        /**
         * 顯示查詢對話框。
         * @param {boolean} isInitial - 是否為首次啟動。
         */
        const showQueryDialog = (isInitial = false) => {
            const {
                env
            } = StateModule.get();
            const html = `
                    ...
                    <div class="pct-query-controls">
                        ...
                        <div class="pct-query-row">
                            <div class="pct-query-group">
                                <label>險種代號/名稱：</label>
                                <input type="text" id="pct-keyword-input" class="pct-query-input" placeholder="輸入險種代號或名稱進行搜尋">
                            </div>
                        </div>
                        <div class="pct-query-row">
                            <div class="pct-query-group">
                                ...
                                <div class="pct-checkbox-group">
                                    ${Object.entries(ConfigModule.MASTER_STATUS_FILTER_OPTIONS).map(([key, text]) =>
                                        `<div class="pct-checkbox-item"><input type="checkbox" class="pct-status-checkbox" value="${key}"> ${text}</div>`
                                    ).join('')}
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="pct-query-group">
                                ...
                                <div class="pct-checkbox-group">
                                    ${ConfigModule.FIELD_MAPS.CHANNELS.map(ch =>
                                        `<div class="pct-checkbox-item"><input type="checkbox" class="pct-channel-checkbox" value="${ch}"> ${ch}</div>`
                                    ).join('')}
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="pct-query-group">
                                ...
                                <div class="pct-radio-group">
                                    <div class="pct-radio-item">
                                        <input type="radio" name="pct-view-mode" value="${ConfigModule.QUERY_MODES.MASTER_CLASSIFIED}" checked> 主檔格式
                                    </div>
                                    <div class="pct-radio-item">
                                        <input type="radio" name="pct-view-mode" value="${ConfigModule.QUERY_MODES.CHANNEL_CLASSIFIED}"> 通路視窗格式
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            UIModule.Modal.show(html, () => setupQueryDialogListeners(isInitial), 'results');
        };
         ...
         * @param {boolean} isInitial - 是否為首次啟動。
         */
        const setupQueryDialogListeners = (isInitial) => {
            const state = StateModule.get();
            const keywordInput = document.getElementById('pct-keyword-input');
            const statusCheckboxes = document.querySelectorAll('.pct-status-checkbox');
            const channelCheckboxes = document.querySelectorAll('.pct-channel-checkbox');
            const modeRadios = document.querySelectorAll('input[name="pct-view-mode"]');
            const runQueryBtn = document.getElementById('pct-run-query');

            // 載入上次的查詢條件
            keywordInput.value = state.queryInput;
            state.masterStatusSelection.forEach(s => {
                const el = document.querySelector(`.pct-status-checkbox[value="${s}"]`);
                if (el) el.checked = true;
            });
            state.channelSelection.forEach(c => {
                const el = document.querySelector(`.pct-channel-checkbox[value="${c}"]`);
                if (el) el.checked = true;
            });
            const modeEl = document.querySelector(`input[name="pct-view-mode"][value="${state.queryMode}"]`);
            if (modeEl) modeEl.checked = true;

            const handleRunQuery = () => {
                StateModule.set({
                    queryInput: keywordInput.value.trim(),
                    masterStatusSelection: new Set(Array.from(statusCheckboxes).filter(cb => cb.checked).map(cb => cb.value)),
                    channelStatusSelection: new Set(Array.from(statusCheckboxes).filter(cb => cb.checked).map(cb => cb.value)),
                    channelSelection: new Set(Array.from(channelCheckboxes).filter(cb => cb.checked).map(cb => cb.value)),
                    queryMode: document.querySelector('input[name="pct-view-mode"]:checked')?.value || ConfigModule.QUERY_MODES.MASTER_CLASSIFIED,
                    isFilterVisible: true // 每次查詢都重置為展開狀態
                });
                initialQueryData = DataModule.getInitialData();
                rerenderTable();
                document.getElementById('pct-table-result-container').style.display = 'flex';
            };

            runQueryBtn.addEventListener('click', handleRunQuery);
            keywordInput.addEventListener('input', EventModule.autoFormatInput);
            keywordInput.addEventListener('keydown', (e) => {
                if (e.key === 'Enter') handleRunQuery();
            });

            // 綁定結果頁面的事件
            document.getElementById('pct-search-input')?.addEventListener('input', EventModule.autoFormatInput);
            document.getElementById('pct-search-input')?.addEventListener('input', () => {
                const timers = StateModule.get().debounceTimers;
                clearTimeout(timers.search);
                timers.search = setTimeout(() => {
                    StateModule.set({
                        searchKeyword: document.getElementById('pct-search-input').value.trim(),
                        pageNo: 1
                    });
                    rerenderTable();
        ...
        const showResultsDialog = () => {
            ...
            const tableHeaderHtml = generateResultsTableHeader();

            const html = `
                <div class="pct-modal-header">
                    <div class="pct-modal-header-title">查詢結果 (${env})</div>
                ...
                <div class="pct-modal-body" style="display: flex; flex-direction: column;">
                    ...
                    <div id="pct-table-result-container" style="display: flex; flex-direction: column; height: 100%; gap: 10px;">
                        <div class="pct-result-top-controls">
                            <div class="pct-search-name-wrapper">
                                <div class="search-wrapper" style="position: relative;">
                                    <label for="pct-search-input" style="font-size: 14px; color: #666; margin-right: 5px;">結果內搜尋:</label>
                        ...
                        <div id="pct-table-view-wrapper">
                            <div id="pct-filters-wrapper"></div>
                            <div class="pct-table-wrap" id="pct-table-wrap">
                                <table class="pct-table">${tableHeaderHtml}<tbody id="pct-table-body"></tbody></table>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        const setupResultsDialogListeners = (modal) => {
            ...
            runQueryBtn.addEventListener('click', () => {
                ...
                modal.querySelector('#pct-back-to-query').style.display = 'block';
            });
            ...
            document.getElementById('pct-preview-all')?.addEventListener('click', handlePreviewAll);
            document.getElementById('pct-copy-all')?.addEventListener('click', handleCopyAll);
            document.getElementById('pct-back-to-query')?.addEventListener('click', () => showQueryDialog(false));
        };
        ...
        const renderTableRow = (item) => {
            ...
            if (queryMode === ConfigModule.QUERY_MODES.CHANNEL_CLASSIFIED) {
                const channelCells = ConfigModule.FIELD_MAPS.CHANNELS.map(channelName => {
                    ...
                    // 修正：如果狀態是 'not sold' 則顯示空白，否則才顯示圖示和文字
        ...
        const renderChannelsCell = (item) => {
            // 修正：未銷售的通路不顯示在表格內，但會顯示在 tooltip
