javascript:(function() {
    ...
    /**
     * =================================================================================
     * KGI Plan Code Query Tool v2.0.1 (Tabbed & Unified Interface - Complete)
     *
     * @version 2.0.1
     * @author Gemini AI
     * @description
     * 基於 v2.0.0 的修正與補完版本。此版本確保了所有 v1.9.0 的輔助功能
     * (如全表複製、預覽、進階篩選器) 都已完整地適配並整合到新的頁籤化架構中，
     * 實現了功能上的完全對應，沒有任何省略。
     *
     * --- 核心功能 ---
     * 1. [統一查詢介面] 廢除了原有的四種獨立查詢模式，整合為一個包含
     * 智慧搜尋框與多維度篩選器的單一查詢介面，大幅提升查詢的彈性與效率。
     * 2. [頁籤 (Tab) 系統] 引入了類似瀏覽器的頁籤功能，允許使用者同時進行
     * 多個獨立的查詢任務。每個頁籤都會保存其完整的查詢條件與結果。
     * 3. [功能完整性] 補完了 v2.0.0 中被省略的輔助功能，包含結果頁的
     * 進階篩選器、單行複製、全表複製與全表預覽功能。
     * =================================================================================
     */
    ...
    const StateModule = (() => {
        const state = {
            global: {
                ...
                lastDbUpdateTime: '', currentQueryController: null, debounceTimers: {}, modalPosition: { top: null, left: null }
            },
            tabs: [],
            activeTabId: null,
        };
        ...
        const createNewTabState = (id, name) => ({
            ...
            results: {
                initialData: [], filteredData: [], filterOptions: {}, activeFilters: {}, advancedFilters: {},
...
.pct-modal { font-family: 'Microsoft JhengHei', 'Segoe UI', sans-serif; background: #FFFFFF; border-radius: 10px; box-shadow: 0 4px 24px rgba(0,0,0,0.15); padding: 0; max-width: 95vw; width: 1050px; height: 850px; position: fixed; top: 60px; left: 50%; transform: translateX(-50%); z-index: 2147483647; display: flex; flex-direction: column; }
...
`;
        ...
        const Modal = {
            ...
            close: () => {
                ...
                if (modal) {
                    ...
                    StateModule.setGlobal({ modalPosition: { top, left } });
                }
            ...
            render: () => {
                const modal = document.getElementById(ConfigModule.TOOL_ID);
                if (!modal) return;
                const { env } = StateModule.getGlobal();
                modal.innerHTML = `
                    ...
                    <div class="pct-modal-footer">
                        <div style="display:flex; gap:10px;">
                           ...
                           <button id="pct-force-update" class="pct-btn pct-btn-outline">強制更新資料</button>
                        </div>
            ...
            renderTabs: () => {
                ...
                tabBar.innerHTML = `
                    ${tabs.map(tab => `
                        <div class="pct-tab ${tab.id === activeTabId ? 'active' : ''}" data-tab-id="${tab.id}">
                           <span class="pct-tab-name" title="${UtilsModule.escapeHtml(tab.name)}">${UtilsModule.escapeHtml(tab.name)}</span>
        ...
        const getResultsScreenHTML = (tabState) => {
             const { lastDbUpdateTime } = StateModule.getGlobal();
             const { showPlanName } = tabState.results;
                ...
                <div class="pct-result-top-controls">
                    <div class="pct-search-name-wrapper">
                        <div class="search-wrapper" style="position: relative;">
                            ...
                            <input type="text" id="pct-search-input" placeholder="搜尋關鍵字..." value="${UtilsModule.escapeHtml(tabState.results.searchKeyword)}">
    ...
    const EventModule = (() => {
        ...
        const dragMouseDown = (e) => {
            const modal = document.getElementById(ConfigModule.TOOL_ID);
            if (!modal || e.target.closest('.pct-close-btn-custom, .pct-tab-bar')) return;
            e.preventDefault();
        ...
        const bindModalEvents = () => {
            ...
            document.getElementById('pct-change-token')?.addEventListener('click', ControllerModule.showTokenDialog);
            document.getElementById('pct-force-update')?.addEventListener('click', () => ControllerModule.handleForceUpdate());
            const tabBar = document.getElementById('pct-tab-bar');
        ...
        const bindContentEvents = () => {
            ...
            if (activeTab.uiState === 'query') {
                ...
                const keywordInput = document.getElementById('pct-query-keyword');
                keywordInput?.addEventListener('input', UtilsModule.toHalfWidthUpperCase);
                document.querySelectorAll('.pct-query-btn').forEach(btn => {
                    btn.addEventListener('click', ControllerModule.handleQuerySelection);
                });
            } else if (activeTab.uiState === 'results') {
                ...
                document.getElementById('pct-table-body')?.addEventListener('click', ControllerModule.handleTableClick);
                 document.querySelector('.pct-table thead')?.querySelectorAll('th[data-key]').forEach(th => {
                    th.addEventListener('click', () => ControllerModule.handleSort(th.dataset.key));
                });
            }
        };
    ...
    const ControllerModule = (() => {
        ...
        const showTokenDialog = () => {
            ...
            dialog.innerHTML = `<div class="pct-modal-mask"></div>
                <div class="pct-modal" style="width: 600px; height: auto;">
        ...
        const handleStartQuery = async () => {
            ...
            if (!activeTab.query.keyword && activeTab.query.masterStatus.size === 0 && activeTab.query.channels.size === 0 && activeTab.query.channelStatus.size === 0) {
                UIModule.Toast.show('請至少輸入一個查詢條件', 'warning'); return;
            }
        ...
        const rerenderTable = () => {
            ...
            if (!activeTab || activeTab.uiState !== 'results' || !tableWrap) {
                document.getElementById('pct-table-body')?.remove(); return;
            }
            ...
            const { isFullView, pageNo, pageSize, sortKey, sortAsc } = activeTab.results;
            ...
            tableWrap.innerHTML = `<table class="pct-table">
                ...
                <tbody id="pct-table-body">${displayData.map(renderTableRow).join('') || `<tr><td colspan="11" style="text-align:center; padding: 20px;">查無符合條件的資料</td></tr>`}</tbody>
            </table>`;
            tableWrap.querySelectorAll('thead th[data-key]').forEach(th => th.addEventListener('click', () => handleSort(th.dataset.key)));
        ...
        const handleSort = (key) => {
            ...
            results.sortKey = key;
            results.sortAsc = (results.sortKey === key) ? !results.sortAsc : true;
            rerenderTable();
        };
        ...
        const generateResultsTableHeader = () => `<thead><tr>
            <th data-key="no" style="width: 4%;">No</th><th data-key="planCode" style="width: 7%;">代號</th>
            <th data-key="displayName" style="width: 27%;">商品名稱</th><th data-key="currency" style="width: 5%;">幣別</th>
            <th data-key="unit" style="width: 5%;">單位</th><th data-key="coverageType" style="width: 6%;">型態</th>
            <th data-key="saleStartDate" style="width: 8%;">主檔銷售日</th><th data-key="saleEndDate" style="width: 8%;">主檔停售日</th>
            <th data-key="mainStatus" style="width: 9%;">主檔狀態</th><th data-key="polpln" style="width: 7%;">POLPLN</th>
            <th data-key="channels" style="width: 14%;">銷售通路</th></tr></thead>`;
        const renderTableRow = (item) => {
            const { showPlanName } = StateModule.getActiveTab().results;
            ...
            const polplnCellContent = (polplnValue === undefined) ? `<button class="pct-load-polpln-btn" data-plancode="${item.planCode}">more</button>` : `<span class="clickable-cell">${UtilsModule.escapeHtml(polplnValue)}</span>`;
            const { content: channelsCellContent, tooltip: tooltipText } = renderChannelsCell(item);
            return `<tr class="${item.mainStatus === ConfigModule.MASTER_STATUS_TYPES.ABNORMAL_DATE ? 'pct-row-abnormal-date' : ''}">
                ...
                <td class="clickable-cell">${UtilsModule.escapeHtml(item.currency)}</td><td class="clickable-cell">${UtilsModule.escapeHtml(item.unit)}</td>
                <td class="clickable-cell">${UtilsModule.escapeHtml(item.coverageType)}</td><td class="clickable-cell">${UtilsModule.escapeHtml(item.saleStartDate)}</td>
                <td class="clickable-cell">${UtilsModule.escapeHtml(item.saleEndDate)}</td><td>${renderStatusPill(item.mainStatus)}</td>
                <td>${polplnCellContent}</td><td class="pct-align-left" title="${tooltipText}">${channelsCellContent}</td></tr>`;
        };
        const renderChannelsCell = (item) => {
            ...
            const content = channelsToDisplay.map(ch => {
                const classMap = {
                ...
                return `<span class="${classMap[ch.status] || 'pct-channel-notsold'}">${ch.channel}</span>`;
            }).join(' ');
        ...
        const handleTableClick = async (e) => {
            ...
            if (target.classList.contains('clickable-cell')) {
                ...
                const item = results.filteredData.find(d => d.no === rowNo);
        ...
        const generatePreviewPageHTML = (data) => {
            const { results } = StateModule.getActiveTab();
            const { polplnDataCache, rawPolplnDataCache } = StateModule.getGlobal();
            const createTableHTML = (dataArray, keysToShow) => {
                ...
                let tbody = '<tbody>' + dataArray.map(row => '<tr>' + headers.map(hKey => `<td>${UtilsModule.escapeHtml(row[hKey])}</td>`).join('') + '</tr>').join('') + '</tbody>';
                return `<div class="pct-preview-table-wrap"><table>${thead}${tbody}</table></div>`;
            };
            const processedData = data.map(item => {
                const { content } = renderChannelsCell(item);
                return {
                    'No': item.no, '代號': item.planCode, '商品名稱': results.showPlanName ? item.fullName : item.displayName, '幣別': item.currency, '單位': item.unit,
                    '型態': item.coverageType, '主檔銷售日': item.saleStartDate, '主檔停售日': item.saleEndDate, '主檔狀態': ConfigModule.MASTER_STATUS_TEXT[item.mainStatus],
                    'POLPLN': polplnDataCache.get(item.planCode) || '', '銷售通路': content.replace(/<[^>]*>/g, ' ')
                };
            });
            const rawMasterData = data.map(d => d._raw.master);
            const rawChannelData = data.flatMap(d => d._raw.channels.map(c => ({planCode: d.planCode, ...c})));
            const currentPlanCodes = new Set(data.map(d => d.planCode));
            const rawPolplnData = Array.from(rawPolplnDataCache.entries()).filter(([pc]) => currentPlanCodes.has(pc)).flatMap(([, r]) => r);

            return `<!DOCTYPE html><html><head><title>全表預覽</title><style>body{font-family:sans-serif;} table{border-collapse:collapse;font-size:12px;} th,td{border:1px solid #ccc;padding:4px 6px;}</style></head>
                <body><h2>合併處理後資料 (${processedData.length}筆)</h2>${createTableHTML(processedData, ['No', '代號', '商品名稱', '幣別', '單位', '型態', '主檔銷售日', '主檔停售日', '主檔狀態', 'POLPLN', '銷售通路'])}
                <h2>商品主檔原始資料 (${rawMasterData.length}筆)</h2>${createTableHTML(rawMasterData)}
                <h2>通路銷售原始資料 (${rawChannelData.length}筆)</h2>${createTableHTML(rawChannelData)}
                <h2>POLPLN 原始資料 (${rawPolplnData.length}筆)</h2>${createTableHTML(rawPolplnData)}</body></html>`;
        };
        const handleForceUpdate = () => {
             UIModule.Toast.show('正在清除快取並從伺服器更新資料...', 'info', 3000);
             StateModule.clearAllCaches();
             UIModule.Toast.show('資料快取已清除。請重新點擊查詢以載入最新資料。', 'success', 4000);
             StateModule.getTabs().forEach(tab => {
                tab.uiState = 'query'; tab.results.initialData = []; tab.results.filteredData = [];
             });
             UIModule.Modal.renderContent();
        };
        ...
        const renderFilterControls = (shouldUpdateOptions = true) => {
            ...
            let headerHtml = `<div class="pct-filters-header"><h4>篩選器</h4><button id="pct-toggle-filters-btn" class="pct-btn pct-btn-outline">${results.isFilterVisible ? '收折' : '展開'}</button></div>`;
        ...
        return { 
            ...
            handleQuerySelection, handleStartQuery, handleBackToQuery, rerenderTable, handleSort,
            handleResultSearch, handleClearResultSearch, handleNameToggle, changePage, handleTableClick,
            handleCopyAll, handlePreviewAll, handleForceUpdate
        };
    })();
