javascript:(function() {
    ...
    /**
     * =================================================================================
     * KGI Plan Code Query Tool v2.2.3 (Date Simulation Build)
     *
     * @version 2.2.3
     * @author Gemini AI
     * @description
     * 新增「模擬日期查詢」功能，允許使用者自訂基準日期來判斷商品銷售狀態，
     * 便於預測未來或回溯過去的商品上下架情況。
     *
     * --- v2.2.3 更新日誌 ---
     * 1. [新增] 模擬日期查詢：查詢頁面新增「基準日期」選擇器。若設定，所有狀態判斷
     * （主檔、通路、即將停售等）皆以此日期為準。
     * 2. [優化] 狀態判斷邏輯：重構資料處理流程，商品狀態改為在查詢當下根據
     * 「基準日期」動態計算，而非在資料載入時預先處理，以支援模擬日期功能。
     * 3. [優化] 資訊呈現：已選的「基準日期」會顯示在結果頁的查詢摘要與頁籤懸停提示中。
     * =================================================================================
     */
...
.pct-modal { font-family: 'Microsoft JhengHei', 'Segoe UI', sans-serif; background: #FFFFFF; border-radius: 10px; box-shadow: 0 4px 24px rgba(0,0,0,0.15); padding: 0; max-width: 90vw; width: 1250px; height: 750px; position: fixed; top: 60px; left: 50%; transform: translateX(-50%); z-index: 2147483647; display: flex; flex-direction: column; transition: transform 0.3s ease, opacity 0.3s ease, height 0.3s ease; }
...
`;
        ...
        const getQueryScreenHTML = (tabState) => {
            ...
            <div class="pct-query-container">
                <div class="pct-query-section">
                 ...
                 <div class="pct-query-section">
                    <h3 class="pct-query-section-title">2. 基準日期 (可選，預設為今天)</h3>
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <input type="date" id="pct-query-basedate" class="pct-input" style="flex-grow: 1; margin-top: 0;" value="${baseDate || ''}">
                        <button id="pct-clear-basedate" class="pct-btn pct-btn-outline">清除</button>
                    </div>
                </div>
                <div class="pct-query-section">
    ...
    const DataModule = (() => {
        ...
        const getInitialDataForTab = (queryState) => {
            ...
            const referenceDate = (baseDate ? baseDate.replace(/-/g, '') : UtilsModule.formatToday(new Date()));
            
            // Step 1: Map raw data to include dynamic statuses based on the reference date
            ...
            // Step 2: Filter the data with dynamic statuses
    ...
    const ControllerModule = (() => {
        ...
        const PreviewGenerator = {
            ...
            generateHTML: (data) => {
                ...
                const tab2Content = PreviewGenerator._createTableHTML('preview-table-master', rawMasterData);
                const rawChannelData = data.flatMap(d => d._raw.channels.map(channel => ({planCode: d.planCode, ...channel})));
                const tab3Content = PreviewGenerator._createTableHTML('preview-table-channel', rawChannelData);
