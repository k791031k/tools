 ...
 * @version 25.0.0-Refactored
 * @description
 * 重構版本：此版本基於 v25.0.0 的完整功能進行架構重構。
 * - 導入依賴注入 (DI) 與事件匯流排 (EventBus) 模式，實現高度解耦。
 * - 將原有模組 (Module) 拆分為職責更單一的服務類別 (Service Class)。
 * - 保持所有原有功能、查詢邏輯、UI/UX 不變。
 * - 提升程式碼的可讀性、可擴展性與可維護性。
 */
...
javascript:(function() {
    ...
    /**
     * @module Constants
     * @description 統一常數管理，取代原 ConfigModule。
     */
    ...
    class StateService {
        ...
        _getInitialState() {
            return {
                ...
                pageNo: 1,
                pageSize: Constants.DEFAULTS.PAGE_SIZE_TABLE,
                isFullView: false,
                searchKeyword: '',
                sortKey: 'no',
                sortAsc: true,
    ...
    class ErrorHandler {
        ...
        handleApiError(error, context = 'API 操作') {
            if (error.name === 'AbortError') {
                this.toastService.show('操作已取消', 'info');
                return;
            }
        ...
        async callApi(endpoint, params, signal) {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    try { errorText = JSON.parse(errorText).message || errorText; } catch (e) { /* silent */ }
    ...
    /**
     * @class DataService
     * @description 負責資料的獲取、合併、快取、篩選與排序。
     */
    class DataService {
        ...
        getFilteredData() {
            ...
            switch (queryMode) {
                case Constants.QUERY_MODES.PLAN_CODE:
                    const codesToSearch = this.utilsService.splitInput(queryInput);
                ...
                case Constants.QUERY_MODES.CHANNEL_CLASSIFIED:
                    ...
                    if (channelStatusSelection === Constants.STATUS_TYPES.IN_SALE) {
                        data = data.filter(item => item.channels.some(c => channelSelection.has(c.channel) && c.status === Constants.STATUS_TYPES.IN_SALE));
                    } else if (channelStatusSelection === Constants.STATUS_TYPES.STOPPED) {
                        data = data.filter(item => {
                            const relevantChannels = item.channels.filter(c => channelSelection.has(c.channel));
                            return relevantChannels.length > 0 && !relevantChannels.some(c => c.status === Constants.STATUS_TYPES.IN_SALE);
                        });
                    }
                    break;
            }
            ...
            if (searchKeyword) {
                ...
                data = data.filter(item => Object.values(item).some(value => String(value === null ? '' : value).toLowerCase().includes(keyword)));
            }
        ...
        async loadSinglePolpln(planCode, signal) {
            const { polplnDataCache } = this.stateService.get();
            if (polplnDataCache.has(planCode) && polplnDataCache.get(planCode) !== null) return;
        
            polplnDataCache.set(planCode, '載入中...');
            this.stateService.set({ polplnDataCache });
        
            try {
                const polplnRecords = await this.apiService.fetchPolplnForCode(planCode, signal);
                ...
                else polplnDataCache.set(planCode, null); // allow retry
            }
            this.stateService.set({ polplnDataCache });
        }
    }
    ...
    class UIDialogService {
        ...
        _closeModal() {
            ...
            if (modal) {
                ...
                this.stateService.set({ modalPosition: { top, left } });
            }
        ...
        _elementDrag(e) {
            ...
            modal.style.left = `${this.dragState.initialLeft + dx}px`;
            modal.style.top = `${this.dragState.initialTop + dy}px`;
            modal.style.transform = 'none';
        }
            ...
            this._showModal(html, (modal) => {
                ...
                const handleConfirm = async () => {
                    ...
                    if (await this.apiService.verifyToken(token)) {
                        ...
                        confirmBtn.textContent = isModification ? '確定' : '驗證';
                    }
                };

                cancelBtn.addEventListener('click', () => {
                    if (isModification) this.showQueryDialog(true);
                    else {
        ...
        showQueryDialog(preserveState = false) {
            ...
            const html = `
                ...
                <div class="pct-modal-body">
                    <div class="pct-form-group">
                        ...
                        <div class="pct-mode-card-grid">
                            <div class="pct-mode-card" data-mode="${Constants.QUERY_MODES.PLAN_CODE}">商品代號</div>
                            <div class="pct-mode-card" data-mode="${Constants.QUERY_MODES.PLAN_NAME}">商品名稱</div>
                            <div class="pct-mode-card" data-mode="${Constants.QUERY_MODES.MASTER_CLASSIFIED}">主約銷售時間</div>
                            <div class="pct-mode-card" data-mode="${Constants.QUERY_MODES.CHANNEL_CLASSIFIED}">通路銷售時間</div>
                        </div>
                ...
                <div class="pct-modal-footer">
                    <div class="pct-modal-footer-left" style="display:flex; gap:10px;">
                        ...
                        ${preserveState ? '<button id="pct-clear-selection" class="pct-btn pct-btn-outline">清除選擇</button>' : ''}
                    </div>
            ...
            this._showModal(html, (modal) => {
                ...
                const updateDynamicOptions = (mode) => {
                    ...
                    switch (mode) {
                        case Constants.QUERY_MODES.PLAN_CODE:
                            content = `<label for="pct-plan-code-input">商品代碼(模糊)：(多筆可用空白、逗號或換行分隔)</label><textarea id="pct-plan-code-input" class="pct-input" rows="3"></textarea>`;
                            break;
                        case Constants.QUERY_MODES.PLAN_NAME:
                            ...
                            break;
                        case Constants.QUERY_MODES.MASTER_CLASSIFIED:
                            content = `<label>主約銷售狀態：</label><div class="pct-sub-option-grid master-status">${Object.values(Constants.STATUS_TYPES).map(s => `<div class="pct-sub-option" data-status="${s}">${s}</div>`).join('')}</div>`;
                            break;
                        case Constants.QUERY_MODES.CHANNEL_CLASSIFIED:
                            content = `
                                <label>選擇通路：</label>
                                <div class="pct-channel-option-grid" style="display: flex; flex-wrap: wrap; gap: 8px; margin-top: 8px;">
                                    ...
                                    ${Constants.FIELD_MAPS.CHANNELS.map(ch => `<div class="pct-channel-option" data-channel="${ch}">${ch}</div>`).join('')}
                                </div>
                                ...
                                <div class="pct-sub-option-grid" style="display: flex; flex-wrap: wrap; gap: 8px; margin-top: 8px;">
                                    <div class="pct-sub-option" data-range="${Constants.STATUS_TYPES.IN_SALE}">現售商品</div>
                                    <div class="pct-sub-option" data-range="${Constants.STATUS_TYPES.STOPPED}">停售商品</div>
                                </div>`;
                            break;
                    }
                    modal.querySelector('#pct-dynamic-options').innerHTML = content;
                    modal.querySelector('#pct-dynamic-options').style.display = content ? 'block' : 'none';
                    bindDynamicEvents();
                };
                ...
                const checkCanStartQuery = () => {
                    ...
                    switch (state.queryMode) {
                        case Constants.QUERY_MODES.PLAN_CODE:
                            canStart = !!(modal.querySelector('#pct-plan-code-input')?.value.trim());
                            break;
                        case Constants.QUERY_MODES.PLAN_NAME:
                            canStart = !!(modal.querySelector('#pct-plan-name-input')?.value.trim());
                            break;
                        case Constants.QUERY_MODES.MASTER_CLASSIFIED:
                ...
                const bindDynamicEvents = () => {
                    modal.querySelectorAll('.pct-sub-option[data-status]').forEach(o => o.addEventListener('click', () => {
                        o.classList.toggle('selected');
                        const selected = new Set(Array.from(modal.querySelectorAll('.pct-sub-option[data-status].selected')).map(el => el.dataset.status));
                        this.stateService.set({ masterStatusSelection: selected });
                        checkCanStartQuery();
                    }));
                    
                    const channelOptions = modal.querySelectorAll('.pct-channel-option');
                    channelOptions.forEach(o => o.addEventListener('click', () => {
                        ...
                        if (channel === 'all') {
                            ...
                            o.classList.toggle('selected');
                            const allBtn = modal.querySelector('.pct-channel-option[data-channel="all"]');
                        ...
                        const selected = new Set(Array.from(modal.querySelectorAll('.pct-channel-option.selected')).map(el => el.dataset.channel).filter(c => c !== 'all'));
                        this.stateService.set({ channelSelection: selected });
                        checkCanStartQuery();
                    }));

                    modal.querySelectorAll('.pct-sub-option[data-range]').forEach(o => o.addEventListener('click', () => {
                        modal.querySelectorAll('.pct-sub-option[data-range]').forEach(i => i.classList.remove('selected'));
                        o.classList.add('selected');
                        this.stateService.set({ channelStatusSelection: o.dataset.range });
                        checkCanStartQuery();
                    }));

                    modal.querySelectorAll('.pct-input').forEach(input => {
                        input.addEventListener('input', (e) => {
                            const { value, selectionStart, selectionEnd } = e.target;
                            e.target.value = this.utilsService.toHalfWidthUpperCase(value);
                ...
                modal.querySelectorAll('.pct-mode-card').forEach(card => card.addEventListener('click', () => {
                    modal.querySelectorAll('.pct-mode-card').forEach(c => c.classList.remove('selected'));
                    card.classList.add('selected');
                    this.stateService.set({ queryMode: card.dataset.mode });
                    updateDynamicOptions(card.dataset.mode);
                    checkCanStartQuery();
                }));

                if (preserveState) {
                    const { queryMode, queryInput, masterStatusSelection, channelSelection, channelStatusSelection } = this.stateService.get();
                    if (queryMode) {
                        const modeCard = modal.querySelector(`.pct-mode-card[data-mode="${queryMode}"]`);
                        if (modeCard) {
                            modeCard.click();
                            if (queryMode === 'planCode' || queryMode === 'planCodeName') {
                                const input = modal.querySelector('.pct-input');
                                if (input) { input.value = queryInput; input.dispatchEvent(new Event('input')); }
                            } else if (queryMode === 'masterClassified') {
                                masterStatusSelection.forEach(status => modal.querySelector(`.pct-sub-option[data-status="${status}"]`)?.click());
                            } else if (queryMode === 'channelClassified') {
                                channelSelection.forEach(channel => modal.querySelector(`.pct-channel-option[data-channel="${channel}"]`)?.click());
                                if (channelStatusSelection) modal.querySelector(`.pct-sub-option[data-range="${channelStatusSelection}"]`)?.click();
                            }
                        }
                    }
                }
                ...
                startQueryBtn.addEventListener('click', () => {
                    let inputVal = '';
                    const inputEl = modal.querySelector('.pct-input');
                    if (inputEl) inputVal = inputEl.value.trim();
                    this.stateService.set({ queryInput: inputVal });
                    this.eventBus.emit(Constants.EVENTS.QUERY_START);
                });
            }, 'query');
        }
        ...
        showResultsDialog() {
            ...
            const html = `
                ...
                <div class="pct-modal-body" style="display: flex; flex-direction: column; height: 100%;">
                    ...
                    <div class="pct-table-wrap">
                        <table class="pct-table">
                            <thead>
                                <tr>
                                    <th data-key="no">No</th> <th data-key="planCode">代號</th> <th data-key="shortName">名稱</th>
                                    <th data-key="currency">幣別</th> <th data-key="unit">單位</th> <th data-key="coverageType">類型</th>
                                    <th data-key="saleStartDate">主約銷售日</th> <th data-key="saleEndDate">主約停賣日</th>
                                    <th data-key="mainStatus">險種狀態</th> <th data-key="polpln">商品名稱</th> <th>銷售通路</th>
                                </tr>
                            </thead>
                ...
                <div class="pct-modal-footer">
                    ...
                    <div class="pct-modal-footer-right" style="display:flex; gap: 15px;">
        ...
        _setupResultsDialogEvents(modal) {
            ...
            searchInput.addEventListener('input', (e) => {
                ...
                clearTimeout(this.stateService.get().searchDebounceTimer);
                const timer = setTimeout(() => {
                    this.stateService.set({ searchKeyword: searchInput.value.trim(), pageNo: 1 });
                    this.rerenderTable();
                }, Constants.DEFAULTS.DEBOUNCE_DELAY);
                this.stateService.set({ searchDebounceTimer: timer });
            });

            modal.querySelector('#pct-clear-search').addEventListener('click', () => {
                searchInput.value = '';
                this.stateService.set({ searchKeyword: '', pageNo: 1 });
                this.rerenderTable();
            });

            modal.querySelector('#pct-table-body').addEventListener('click', async (e) => {
                const target = e.target;
                if (target.classList.contains('clickable-cell')) {
                    const cellValue = target.textContent.trim();
                    if (cellValue && cellValue !== '...') await this.utilsService.copyTextToClipboard(cellValue, this.toastService);
                } else if (target.classList.contains('pct-load-polpln-btn')) {
                    target.disabled = true;
                    target.textContent = '...';
                    await this.dataService.loadSinglePolpln(target.dataset.plancode, new AbortController().signal);
                    this.rerenderTable();
                }
            });
            ...
            modal.querySelectorAll('th[data-key]').forEach(th => th.addEventListener('click', () => {
                ...
                const { sortKey, sortAsc } = this.stateService.get();
                this.stateService.set(sortKey === key ? { sortAsc: !sortAsc } : { sortKey: key, sortAsc: true });
                this.rerenderTable();
            }));

            modal.querySelector('#pct-load-all-polpln').addEventListener('click', async (e) => {
                e.target.disabled = true;
                const itemsToLoad = this.dataService.getFilteredData().filter(item => item.polpln === null);
                if (itemsToLoad.length === 0) {
                    this.toastService.show('所有 POLPLN 皆已載入', 'info');
                    e.target.disabled = false; return;
                }
                this.progressService.show('批次載入商品名稱...');
                for (let i = 0; i < itemsToLoad.length; i += Constants.DEFAULTS.BATCH_SIZE) {
                    const batch = itemsToLoad.slice(i, i + Constants.DEFAULTS.BATCH_SIZE);
                    this.progressService.update((i + batch.length) / itemsToLoad.length * 100, `載入 ${i + batch.length}/${itemsToLoad.length}...`);
                    await Promise.all(batch.map(item => this.dataService.loadSinglePolpln(item.planCode, new AbortController().signal)));
                    this.rerenderTable();
                }
            ...
            modal.querySelector('#pct-toggle-view').addEventListener('click', (e) => {
                const isFullView = !this.stateService.get().isFullView;
                this.stateService.set({ isFullView, pageNo: 1 });
                e.target.textContent = isFullView ? '分頁顯示' : '一頁顯示';
                this.rerenderTable();
            });

            modal.querySelector('#pct-prev-page').addEventListener('click', () => {
                const { pageNo } = this.stateService.get();
                if (pageNo > 1) { this.stateService.set({ pageNo: pageNo - 1 }); this.rerenderTable(); }
            });

            modal.querySelector('#pct-next-page').addEventListener('click', () => {
                const { pageNo, pageSize } = this.stateService.get();
                const maxPage = Math.ceil(this.dataService.getFilteredData().length / pageSize);
                if (pageNo < maxPage) { this.stateService.set({ pageNo: pageNo + 1 }); this.rerenderTable(); }
            });
            
            modal.querySelector('#pct-copy-all').addEventListener('click', () => {
                const dataToCopy = this.dataService.getFilteredData();
                if (dataToCopy.length === 0) { this.toastService.show('無資料可複製', 'warning'); return; }
                ...
                this.utilsService.copyTextToClipboard(tsvContent, this.toastService);
            });
        ...
        rerenderTable() {
            ...
            if (tableBody) {
                tableBody.innerHTML = displayData.length > 0
                    ? displayData.map(item => this._renderTableRow(item, polplnDataCache)).join('')
            ...
            this._updatePaginationInfo(filteredData.length);
        ...
        _renderTableRow(item, polplnCache) {
            ...
            if (polplnValue === undefined || polplnValue === null) {
                polplnCellContent = `<button class="pct-load-polpln-btn" data-plancode="${item.planCode}">載入</button>`;
            } else {
                polplnCellContent = `<span class="clickable-cell">${this.utilsService.escapeHtml(polplnValue)}</span>`;
            }

            return `
                <tr>
                    <td class="clickable-cell">${item.no}</td>
                    <td class="clickable-cell">${this.utilsService.escapeHtml(item.planCode)}</td>
                    <td class="clickable-cell" title="${this.utilsService.escapeHtml(item.shortName)}">${this.utilsService.escapeHtml(item.shortName)}</td>
                    <td class="clickable-cell">${this.utilsService.escapeHtml(item.currency)}</td>
                    <td class="clickable-cell">${this.utilsService.escapeHtml(item.unit)}</td>
                    <td class="clickable-cell">${this.utilsService.escapeHtml(item.coverageType)}</td>
                    <td class="clickable-cell">${this.utilsService.escapeHtml(item.saleStartDate)}</td>
                    <td class="clickable-cell">${this.utilsService.escapeHtml(item.saleEndDate)}</td>
                    <td>${this._renderStatusPill(item.mainStatus)}</td>
                    <td>${polplnCellContent}</td>
                    <td>${this._renderChannelsCell(item.channels)}</td>
                </tr>`;
        }

        _renderStatusPill(status) {
            const emoji = {
                [Constants.STATUS_TYPES.IN_SALE]: '🟢',
                [Constants.STATUS_TYPES.STOPPED]: '🔴',
                [Constants.STATUS_TYPES.PENDING]: '🔵',
                [Constants.STATUS_TYPES.ABNORMAL]: '🟡',
            }[status] || '⚪';
        ...
        _renderChannelsCell(channels) {
            ...
            const statusOrder = { [IN_SALE]: 1, [STOPPED]: 2 };

            const grouped = channels.reduce((acc, { channel, status }) => {
            ...
            return Object.entries(grouped)
                .sort(([statusA], [statusB]) => (statusOrder[statusA] || 99) - (statusOrder[statusB] || 99))
                .map(([status, channels]) => {
                    const className = status === IN_SALE ? 'pct-channel-insale' : 'pct-channel-offsale';
                    return `<span class="${className}">${channels.join(' ')}</span>`;
                })
                .join('<span class="pct-channel-separator">|</span>');
        }

        _updatePaginationInfo(totalItems) {
            ...
            if (isFullView || totalItems === 0) {
                ...
                paginationEl.querySelector('#pct-page-info').textContent = `${pageNo} / ${maxPage}`;
                paginationEl.querySelector('#pct-prev-page').disabled = pageNo <= 1;
                paginationEl.querySelector('#pct-next-page').disabled = pageNo >= maxPage;
            }
        }
    }
    ...
    /**
     * @class Application
     * @description 主應用程式類別，負責初始化與協調所有服務。
     */
    class Application {
        constructor() {
        ...
        _registerServices() {
            ...
            this.container.register('constants', Constants);
        ...
        _setupEventListeners() {
            this.eventBus.on(Constants.EVENTS.QUERY_START, async () => {
                const dialogService = this.container.get('dialog');
                const dataService = this.container.get('data');
                const stateService = this.container.get('state');
                const progressService = this.container.get('progress');
                
                dialogService.showResultsDialog();
                // 讓 UI 有時間渲染
                await new Promise(resolve => setTimeout(resolve, 50));
                
                if (!stateService.get().isTokenVerified) {
                ...
                stateService.set({ currentQueryController: controller });

                try {
                    await dataService.initializeCaches(controller.signal);
                    progressService.hide();
                    dialogService.rerenderTable();
                } catch (error) {
                    progressService.hide();
                    if (error.name !== 'AbortError') {
                        dialogService.rerenderTable(); // 顯示查無資料
                    }
                } finally {
                    stateService.set({ currentQueryController: null });
                }
            });
        }
        ...
        async start() {
            try {
                ...
                const styleService = this.container.get('style');
                styleService.injectStyle();
                
                const dialogService = this.container.get('dialog');
                const toastService = this.container.get('toast');
                const stateService = this.container.get('state');
                const apiService = this.container.get('api');
                
                const storedToken = localStorage.getItem('SSO-TOKEN') || sessionStorage.getItem('SSO-TOKEN');
                if (storedToken) {
                    toastService.show('正在自動驗證 Token...', 'info', 2000);
                    if (await apiService.verifyToken(storedToken)) {
                        stateService.set({ token: storedToken, isTokenVerified: true });
                        toastService.show('Token 驗證成功', 'success');
                        dialogService.showQueryDialog();
                    } else {
                        stateService.set({ token: '', isTokenVerified: false });
                        toastService.show('自動驗證失敗，請手動輸入', 'warning');
                        dialogService.showTokenDialog();
                    }
                } else {
                    dialogService.showTokenDialog();
                }
