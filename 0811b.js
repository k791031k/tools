javascript: void((() => {
    /**
     * =================================================================================
     * 案件清單作業 - v0811.stable-merged (穩定融合版)
     * =================================================================================
     * @version      0811.stable-merged
     * @description
     * 1. [核心] 以  6穩定版為基礎進行重構，確保所有核心功能與事件迴圈正常運作。
     * 2. [UI/UX] 整合  8的橫向圖示工具列，取代舊版齒輪選單，提升操作直觀性。
     * 3. [修正] 修復篩選條件區塊的收折狀態在切換頁籤後無法記憶的問題，現在會保留使用者的操作狀態。
     * 4. [繼承] 保留 6 所有優點：獨立滾動、查詢條件顯示、欄位計數、API 快取等。
     * =================================================================================
     */
    'use strict';
    ...
    const AppConfig = (() => {
        const staticConfig = {
            VERSION: '0825.stable-merged',
            TOOL_CONTAINER_ID: 'dispatch-tool-container-v25',
            STYLE_ELEMENT_ID: 'dispatch-tool-style-v25',
            TOKEN_KEY: 'euisToken',
            BATCH_PAGE_SIZE: 50,
    ...
    const DOMHelper = (() => ({
        create: (tag, options = {}) => {
            ...
            if (options.children) {
                options.children.forEach(child => { if (child) el.appendChild(child); });
            }
            if (options.events) {
    ...
    const createUIManager = (appConfig, appState, utils, domHelper) => {
        ...
        const Modal = (() => {
            ...
            return {
                ...
                resetPosition() {
                    lastPosition = { left: '50%', top: '50%', transform: 'translate(-50%, -50%)' };
                    const modal = document.getElementById(appConfig.TOOL_CONTAINER_ID);
                    if (modal) {
                        Object.assign(modal.style, lastPosition);
                    }
    ...
    const createApiService = (appConfig, appState, uiManager) => {
        ...
        async function _fetch(url, options) {
            ...
            try {
                ...
                const fetchOptions = { ...options, headers: { ...options.headers, 'SSO-TOKEN': token, 'Content-Type': 'application/json' }, signal: appState.createAbortSignal() };
    ...
    const createUIComponents = (appConfig, uiManager, utils, domHelper) => {
        // 各 Dialog 元件的 show 方法都回傳 Promise，並透過 resolve 傳遞結果
        // 為了簡化主視窗的 Modal.show 調用，所有子視窗都自行處理創建和銷毀
        
        const createSubWindow = (opts) => {
            return new Promise(resolve => {
                const mask = domHelper.create('div', { id: 'dispatch-sub-mask', className: 'dispatch-mask sub-mask show' });
                const modal = domHelper.create('div', { id: 'dispatch-sub-modal', className: 'dispatch-modal sub-window' });
            ...
            show: (opts = {}) => {
                    ...
                    switch (key) {
                        ...
                        default:
                            ...
                            control = domHelper.create('input', {
                                ...
                                attributes: { value: utils.escapeHtml(parsedDate.display), 'data-full-value': utils.escapeHtml(parsedDate.full), placeholder: `請輸入${def.label}...` }
                            });
                ...
                return createSubWindow({
                    ...
                    onOpen: (modal, resolve, close) => {
                        ...
                        modal.querySelector('#reset-defaults-btn').onclick = () => { resolve({ action: appConfig.MODAL_ACTIONS.RESET_AND_RELOAD }); close(); };
                        modal.querySelector('#back-btn').onclick = () => { resolve({ action: appConfig.MODAL_ACTIONS.CLOSE }); close(); };
                    }
                });
            }
        };
                ...
                return createSubWindow({
                    ...
                    body: domHelper.create('div', {
                        ...
                        children: [
                            ...
                            domHelper.create('div', { style: { display: 'flex', alignItems: 'center', gap: '10px' }, children: [
                            ...
                            domHelper.create('div', { style: { marginTop: '15px' }, children: [
                    ...
                    onOpen: (modal, resolve, close) => {
                        ...
                        modal.querySelector('#back-btn').onclick = () => { resolve({ action: appConfig.MODAL_ACTIONS.BACK }); close(); };
                        confirmBtn.onclick = () => {
                    ...
                    onOpen: (modal, resolve, close) => {
                        ...
                        if (!hasFailures) {
                            ...
                            const interval = setInterval(() => {
                                countdown--;
                                if (countdown > 0) closeBtn.textContent = `關閉 (${countdown})`;
                                else { clearInterval(interval); closeAndResolve(); }
                            }, 1000);
                        }
                    }
                });
            ...
            show: ({ error }) => {
                return createSubWindow({
                    ...
                    onOpen: (modal, resolve, close) => {
                        ...
                        modal.style.height = 'auto';
                        modal.querySelector('#back-btn').onclick = () => { resolve({ action: appConfig.MODAL_ACTIONS.BACK }); close(); };
                        modal.querySelector('#retry-btn').onclick = () => { resolve({ action: appConfig.MODAL_ACTIONS.RETRY }); close(); };
                    }
                });
            }
        };
    ...
    const createAppRunner = (appConfig, appState, apiService, uiComponents, utils, uiManager, domHelper) => {
        ...
        async function handleMainView(opts = {}) {
            ...
            const caseListViewShow = (viewOpts) => {
                ...
                function _renderTable(data) {
                    if (isErrorState) {
                        ...
                        const errorLink = domHelper.create('a', {
                            ...
                            events: { click: (e) => { e.preventDefault(); handleTokenChange(); } }
                        });
                ...
                const _applyFiltersAndSort = utils.debounce(() => {
                    ...
                    elements.modal.querySelectorAll('.filter-select, .filter-input').forEach(input => {
                        if (input.value) filterValues[input.id.replace('filter-', '')] = input.value;
                    });
                ...
                function _createHeader() {
                    ...
                    const toolbar = domHelper.create('div', { className: 'header-toolbar' });
                    const buttons = [
                    ...
                    buttons.forEach(btnInfo => {
                        const tooltip = domHelper.create('span', {className: 'tooltip-text', textContent: btnInfo.title });
                        const btn = domHelper.create('button', { textContent: btnInfo.icon, children: [tooltip],
                            events: { click: () => elements.resolve({ action: btnInfo.action }) }
                        });
                                ...
                                children: [
                                    domHelper.create('div', { className: 'section-header', style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' }, children: [
                                        ...
                                        domHelper.create('div', { style: { display: 'flex', gap: '8px'}, children: [
                                            domHelper.create('button', { id: 'import-cases-btn', className: 'dispatch-btn dispatch-outline small', textContent: '匯入' }),
                                            domHelper.create('button', { id: 'clear-cases-btn', className: 'dispatch-btn dispatch-outline small', textContent: '清除' })
                                        ]})
                                    ]}),
                ...
                function _createBody() {
                    ...
                    if (activeTabId === 'manual') {
                        ...
                        const createFilterControls = () => {
                            const renderControl = (key) => {
                                ...
                                if (appConfig.TEXT_INPUT_FILTERS.includes(key)) {
                                    control = domHelper.create('input', { type: 'text', id: `filter-${key}`, className: 'dispatch-input filter-input', attributes: { placeholder: `搜尋 ${labelText}...`, ...(isErrorState && { disabled: true }) }});
                                    const label = domHelper.create('label', { style: { fontSize: '14px', display: 'block', textAlign: 'left', marginBottom: '4px' }, textContent: labelText });
                                    controlWrapper.append(label, control);
                                } else { 
                                    const options = filterOptions[key] || [];
                                    control = domHelper.create('select', { id: `filter-${key}`, className: 'dispatch-input filter-select', attributes: { ...(isErrorState && { disabled: true }) }, children: [ domHelper.create('option', { textContent: '全部', attributes: { value: '' } }), ...options.map(opt => domHelper.create('option', { textContent: opt, attributes: { value: opt } }))]});
                                    const countSpan = domHelper.create('span', { className: 'filter-label-count', textContent: `(${options.length})` });
                                    const label = domHelper.create('label', { style: { fontSize: '14px', display: 'block', textAlign: 'left', marginBottom: '4px' }, children: [ document.createTextNode(labelText), countSpan ] });
                                    controlWrapper.append(label, control);
                                }
                                return controlWrapper;
                            };
                            const filterControlsDiv = domHelper.create('div', { className: 'filter-controls', children: appConfig.SHARED_FILTER_CONFIG.map(renderControl).filter(Boolean) });
                            
                            if (state.isFiltersCollapsed) {
                                filterControlsDiv.classList.add('collapsed');
                            }
                            return filterControlsDiv;
                        };
                        ...
                        const toggleFilterBtnText = state.isFiltersCollapsed ? '展開篩選條件' : '收折篩選條件';

                        const listViewElements = [
                            ...(queryDisplay ? [queryDisplay] : []), createFilterControls(),
                            domHelper.create('div', { className: 'controls-row', style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', padding: '0 4px', flexShrink: 0 }, children: [
                                domHelper.create('span', { id: 'case-count', style: { fontSize: '14px' } }),
                                domHelper.create('div', { className: 'right-controls', style: { display: 'flex', gap: '8px', alignItems: 'center' }, children: [
                                    domHelper.create('button', { id: 'toggle-filters-btn', className: 'dispatch-btn dispatch-outline small', textContent: toggleFilterBtnText }),
                            ...
                            domHelper.create('div', { id: 'case-table-container', className: 'case-table-container', children: [domHelper.create('table', { id: 'case-table', className: 'case-table', children: [
                                domHelper.create('thead', { children: [domHelper.create('tr', { innerHTML: TableRenderer.createHeaderRow(viewConfig) })] }), domHelper.create('tbody')
                            ]})]})
                        ];
                ...
                function _createFooter() {
                    ...
                    if (activeTabId === 'manual') rightSide.appendChild(domHelper.create('button', { id: 'manual-dispatch-btn', className: 'dispatch-btn', textContent: '執行派件' }));
                    else if (viewConfig && !isErrorState) rightSide.appendChild(domHelper.create('button', { id: 'next-step-btn', className: 'dispatch-btn', textContent: '下一步 (0)', attributes: { disabled: true } }));
                    return footer;
                }
                function _bindEvents(resolve) {
                    ...
                    if (activeTabId === 'manual') {
                        ...
                        container.querySelector('#clear-cases-btn').onclick = () => { casesInput.value = ''; };
                    } else if (viewConfig) {
                        ...
                        if (toggleColumnsBtn) toggleColumnsBtn.onclick = (e) => {
                            const tableContainer = elements.modal.querySelector('.case-table-container');
                            tableContainer.classList.toggle('show-all-columns');
                            e.target.textContent = tableContainer.classList.contains('show-all-columns') ? '隱藏部分欄位' : '顯示更多欄位';
                        };
                        elements.modal.querySelector('#clear-filters-btn').onclick = () => { elements.modal.querySelectorAll('.filter-select, .filter-input').forEach(el => el.value = ''); _applyFiltersAndSort(); };
                        ...
                        if (isErrorState) {
                            ...
                            elements.modal.querySelectorAll('.filter-select, .filter-input').forEach(el => { el.addEventListener('change', _applyFiltersAndSort); if(el.tagName === 'INPUT') el.addEventListener('keyup', _applyFiltersAndSort); });
                        }
                ...
                const { headerContent, toolbar } = _createHeader();
                return uiManager.Modal.show({
                    ...
                    onOpen: (modal, resolve) => {
                        ...
                        modal.style.width = '';
                        modal.style.height = '';
                        elements = { modal, resolve, tbody: modal.querySelector('tbody'), table: modal.querySelector('#case-table'), countElem: modal.querySelector('#case-count'), nextBtn: modal.querySelector('#next-step-btn') };
                        _bindEvents(resolve);
            ...
            switch (res?.action) {
                ...
                case appConfig.MODAL_ACTIONS.RELOAD_VIEW: state.isFiltersCollapsed = true; await handleMainView({ forceFetch: true, targetTabId: appState.get('activeTabId') }); break;
                ...
                case appConfig.MODAL_ACTIONS.RESET_WINDOW: uiManager.Modal.resetPosition(); break;
                default: break;
            }
        }
