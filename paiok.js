javascript: (() => {
  /**
   * =================================================================================
   * 多功能派件整合 - v15.20 (Regex 錯誤修復)
   * =================================================================================
   * @version   15.20.0 (Refactored)
   * @description
   * 本版本主要修復 v15.19 中因正規表示式語法錯誤，導致腳本完全無法執行的問題。
   *
   * - [修復] 更正了 Utils.parseDate 函式中，日期格式匹配的正規表示式錯誤。
   * =================================================================================
   */
  'use strict';
  ...
  try {
    ...
    const UIComponents = (() => {
        const TokenDialog = {
            show: (opts = {}) => {
                ...
                return UIManager.Modal.show({
                    header: `重新檢核TOKEN (${AppConfig.ENV})`,
                    width: '450px',
        ...
        const QueryBuilderDialog = {
            show: (opts = {}) => {
                const { dynamicBatchDefaults } = opts;
                ...
                const currentPresets = {
                    personal: { ...AppConfig.DEFAULT_FILTERS.personal },
                    batch: batchPresets
                };
                ...
                const createFormColumn = (mode, data) => {
                    ...
                    const inputs = keys.map(key => {
                        ...
                        const input = DOMHelper.create('input', { 
                            type: 'text', id: `preset-${mode}-${key}`, className: 'dispatch-input', 
                            attributes: { 
                    ...
                    return DOMHelper.create('div', {
                        ...
                        children: [
                            DOMHelper.create('h3', { textContent: mode === 'personal' ? '個人案件篩選' : '批次案件篩選' }),
                            ...inputs
                        ]
                    });
                };
                ...
                const body = DOMHelper.create('div', {
                    ...
                    children: [
                        DOMHelper.create('div', {
                            ...
                            children: [ createFormColumn('batch', currentPresets.batch) ]
                        })
                    ]
                });
                ...
                return UIManager.Modal.show({
                    ...
                    onOpen: (modal, resolve) => {
                        modal.querySelector('#apply-filters-btn').onclick = () => {
                            ...
                            modes.forEach(mode => {
                                const config = AppConfig.FILTER_CONFIG;
                                const keys = [...config.batch_common, ...config.batch_advanced];
                                
                                keys.forEach(key => {
                                    const input = modal.querySelector(`#preset-${mode}-${key}`);
                                    if (input && (input.dataset.fullValue || input.value.trim())) {
                                        newPresets[mode][key] = input.dataset.fullValue || input.value.trim();
                                    }
                                });
                            });

                            resolve({ action: AppConfig.MODAL_ACTIONS.APPLY_SESSION_FILTERS, payload: newPresets });
                        };
        ...
        const CaseListView = { show: async (opts) => {
            const { tabs, activeTabId, caseList, error, viewConfig, filterOptions, initialFilters, assigneeList } = opts;
            ...
            function _createHeader() {
                return `多功能派件整合 v${AppConfig.VERSION} (${AppConfig.ENV})
                    ...
                    <div class="config-menu" id="config-menu">
                        <button id="change-token-btn">重新檢核token</button>
            ...
            function _createBody() {
                ...
                const contentContainer = DOMHelper.create('div', { style: { display: 'flex', flexDirection: 'column', flexGrow: 1 }});
            ...
            function _createListView() {
                const { type, columns, widths, foldedColumns } = viewConfig;
                ...
                const createFilterControls = () => {
                    ...
                    const renderSelect = (key) => {
                        ...
                        if (initialFilters && initialFilters[key]) {
                            const valueToSet = initialFilters[key].split(' ')[0];
                ...
                const tableWidth = Object.values(widths).reduce((sum, width) => sum + parseInt(width, 10), 0);
                        ...
                        DOMHelper.create('table', { id: 'case-table', className: 'case-table', style: { width: `${tableWidth}px` }, children: [
                            DOMHelper.create('thead', { children: [
                                DOMHelper.create('tr', { innerHTML: columns.map(key => {
                                    const def = AppConfig.COLUMN_DEFINITIONS[key];
                                    const width = widths[key] || 'auto';
                                    const isFolded = foldedColumns?.includes(key);
                                    const className = isFolded ? 'class="folded-column"' : '';
                                    if (key === 'select') {
                                       return `<th style="width:${width};" ${className}><input type="checkbox" id="select-all-header" ${isErrorState ? 'disabled' : ''}></th>`;
                                    }
                                    return `<th style="width:${width};" ${className} data-key="${def.key}" data-type="${def.type || ''}" title="點擊排序">${def.label} <span class="sort-indicator"></span></th>`;
                                }).join('') })
                            ]}),
            ...
            function _createManualOpView() {
                const assigneeSelectContainer = DOMHelper.create('div');
                const tempDialog = UIComponents.PersonnelSelectDialog.show({ selectedCount: 0, defaultUsers: assigneeList });
                
                tempDialog.then(res => { 
                  UIManager.Modal.hide();
                  elements.resolve(res); 
                });
                
                setTimeout(() => {
                    const dialogBody = document.querySelector(`#${AppConfig.TOOL_CONTAINER_ID} .dispatch-body`);
                    if(dialogBody) {
                        // Move nodes instead of cloning
                        while(dialogBody.firstChild) {
                            assigneeSelectContainer.appendChild(dialogBody.firstChild);
                        }
                        // Remove the 'p' tag that shows the selected count
                        assigneeSelectContainer.querySelector('p')?.remove();
                        document.querySelector(`#${AppConfig.TOOL_CONTAINER_ID} .dispatch-close`).click();
                    }
                }, 0);
            ...
            function _createFooter() {
                if (activeTabId === 'manual') return DOMHelper.create('div', { className: 'dispatch-footer' });
                return DOMHelper.create('div', {
                    className: 'dispatch-footer',
            ...
            function _bindHeaderEvents() {
                ...
                elements.modal.querySelector('#open-new-query-btn').onclick = () => elements.resolve({ action: AppConfig.MODAL_ACTIONS.OPEN_NEW_QUERY });
            ...
            function _bindFilterEvents() {
                ...
                if (toggleColumnsBtn) {
                    toggleColumnsBtn.onclick = (e) => {
                        ...
                        const isFolded = !table.classList.contains('show-all-columns');
                        table.classList.toggle('show-all-columns');
                        e.target.textContent = isFolded ? '隱藏部分欄位' : '顯示更多欄位';
                    };
                }
            ...
            function _bindManualOpEvents() {
                 ...
                 const manualCheckbox = container.querySelector('#manual-assignee-checkbox');
                 const manualInput = container.querySelector('#manual-assignee-input');
                 const selectEl = container.querySelector('#assignee-select');
