javascript:(function() {    'use strict';
    ...
    /**
     * =================================================================================
     * KGI Plan Code Query Tool v1.5.8 (No-Export Version)
     *
     * @version 1.5.8
     * @author Gemini AI (With user-requested modifications)
     * @description
     * 本版本為根據使用者指示，移除了所有 Excel 匯出功能的版本。
     *
     * --- 本版主要變更 ---
     * 1. [功能移除] 已完全移除 SheetJS 函式庫與所有相關的 Excel 匯出功能。
     *
     * --- 核心功能 (承襲) ---
     * 1. [欄位釘選] 查詢結果表格的前兩欄「No」和「代號」已釘選，方便水平捲動時對照。
     * 2. [中文標題] 預覽頁面中的「商品主檔」與「通路銷售」原始資料表格，
     * 欄位標題已根據指定內容中文化，並按指定順序排列。
     * 3. [邏輯修正] 所有欄位的中文對照、顯示邏輯、篩選器標題皆為最終確認版本。
     * 4. 嚴格遵守「一次查詢，前端處理」原則。
     * 5. 高效能 DOM 操作與資料處理邏輯。
     * 6. 完整的記憶體釋放機制。
     * =================================================================================
    ...
    const ConfigModule = Object.freeze({
        ...
        RESULTS_PAGE_FILTERS: [
            ...
            { key: 'noRiderAttachable', rawKey: 'noCoverageFlag', label: '可附加附約', type: 'advanced', filterType: 'select' },
        ...
        FIELD_MAPS: {
            ...
            ACCUMULATION_TYPE: {
                '01': '豁免', '02': '壽險', '03': '意外險', '04': '醫療日額', '05': '癌症險',
                '06': '意外日額', '07': '重大疾病', '08': 'UNIT LINK', '09': '不累計', '10': '重大傷病',
                '11': '長期看護', '12': '婦幼/婦嬰', '13': '初次罹癌商品(FCR)保額', '14': '圓夢人生累計體檢',
                '15': '年金保險保費累計(FRDA)', '16': '投資型年金累計帳戶價值', '17': '殘廢保險金',
                '18': '萬能壽險商品累計體檢', '19': '即期年金累計', '20': '提前給付', '21': '手術日額',
                '22': '微型保險_一年期傷害保險', '23': '微型保險_一年期定期壽險', '24': '微型保險_一年期實支實付險',
                '25': '小額終老壽險', '26': '小額終老傷害險', '91': '不累計但須通報公會類別10',
                '92': '不累計但須通報公會類別11'
            },
            POLICY_TYPE: {
                'BAY': '婦幼險', 'DAN': 'Deferred Annuity Non Profits', 'DIA': '失能險', 'GRP': '團險',
                'HTH': '健康險(Health Product)', 'IAN': 'Immediate Annuity', 'ICA': '癌症險(without PV)',
                'IPA': '意外險', 'LCN': '癌症險(With PV)', 'LFE': '壽險(Life Product)', 'RUL': 'U-Link',
                'RVA': 'Variable Annuity', 'SCL': '躉繳 外幣保單', 'SFL': '躉繳 外幣保單',
                'SUL': 'U-Link 躉繳', 'SVA': 'Variable Annuity', 'UVL': '萬能(Universal Life)'
            },
            BURIAL_AMOUNT_FLAG: {
                'Y': '第一類', 'Z': '第二類', 'B': '第三類', 'N': '第四類'
            },
            NO_COVERAGE_FLAG: {
                'N': '可附加', 'Y': '不可附加'
            }
        ...
        PREVIEW_HEADERS: {
            MASTER: {
                planCode: '險種代碼', coverageType: '主附約', cnttype: 'CNTYPE_保單類型', accumulationType: '大分類_保額累計類別', insuranceAmountUnit: '保額特殊規則',
                unitType: '單位類別', unitTransCoefficient: '計算單位轉換係數', saleStartDate: '銷售起日', saleEndDate: '銷售迄日', shortName: '商品中文名稱',
                planCodeName: '商品中文全名', crtype: '中分類_險種類別', riskMultiple: '危額係數', physicalRiskMultiple: '體檢危額係數', occupationLevel: '職業等級',
                unitTransCoefficient2: '計算單位轉換係數二', ulsType: '人生帳戶價值分類', reportInsuranceAmountUnit: '報表呈現保額特殊規則', physicalAccType: '累計體檢保額使用的商品類別',
                currency: '幣別', randomInspectionExclude: '排除抽檢商品', corporationInsuredFlag: '法人可投保商品', burialAmountFlag: '具喪葬費用', phoneInterviewLetter: '售後電訪信函',
                lowRiskLifeInsurance: '低危額壽險商品', termInsuranceFlag: '定期險商品', contractType: '契約類型', wholeLifeFlag: '是否為終身契約', policyDividendFlag: '分紅商品',
                interestSensitive: '利率變動型', investmentInsurance: '投資型商品', annuityInsurance: '年金型', installmentInsurance: '分期定期型', freeLookProvision: '適用審閱期',
                majorInjury: '重大傷病型', highAssets: '高資產商品', livingBenefits: '生存給付含祝壽', lifePhysicalAcc: '壽險體檢保額_單獨累計商品', soundBite: '售後電訪話術',
                transferFlag: '可平行移轉商品', declaration: '告知事項組合', declarationSpouse: '告知事項組合_配偶', declarationChild: '告知事項組合_子女', declarationOwner: '告知事項組合_要保人豁免',
                ahAdjust: 'A&H', covrType: '險別', regularTopupFlag: '定期超額保費', noCoverageFlag: '不可附加附約', microFlag: '微型商品',
                immediateFlag: '即期型', incomeFlag: '收益分配', reductionFlag: '扣費順序', governmentFlag: '公教商品', survivalInsuranceFlag: '生存保險金給付',
                maturityInsuranceFlag: '滿期保險金給付', stepTransFlag: '步數/數據資料傳輸', noAdvancedAgeFlag: '不適用高齡評估', singlePaymentFlag: '躉繳商品',
                nonCrsfatcaFlag: '非CRS/FATCA商品', mortgageFlag: '房貸商品', oiuFlag: 'OIU商品', versionNumber: '版號'
            },
            CHANNEL: {
                planCode: '險種代碼', seqNo: 'seqNo', channel: '業務來源', saleStartDate: '通路銷售起日', saleEndDate: '通路銷售迄日',
                assistantAcceptStartDate: '行政助理收件起日', assistantAcceptEndDate: '行政助理收件迄日', lastNoteDeadline: '最後照會期限',
                createdUser: '建檔人員', createdDate: '建檔日期', modifiedUser: '異動人員', modifiedDate: '異動日期', versionNumber: '版號'
            }
        },
    ...
    const UtilsModule = (() => {
        ...
        const getSaleStatus = (today, saleStartStr, saleEndStr) => {
...
.pct-table th { background: #f0f2f5; position: sticky; top: 0; z-index: 3; cursor: pointer; font-size: 14px; font-weight: bold; text-align: center !important; white-space: nowrap; }
...
.pct-table th:nth-child(1), .pct-table td:nth-child(1),
.pct-table th:nth-child(2), .pct-table td:nth-child(2) {
    position: sticky;
    z-index: 2;
}
.pct-table th:nth-child(1), .pct-table td:nth-child(1) { left: 0; }
.pct-table th:nth-child(2), .pct-table td:nth-child(2) { left: 4%; }
.pct-table td:nth-child(1), .pct-table td:nth-child(2) { background-color: #ffffff; }
.pct-table tr:hover td:nth-child(1), .pct-table tr:hover td:nth-child(2) { background-color: #e3f2fd; }
.pct-table tr.pct-row-abnormal-date td:nth-child(1), .pct-table tr.pct-row-abnormal-date td:nth-child(2) { background-color: var(--abnormal-bg-color); }
.pct-table tr.pct-row-abnormal-date:hover td:nth-child(1), .pct-table tr.pct-row-abnormal-date:hover td:nth-child(2) { background-color: #e3f2fd; }
.pct-table th:nth-child(1), .pct-table th:nth-child(2) { z-index: 4; }
`;
    ...
    const DataModule = (() => {
        ...
        const mergeData = () => {
            ...
            const mergedData = masterDataCache.map((rawMasterItem) => {
                ...
                return {
                    ...
                    mainStatus: UtilsModule.getSaleStatus(today, masterSaleStart, masterSaleEnd),
        ...
        const getFilteredData = (baseData) => {
            ...
            const hasStandardFilters = activeFilterKeys.length > 0;
            const hasAdvancedFilters = advancedFilterKeys.length > 0;
            const hasSearch = !!searchKeyword;

            if (hasStandardFilters || hasAdvancedFilters || hasSearch) {
                const lowerCaseKeyword = hasSearch ? searchKeyword.toLowerCase() : '';

                data = data.filter(item => {
                    if (hasStandardFilters) {
                        const standardMatch = activeFilterKeys.every(key => {
                            const filterValue = activeFilters[key];
                            const itemValue = (key === 'mainStatus') ? ConfigModule.MASTER_STATUS_TEXT[item.mainStatus] : item[key];
                    ...
                    if (hasSearch) {
                        const { _raw, ...searchableItem } = item;
                        const searchMatch = Object.values(searchableItem).some(value =>
    ...
    const ControllerModule = (() => {
        ...
        const setupResultsDialogListeners = (modal) => {
            ...
            domCache.searchInput.addEventListener('input', () => {
                ...
                timers.search = setTimeout(() => {
                    StateModule.set({ searchKeyword: domCache.searchInput.value.trim(), pageNo: 1 });
                    rerenderTable();
                }, ConfigModule.DEBOUNCE_DELAY.SEARCH);
            });
            domCache.clearSearchBtn.addEventListener('click', () => {
                domCache.searchInput.value = '';
                StateModule.set({ searchKeyword: '', pageNo: 1 });
                rerenderTable();
            });
            
            domCache.tableBody.addEventListener('click', handleTableClick);
            domCache.header.forEach(th => th.addEventListener('click', () => {
            ...
            domCache.nameToggle.addEventListener('change', (e) => {
                StateModule.set({ showPlanName: e.target.checked });
                rerenderTable();
            });

            domCache.toggleViewBtn.addEventListener('click', handleToggleView);
            domCache.prevPageBtn.addEventListener('click', () => changePage(-1));
            domCache.nextPageBtn.addEventListener('click', () => changePage(1));
            domCache.forceUpdateBtn.addEventListener('click', () => handleForceUpdate(true));
            domCache.previewAllBtn.addEventListener('click', handlePreviewAll);
            domCache.copyAllBtn.addEventListener('click', handleCopyAll);
            domCache.backToQueryBtn.addEventListener('click', () => showQueryDialog(true));
            
            buildAndBindFilterControls();
        };
        ...
        const renderNoTokenError = () => {
            ...
            if (domCache.tableBody) {
                domCache.tableBody.innerHTML = errorHtml;
                document.getElementById('pct-goto-token-settings')?.addEventListener('click', (e) => { e.preventDefault(); showTokenDialog(); });
            }
            if (domCache.resultCount) domCache.resultCount.textContent = '共 0 筆資料';
        };
        const rerenderTable = () => {
            const filteredData = DataModule.getFilteredData(initialQueryData);

            if(domCache.resultCount) domCache.resultCount.textContent = `共 ${filteredData.length} 筆資料`;
            ...
            if (!domCache.tableBody) return;

            const fragment = document.createDocumentFragment();
            if (displayData.length > 0) {
                 displayData.forEach(item => {
                    const tempContainer = document.createElement('tbody');
                    tempContainer.innerHTML = renderTableRow(item);
                    fragment.appendChild(tempContainer.firstChild);
                });
            } else {
                const tr = document.createElement('tr');
                tr.innerHTML = `<td colspan="11" style="text-align:center; padding: 20px;">查無符合條件的資料</td>`;
                fragment.appendChild(tr);
            }
            domCache.tableBody.innerHTML = '';
            domCache.tableBody.appendChild(fragment);
            
            domCache.header.forEach(th => {
        ...
        const updatePaginationInfo = (totalItems) => {
            ...
            const paginationEl = domCache.pagination;
            ...
            if (isFullView || totalItems === 0) {
                ...
                domCache.pageInfo.textContent = `${pageNo} / ${maxPage}`;
                domCache.prevPageBtn.disabled = pageNo <= 1;
                domCache.nextPageBtn.disabled = pageNo >= maxPage;
            }
        };
        ...
        const handleCopyAll = () => {
            ...
            const { showPlanName, polplnDataCache } = StateModule.get();
            const headers = ['No', '代號', '商品名稱', '幣別', '單位', '型態', '主檔銷售日', '主檔停售日', '主檔狀態', 'POLPLN', '銷售通路', '保單類型', '分類類別', '喪葬費用', '可附加附約'];
            const rows = dataToCopy.map(item => {
                const { master } = item._raw;
                ...
                return [
                    ...
                    UtilsModule.convertCodeToText(master.cnttype, ConfigModule.FIELD_MAPS.POLICY_TYPE),
                    UtilsModule.convertCodeToText(master.accumulationType, ConfigModule.FIELD_MAPS.ACCUMULATION_TYPE),
                    UtilsModule.convertCodeToText(master.burialAmountFlag, ConfigModule.FIELD_MAPS.BURIAL_AMOUNT_FLAG),
                    UtilsModule.convertCodeToText(master.noCoverageFlag, ConfigModule.FIELD_MAPS.NO_COVERAGE_FLAG)
                ];
            });
        ...
        const generatePreviewPageHTML = (data) => {
            ...
            const createTableHTML = (tableId, dataArray, keysToShow, customHeaders = {}) => {
                ...
                let tbody = '<tbody>' + dataArray.map(row => {
                    let tr = '<tr>';
                    tr += headers.map(headerKey => {
                        const val = row[headerKey] === null || row[headerKey] === undefined ? '' : row[headerKey];
                        return `<td>${UtilsModule.escapeHtml(val)}</td>`;
                    }).join('');
                    tr += '</tr>';
                    return tr;
                }).join('') + '</tbody>';
            ...
            const processedDataForTable = data.map(item => {
                ...
                return {
                    ...
                    '保單類型': UtilsModule.convertCodeToText(master.cnttype, ConfigModule.FIELD_MAPS.POLICY_TYPE),
                    '分類類別': UtilsModule.convertCodeToText(master.accumulationType, ConfigModule.FIELD_MAPS.ACCUMULATION_TYPE),
                    '喪葬費用': UtilsModule.convertCodeToText(master.burialAmountFlag, ConfigModule.FIELD_MAPS.BURIAL_AMOUNT_FLAG),
                    '可附加附約': UtilsModule.convertCodeToText(master.noCoverageFlag, ConfigModule.FIELD_MAPS.NO_COVERAGE_FLAG)
                };
            });
            const tab1Headers = ['No', '代號', '商品名稱', '幣別', '單位', '型態', '主檔銷售日', '主檔停售日', '主檔狀態', 'POLPLN', '銷售通路', '保單類型', '分類類別', '喪葬費用', '可附加附約'];
            const tab1Content = createTableHTML('preview-table-processed', processedDataForTable, tab1Headers);

            const rawMasterData = data.map(d => d._raw.master);
            const masterKeys = Object.keys(ConfigModule.PREVIEW_HEADERS.MASTER);
            const tab2Content = createTableHTML('preview-table-master', rawMasterData, masterKeys, ConfigModule.PREVIEW_HEADERS.MASTER);

            const rawChannelData = data.flatMap(d => d._raw.channels.map(channel => ({planCode: d.planCode, ...channel})));
            const channelKeys = Object.keys(ConfigModule.PREVIEW_HEADERS.CHANNEL);
            const tab3Content = createTableHTML('preview-table-channel', rawChannelData, channelKeys, ConfigModule.PREVIEW_HEADERS.CHANNEL);
...
function setupPreviewPage() {
    ...
    switchTab('tab-processed');
            ...
            return `<!DOCTYPE html><html lang="zh-Hant"><head><meta charset="UTF-8"><title>全表預覽</title>
            <style>
...
<script>(${scriptForPreviewPage.toString()})();<\/script>
            ...
            baseData.forEach(item => {
                ConfigModule.RESULTS_PAGE_FILTERS.forEach(filter => {
                    ...
                    if (filter.type === 'standard') {
                        value = (filter.key === 'mainStatus')
                            ? (item.mainStatus ? ConfigModule.MASTER_STATUS_TEXT[item.mainStatus] : undefined)
                            : item[filter.key];
                    } else {
                        value = item._raw?.master?.[filter.rawKey];
                    }

                    if (value !== undefined && value !== null) {
                        newOptions[filter.key].add(String(value));
                    }
                });
            });

            Object.keys(newOptions).forEach(key => {
                newOptions[key] = Array.from(newOptions[key]).sort();
            });

            StateModule.set({ filterOptions: newOptions });
        };

        const buildAndBindFilterControls = () => {
            const wrapper = document.getElementById('pct-filters-wrapper');
            if (!wrapper) return;

            const { isFilterVisible } = StateModule.get();
            let labelsHtml = '';
            let controlsHtml = '';
            ...
            ConfigModule.RESULTS_PAGE_FILTERS.forEach(filter => {
                ...
                if (filter.filterType === 'select') {
                    controlHtml += `<select id="filter-${filter.key}" data-key="${filter.key}" data-type="${filter.type}"><option value="">全部</option></select>`;
                } else {
                    controlHtml += `<input type="text" id="filter-${filter.key}" data-key="${filter.key}" data-type="${filter.type}" class="pct-input" placeholder="關鍵字...">`;
                }
                controlHtml += '</div>';
            ...
            const containerHtml = `<div id="pct-all-filters-container" class="${isFilterVisible ? '' : 'collapsed'}">${labelsHtml}${controlsHtml}</div>`;
            const headerHtml = `<div class="pct-filters-header"><h4>篩選器</h4><button id="pct-toggle-filters-btn" class="pct-btn pct-btn-outline">${isFilterVisible ? '收折' : '展開'}</button></div>`;
            
            wrapper.innerHTML = headerHtml + containerHtml;
            ...
            document.querySelectorAll('#pct-all-filters-container select, #pct-all-filters-container input').forEach(el => {
                el.addEventListener('input', (e) => {
                    ...
                    timers[key] = setTimeout(() => {
                        ...
                        const stateKey = type === 'standard' ? 'activeFilters' : 'advancedFilters';
                        const newFilters = { ...StateModule.get()[stateKey] };

                        if (value) newFilters[key] = value;
                        else delete newFilters[key];
                        
                        StateModule.set({ [stateKey]: newFilters, pageNo: 1 });
                        rerenderTable();
                    }, ConfigModule.DEBOUNCE_DELAY.SEARCH);
                });
            });
        ...
        const updateFilterOptionsInDOM = () => {
             const { filterOptions, activeFilters, advancedFilters } = StateModule.get();
             ConfigModule.RESULTS_PAGE_FILTERS.forEach(filter => {
                 if (filter.filterType === 'select') {
                     const selectEl = document.getElementById(`filter-${filter.key}`);
                     if (!selectEl) return;
                     ...
                     switch(filter.key) {
                        case 'category':
                            translationMap = ConfigModule.FIELD_MAPS.ACCUMULATION_TYPE;
                            displayFormat = 'key_value';
                            break;
                        case 'policyType':
                            translationMap = ConfigModule.FIELD_MAPS.POLICY_TYPE;
                            displayFormat = 'key_value';
                            break;
                        case 'hasFuneralExpenses':
                            translationMap = ConfigModule.FIELD_MAPS.BURIAL_AMOUNT_FLAG;
                            displayFormat = 'value_only';
                            break;
                        case 'noRiderAttachable':
                            translationMap = ConfigModule.FIELD_MAPS.NO_COVERAGE_FLAG;
                            displayFormat = 'value_only';
                            break;
                     }
                     ...
                     options.forEach(opt => {
                         ...
                         let displayText = opt;
                         if (translationMap) {
                             const chineseText = translationMap[opt] || '未知代碼';
                             if (displayFormat === 'key_value') {
                                displayText = `${opt}_${chineseText}`;
                             } else if (displayFormat === 'value_only') {
                                displayText = chineseText;
                             }
                         }
                         optionsHtml += `<option value="${UtilsModule.escapeHtml(opt)}" ${selected}>${UtilsModule.escapeHtml(displayText)}</option>`;
                     });
