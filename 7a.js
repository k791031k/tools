javascript: (() => {
    /**
     * =================================================================================
     * 多功能派件整合 - v15.25 (Feature & UX Refactor)
     * =================================================================================
     * @version     15.25.0
     * @description
     * [功能重構] 全面重構「開啟新查詢」功能，提供「全新空白查詢」與「帶入批次條件查詢」兩種獨立模式。
     * [介面重構] 全面重構「人工執行」分頁，改為更直觀的 Radio 單選按鈕列表，並按要求進行分組與排序。
     * [流程修復] 修復了操作後流程卡死的問題，確保「返回」、「重試」、「重新整理」能正確觸發資料的重新載入。
     * [穩定性] 延續 v15.24.1 的全局命名統一修正，確保底層穩定性。
     * =================================================================================
     */
    'use strict';
    ...
    const AppConfig = (() => {
        const staticConfig = {
            VERSION: '15.25 (Feature & UX Refactor)',
            ...
            MODAL_ACTIONS: {
                ...
                OPEN_NEW_QUERY_WITH_DEFAULTS: 'open_new_query_with_defaults', // 新增動作
        ...
        function injectStyle() {
            ...
            const style = domHelper.create('style', {
                ...
                textContent: `
                    ...
                    .manual-op-container { display: flex; flex-direction: column; gap: 15px; padding: 10px 0; height: 100%; }
                    ...
                    .personnel-radio-list { list-style: none; padding: 0; margin: 0; max-height: 180px; overflow-y: auto; display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 8px; }
                ...
                if (totalPages > 1) {
                    ...
                    for (const chunk of pageChunks) {
                        ...
                        const loadedCount = allRecords.length,
                            totalCount = firstPageData.total,
                            percent = Math.round(100 * loadedCount / totalCount);
    ...
    const createUIComponents = (appConfig, uiManager, utils, domHelper) => {
        ...
        // For brevity, re-inserting the full (and now correct) implementations
        TokenDialog.show = (opts = {}) => {
            const {
                mode
            } = opts;
        ...
        QueryBuilderDialog.show = (opts = {}) => {
            const {
                initialData = {}
            } = opts; // MODIFIED to accept initialData
            ...
            const createForm = (presets) => {
                ...
                const createInput = (key) => {
                    ...
                    return domHelper.create('div', {
                        children: [domHelper.create('label', {
                            textContent: `${label}:`,
                            style: {
                                fontSize: '14px',
                                display: 'block'
                ...
                const commonCol = domHelper.create('div', {
                    className: 'preset-form-col',
                    children: [domHelper.create('h3', {
                        textContent: '常用篩選'
                    }), ...commonInputs]
                });
                const advancedCol = domHelper.create('div', {
                    className: 'preset-form-col',
                    children: [domHelper.create('h3', {
                        textContent: '進階篩選'
                    }), ...advancedInputs]
                ...
                if (allFieldsVisible) {
                    ...
                    formContainer.appendChild(domHelper.create('div', {
                        className: 'preset-form-col',
                        children: [domHelper.create('h3', {
                            textContent: '其他欄位'
                        }), ...otherInputs]
            ...
            body.appendChild(domHelper.create('button', {
                id: 'load-more-filters-btn',
                textContent: '載入更多查詢欄位',
        ...
        PersonnelSelectDialog.show = (opts) => {
            const {
                selectedCount,
                defaultUsers
            } = opts;
            return uiManager.Modal.show({
                ...
                body: domHelper.create('div', {
                    ...
                    children: [
                        domHelper.create('p', {
                            innerHTML: `已選取 <strong>${selectedCount}</strong> 筆案件，請選擇或輸入指派人員：`,
                            style: {
                                marginTop: '0'
                            }
                        }),
                ...
                onOpen: (modal, resolve) => {
                    ...
                    const populateSelect = (list) => {
                        const specialUsers = [],
                            regularUsers = [];
                        list.forEach(user => {
                            appConfig.SPECIAL_ASSIGNEES.includes(user) ? specialUsers.push(user) : regularUsers.push(user);
                        });
                        specialUsers.sort();
                        regularUsers.sort();
                    ...
                    modal.querySelector('#back-btn').onclick = () => resolve({
        ...
        AssignmentResultDialog.show = ({
            ...
            const bodyChildren = [
                domHelper.create('p', {
                    innerHTML: `已完成對 <strong>${utils.escapeHtml(assignee)}</strong> 的派件操作。`
                }),
                domHelper.create('p', {
        ...
        CaseListView.show = async (opts) => {
                ...
                if (isErrorState) {
                    ...
                    elements.tbody.appendChild(domHelper.create('tr', {
                        children: [domHelper.create('td', {
                            attributes: {
                                colspan: viewConfig.columns.length
                            },
                            style: {
                                color: 'var(--error-color)',
                                fontWeight: 'bold',
                            ...
                            children: [domHelper.create('span', {
                                textContent: `資料載入失敗：${utils.escapeHtml(error.message)} `
                            }), errorLink]
                        })]
                    }));
            ...
            const _applyFiltersAndSort = utils.debounce(() => {
                ...
                elements.filterSelects.forEach(select => {
                    if (select.value) filterValues[select.id.replace('filter-', '')] = select.value;
                });
            ...
            function _createHeader() {
                return `多功能派件整合 v${appConfig.VERSION} (${appConfig.ENV})
                        ...
                        <div class="config-menu" id="config-menu">
                            <button data-action="${appConfig.MODAL_ACTIONS.CHANGE_TOKEN}">重新驗核token</button>
                            ...
                            <button data-action="${appConfig.MODAL_ACTIONS.CLEAR_CACHE}">清除暫存檔</button>
                        </div>`;
            }
            ...
            function _createManualOpView() {
                ...
                const createPersonnelRadioList = (assignees, specialAssignees) => {
                    const special = assignees.filter(a => specialAssignees.includes(a)).sort((a, b) => a.localeCompare(b));
                    const regular = assignees.filter(a => !specialAssignees.includes(a)).sort((a, b) => a.localeCompare(b));
                    const sortedList = [...special, ...regular];

                    const listContainer = domHelper.create('ul', {
                        className: 'personnel-radio-list'
                    });

                    sortedList.forEach(assignee => {
                        const isSpecial = specialAssignees.includes(assignee);
                ...
                const personnelSection = domHelper.create('div', {
                    ...
                    children: [
                        domHelper.create('h3', {
                            textContent: '1. 選擇指派人員'
                        }),
                        createPersonnelRadioList(assigneeList, appConfig.SPECIAL_ASSIGNEES)
                    ]
                });

                const casesSection = domHelper.create('div', {
                    className: 'manual-op-section',
                    children: [
                        domHelper.create('h3', {
                            textContent: '2. 輸入受理號碼'
                        }),
                        domHelper.create('textarea', {
                            id: 'manual-cases-input',
                            className: 'dispatch-input',
                            attributes: {
                                placeholder: '請輸入受理號碼，以逗號、空格或換行分隔',
                ...
                container.append(personnelSection, casesSection, footer);
                return container;
            }
            ...
            function _createBody() {
                ...
                const contentContainer = domHelper.create('div', {
                    style: {
                        display: 'flex',
                ...
                if (activeTabId === 'manual') {
                    ...
                    contentContainer.appendChild(domHelper.create('p', {
                        textContent: '請選擇一個頁籤。'
                    }));
                }
                return domHelper.create('div', {
                ...
                const createFilterControls = () => {
                    ...
                    const renderSelect = (key) => {
                        ...
                        const select = domHelper.create('select', {
                            ...
                            children: [domHelper.create('option', {
                                textContent: '全部',
                                attributes: {
                                    value: ''
                                }
                        ...
                        if (initialFilters && initialFilters[key]) {
                            ...
                            if (!options.includes(valueToSet)) {
                                select.appendChild(domHelper.create('option', {
                                    textContent: valueToSet,
                                    attributes: {
                                        value: valueToSet
                                    }
                                }));
                    ...
                    return [domHelper.create('div', {
                        ...
                        children: commonKeys.map(renderSelect).filter(Boolean)
                    }), domHelper.create('div', {
                        className: 'filter-controls advanced-filters',
                        children: advancedKeys.map(renderSelect).filter(Boolean)
                    })];
                };
                    ...
                    domHelper.create('div', {
                        ...
                        children: [
                            ...
                            domHelper.create('div', {
                                ...
                                children: [
                                    ...(type === 'batch' || type === 'query' ? [domHelper.create('button', {
                            ...
                            domHelper.create('table', {
                                ...
                                children: [
                                    domHelper.create('thead', {
                                        children: [
                                            domHelper.create('tr', {
                                                innerHTML: viewConfig.columns.map(key => {
                                                    const def = appConfig.COLUMN_DEFINITIONS[key];
            ...
            function _createFooter() {
                if (activeTabId === 'manual' || !viewConfig) return domHelper.create('div', {
                    className: 'dispatch-footer'
                });
                return domHelper.create('div', {
                    className: 'dispatch-footer',
            ...
            function _bindFilterEvents() {
                ...
                elements.modal.querySelector('#export-csv-btn').onclick = () => {
                    ...
                    const csvData = utils.jsonToCsv(currentData, {
                        dynamicHeaders: viewConfig.type === 'batch' || viewConfig.type === 'query'
                    });
                    utils.downloadCsv(csvData, filename);
                };
                elements.modal.querySelector('#reload-view-btn').onclick = () => elements.resolve({
                    action: appConfig.MODAL_ACTIONS.RELOAD_VIEW
                });
                if (isErrorState) {
                    elements.modal.querySelector('#retry-fetch-btn').onclick = () => elements.resolve({
                        action: appConfig.MODAL_ACTIONS.RELOAD_VIEW
                    });
                } else {
                    elements.filterSelects.forEach(select => {
                        select.onchange = _applyFiltersAndSort;
                    });
                }
            }
            ...
            function _bindManualOpEvents() {
                ...
                dispatchBtn.onclick = () => {
                    ...
                    if (cases.length === 0) return uiManager.Toast.show('請輸入至少一個受理號碼', 'error');
                    const selectedRadio = container.querySelector('input[name="manual_assignee"]:checked');
                    if (!selectedRadio) return uiManager.Toast.show('請選擇指派人員', 'error');
                    const assignee = selectedRadio.value;
                    elements.resolve({
            ...
            function _bindFooterEvents() {
                ...
                elements.nextBtn.onclick = () => {
                    ...
                    if (selectedCases.length > 0) elements.resolve({
    ...
    const createAppRunner = (appConfig, appState, apiService, uiComponents, utils, uiManager) => {
        ...
        const _generateFilterOptions = (caseList, mode) => {
            ...
            keys.forEach(key => {
                ...
                options[key] = [...uniqueValues].sort((a, b) => a.localeCompare(b, undefined, {
                    numeric: true
                }));
            });
            return options;
        ...
        async function fetchCases(mode, filters) {
            ...
            try {
                ...
                cases.sort((a, b) => {
                    const applyNumberCompare = String(b.applyNumber ?? '').localeCompare(String(a.applyNumber ?? ''), undefined, {
                        numeric: true
                    });
                    if (applyNumberCompare !== 0) return applyNumberCompare;
                    const policyNumberCompare = String(b.policyNumber ?? '').localeCompare(String(a.policyNumber ?? ''), undefined, {
                        numeric: true
                    });
        ...
        async function handleMainView(opts = {}) {
            ...
            switch (res?.action) {
                ...
                case appConfig.MODAL_ACTIONS.OPEN_NEW_QUERY:
                    await handleNewQuery(false);
                    break;
                case appConfig.MODAL_ACTIONS.OPEN_NEW_QUERY_WITH_DEFAULTS:
                    await handleNewQuery(true);
                    break;
                ...
                case appConfig.MODAL_ACTIONS.CLOSE_QUERY_TAB:
                    ...
                    break;
                case appConfig.MODAL_ACTIONS.BACK:
                    await handleMainView();
                    break;
        ...
        async function handleNewQuery(prefillDefaults = false) {
            const res = await uiComponents.QueryBuilderDialog.show({
                initialData: prefillDefaults ? _getBatchDefaultFilters() : {}
            });
            if (res?.action === appConfig.MODAL_ACTIONS.APPLY_SESSION_FILTERS) {
                ...
                if (result.data) {
                    ...
                    if (retryRes?.action === appConfig.MODAL_ACTIONS.RETRY) {
                        await handleNewQuery(prefillDefaults);
                    } else {
                        await handleMainView();
                    }
                } else {
        ...
        async function handleAssignment(assignee, cases) {
            ...
            try {
                ...
                // Force reload all data after assignment
                state.personalCases = null;
                state.batchCases = null;
                state.queryTabs = [];
                await handleMainView({
