javascript: (() => {
  /**
   * =================================================================================
   * 多功能派件整合 - v15.12 (人員清單優化版)
   * =================================================================================
   * @version   15.12.0 (Refactored)
   * @description
   * 本版本在 v15.11 的基礎上進行了重構，並根據使用者需求調整了派件人員清單邏輯。
   *
   * - [新功能] 新增預設人員與特殊人員清單。
   * - [新功能] 在「選擇派件人員」下拉選單中，特殊人員會以特殊樣式 (粗體、藍字、黃底)
   * 顯示，並被優先置於清單頂部。
   * - [架構優化] 將人員設定集中到 AppConfig 模組，方便統一維護。
   * - [UI 優化] 優化了人員選擇下拉選單的生成邏輯，以支援新的特殊顯示需求。
   * =================================================================================
   */
  'use strict';
  ...
  try {
    // === 1. 設定模組 (AppConfig) ===
    // 提供所有應用程式的靜態設定，例如版本號、ID、API 端點、預設值等。
    const AppConfig = Object.freeze({
      VERSION: '15.12 (人員清單優化版)',
    ...
    const UIComponents = (() => {
            ...
            show: (opts) => {
                ...
                return UIManager.Modal.show({
                    ...
                    onOpen: (modal, resolve) => {
                        ...
                        /**
                         * [重構] 填充下拉選單，並處理特殊人員樣式與排序
                         * @param {string[]} list - 完整的人員名單
                         */
        ...
        const AssignmentResultDialog = {
            show: (result) => {
                const { successful, failed, assignee } = result;
                            ...
                            DOMHelper.create('table', { id: 'case-table', className: 'case-table', style: { width: `${tableWidth}px` }, children: [
                                DOMHelper.create('thead', { children: [
                                    DOMHelper.create('tr', { innerHTML: columnKeys.map(key => {
                                        ...
                                        if (key === 'select') return `<th style="width:${width};"><input type="checkbox" id="select-all-header" ${isErrorState ? 'disabled' : ''}></th>`;
    ...
    const AppRunner = (() => {
        ...
        async function handleAssignment(assignee) {
            ...
            try {
                const result = await ApiService.manualAssign(state.selectedCases, assignee);
                UIManager.Progress.hide();
